...
export function paddedBounds(b: GeoBounds): GeoBounds {
  const pad = Math.max(0.5, (b.north - b.south) * 0.25);
  return { west: Math.max(-180, b.west - pad), east: Math.min(180, b.east + pad), south: Math.max(-90, b.south - pad), north: Math.min(90, b.north + pad) };
}
...
export function MapHost() {
  ...
  useEffect(() => {
    ...
    const h = hosts.get();
    if (!el) return;
    if (!h) { setMounted('missing'); dispatch({ type: 'ui/hostCapabilities', supports3D: false }); return; }
    ...
    const sendViewport = (view: ViewState) => {
      ...
      const flush = () => {
        ...
        client.request('world.viewport', { bounds: v.bounds, zoom: v.zoom }).catch((err: unknown) => console.warn('[worldview] world.viewport failed:', describeError(err)));
      };
      pendingView.current = view;
      if (now - lastViewportAt.current >= VIEWPORT_THROTTLE_MS) { flush(); return; }
      if (viewportTimer.current === null) viewportTimer.current = setTimeout(() => { viewportTimer.current = null; flush(); }, VIEWPORT_THROTTLE_MS - (now - lastViewportAt.current));
    };
    offs.push(h.on('viewChanged', sendViewport));
    offs.push(h.on('pick', (pick) => {
      if (!pick) { void actions.select(null); return; }
      ...
      else if (pick.featureId.startsWith('cluster:')) void h.flyTo({ position: pick.position, zoom: Math.min(16, h.getView().zoom + 2) });
    }));
    offs.push(h.on('hover', (hit) => actions.hover(hit?.objectId ?? null)));
    offs.push(h.on('error', ({ message, fatal }) => { if (fatal) { setMounted('error'); setErrorText(message); } else actions.notify('Renderer', message, 'MINOR'); }));
    offs.push(h.on('ready', () => { if (!disposed) { setMounted('ready'); dispatch({ type: 'ui/activeMode', mode: h.activeMode() }); sendViewport(h.getView()); } }));
    Promise.resolve(h.mount(el)).then(() => { if (!disposed) { setMounted('ready'); dispatch({ type: 'ui/activeMode', mode: h.activeMode() }); sendViewport(h.getView()); } }).catch((err: unknown) => { setMounted('error'); setErrorText(describeError(err)); });
    return () => {
      disposed = true;
  ...
  // ---- lens → host, mode → host ----
  useEffect(() => { if (lens && host) host.setLens(lens); }, [lens, host]);
  useEffect(() => { if (host && mounted === 'ready') { host.setMode(ui.mode); dispatch({ type: 'ui/activeMode', mode: host.activeMode() }); } }, [ui.mode, host, mounted, dispatch]);
  ...
  useEffect(() => {
    ...
    client.request('world.subscribe', subscription)
      .then((r) => { if (!cancelled) dispatch({ type: 'world/snapshot', objects: r.snapshot, count: r.count, subscription }); })
      .catch((err: unknown) => { if (!cancelled) actions.notify('World subscription failed', describeError(err), 'MINOR'); });
  ...
  useEffect(() => {
    ...
    const schedule = typeof requestAnimationFrame === 'function' ? requestAnimationFrame : (cb: (t: number) => void) => setTimeout(() => cb(0), 16) as unknown as number;
  ...
  const attribution = useMemo(() => {
    ...
    for (const o of world.objects.values()) {
      const text = o.provenance.attribution ?? sources.entries.find((s) => s.providerId === o.provenance.providerId)?.meta.attribution;
  ...
  return (
    <div className="wv-map" role="region" aria-label="Map">
      ...
      {mounted === 'missing' ? (
        <div className="wv-map__state"><EmptyState icon="globe" title="No map renderer is available" description="The shell was started without a renderer host. In Electron the Cesium/MapLibre host is injected; the browser demo uses the canvas host." /></div>
      ) : null}
      {mounted === 'error' ? (
        <div className="wv-map__state"><EmptyState icon="error" title="The renderer could not start" description={errorText ?? 'Unknown renderer error'} /></div>
      ) : null}
      <div className="wv-map__controls" role="group" aria-label="Map controls">
        <div className="wv-map__modes" role="radiogroup" aria-label="Render mode">
          <button type="button" role="radio" aria-checked={ui.activeMode === '2D'} className={`wv-map__mode${ui.activeMode === '2D' ? ' wv-map__mode--active' : ''}`} onClick={() => void actions.setMode('2D')} title="2D map (2)"><Icon name="map2d" size={14} /> 2D</button>
          {supports3D ? (
            <button type="button" role="radio" aria-checked={ui.activeMode === '3D'} className={`wv-map__mode${ui.activeMode === '3D' ? ' wv-map__mode--active' : ''}`} onClick={() => void actions.setMode('3D')} title="3D globe (3)"><Icon name="map3d" size={14} /> 3D</button>
          ) : null}
        </div>
        {world.selectedId ? <Button size="sm" variant="secondary" icon="close" onClick={() => actions.clearSelection()}>Clear selection</Button> : null}
      </div>
      {attribution.length ? <div className="wv-map__attribution" aria-label="Attribution">{attribution.join(' · ')}</div> : null}
    </div>
  );
}
