    ...
    render: ({ object }) => (
      <FieldList
        rows={[
          ...
          ...Object.entries(object.labels)
            .filter(([k]) => !['name', 'callsign', 'title'].includes(k))
            .map(([k, v]) => ({ label: formatObjectType(k), value: v })),
        ]}
      />
    ),
  },
        ...
        <FieldList
          rows={[
            ...
            },
            {
              label: 'Altitude',
              value:
                p && p.altitudeM !== undefined && p.altitudeM >= 0
                  ? `${formatAltitude(p.altitudeM, altUnit)}${p.altitudeDatum ? ` (${p.altitudeDatum})` : ''}`
                  : undefined,
            },
    ...
    render: ({ object, nowMs }) => (
      <div className="wv-ctx-badges">
        <StatusBadge
          ...
          title={`Observed ${formatAgo(object.observedAt, nowMs)}`}
        />
        <StatusBadge
          kind="confidence"
          value={classifyConfidence(object.confidence)}
          title="System-generated metadata, not scientific certainty"
        />
        ...
        <FieldList
          rows={[
            {
              label: 'Observed',
              value: `${formatUtcDateTime(object.observedAt)} · ${formatAgo(object.observedAt, nowMs)}`,
            },
  ...
  },
  {
    ...
    render: ({ object, sources, actions, nowMs }) => {
      ...
      for (const ref of object.sourceRefs)
        if (!byProvider.has(ref.providerId)) byProvider.set(ref.providerId, ref.observedAt);
      ...
      return (
        <div className="wv-ctx-sources">
          {[...byProvider.entries()].map(([providerId, observedAt]) => {
            ...
            return (
              <div key={providerId} className="wv-ctx-source">
                <SourceBadge
                  name={entry?.name ?? object.provenance.sourceName}
                  status={entry?.health.status ?? 'STARTING'}
                  providerId={providerId}
                  onClick={() => actions.openSource(providerId)}
                />
          ...
          />
          {terms ? (
            <Button size="sm" variant="ghost" icon="external" onClick={() => void actions.openExternal(terms)}>
              Terms of use
            </Button>
          ) : null}
        </div>
        ...
        <ul className="wv-ctx-related">
          {related.events.map((e) => (
            <li key={e.id}>
              ...
              <span className="wv-ctx-muted">{formatObjectType(e.type)}</span>
            </li>
          ))}
