...
test('registry composition: defaults + type sections at placement; same id overrides; unregister restores', () => {
  ...
  r.register('*', [
    { id: 'identity', title: 'Identity', render: () => null },
    { id: 'position', title: 'Position', render: () => null },
    { id: 'related', title: 'Related', render: () => null },
  ]);
  assert.deepEqual(
    r.sectionsFor('unknown-type').map((s) => s.id),
    ['identity', 'position', 'related'],
  );
  ...
  assert.equal(
    r.sectionsFor('aircraft').find((s) => s.id === 'position')?.title,
    'Custom position',
    'type section overrides the default with the same id',
  );
  assert.deepEqual(
    r.sectionsFor('vessel').map((s) => s.id),
    ['identity', 'position', 'related'],
  ...
  off();
  assert.deepEqual(
    r.sectionsFor('aircraft').map((s) => s.id),
    ['identity', 'position', 'related'],
  );
