...
test('session, lenses, sources, ui slices', () => {
  let s: RootState = initialState(NOW);
  const settings = { renderMode: '3D' as const, basemapId: 'b', terrainId: 't', activeLensId: 'aviation', reducedMotion: true, textScale: 1.2, updater: { automatic: false, prerelease: false }, demoMode: true, privacy: { telemetry: false as const }, providers: {} };
