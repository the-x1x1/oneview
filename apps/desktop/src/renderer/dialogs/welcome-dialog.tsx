...
export function WelcomeDialog() {
  ...
  return (
    <Dialog
      ...
      onClose={finish}
      size="md"
      description="A local-first browser for the physical world — live, historical and offline."
      footer={
        <>
          {session.appInfo?.demoMode ? <span className="wv-welcome__demo">Demo build: recorded data only</span> : null}
          <Button variant="primary" icon="globe" onClick={finish} autoFocus>
            Start with Earth
          </Button>
        </>
      }
    >
      <div className="wv-welcome">
        <section className="wv-welcome__block">
          <h3>
            <Icon name="globe" size={16} /> What it shows
          </h3>
          <p>
            Aircraft, ships, satellites, earthquakes, fires, weather alerts, infrastructure and public cameras from
            documented sources, each labelled with its freshness and provenance. Lenses on the left focus the world; the
            timeline at the bottom replays what has been recorded.
          </p>
        </section>
        <section className="wv-welcome__block">
          <h3>
            <Icon name="key" size={16} /> Privacy boundary
          </h3>
          <ul>
        ...
        <section className="wv-welcome__block wv-welcome__links">
          ...
          {offline.status?.capabilities.localMap === false || !offline.status ? (
            <Button
              variant="ghost"
              icon="download"
              onClick={() => {
                finish();
                void actions.installOfflinePack();
              }}
            >
          ...
          <Button
            ...
            onClick={() => {
              finish();
              actions.openDialog('attribution');
            }}
          >
            Data &amp; attribution
          </Button>
