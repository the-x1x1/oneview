...
export function RelatedPanel() {
  ...
  if (objects.length === 0 && events.length === 0) return <EmptyState icon="link" title="No related items" description="No events are linked to this selection, and nothing else is within 250 km." compact />;
  return (
    <Panel title="Related" subtitle={`${events.length} linked events · ${objects.length} nearby objects`} flush>
      <ul className="wv-related">
        ...
        {objects.length ? <li className="wv-related__heading wv-ctx-muted">Nearby — the closest objects within 250 km, whatever their type</li> : null}
        {objects.map((o) => (
