...
export function CollectionsPanel() {
  ...
  return (
    <Panel
      title="Collections"
      subtitle={`${collections.collections.length} collections`}
      actions={
        <IconButton icon="upload" label="Import collection" size="sm" onClick={() => void actions.importCollection()} />
      }
    >
      <form className="wv-inline-form" onSubmit={create}>
        <input
          ...
          maxLength={80}
        />
        <Button size="sm" type="submit" variant="primary" icon="plus" disabled={!newName.trim()}>
          Create
        </Button>
      </form>
      {collections.collections.length === 0 ? (
        <EmptyState
          ...
          description="Create one to keep places, objects and events together with notes."
        />
      ) : (
        <ul className="wv-collections" role="list">
          {collections.collections.map((c) => (
            <li
              key={c.id}
              className={`wv-collections__item${c.id === collections.activeId ? ' wv-collections__item--active' : ''}`}
            >
              <button
                type="button"
                className="wv-collections__select"
                aria-pressed={c.id === collections.activeId}
                onClick={() => actions.setActiveCollection(c.id)}
              >
      ...
      actions={
        <>
          ...
          <IconButton
            ...
            size="sm"
            onClick={() => void actions.deleteCollection(collection.id)}
          />
        </>
      }
    >
      {renaming ? (
        <form
          ...
          onSubmit={(e) => {
            e.preventDefault();
            void actions.renameCollection(collection.id, name);
            setRenaming(false);
          }}
        >
          <input
      ...
      <div className="wv-ctx-actions">
        {hasSelection ? (
          <Button size="sm" icon="bookmark" onClick={() => void actions.addSelectionToCollection(collection.id)}>
            Add selection
          </Button>
        ) : null}
        <Button size="sm" icon="pin" onClick={() => void actions.addLocationToCollection(collection.id)}>
          Add map centre
        </Button>
      </div>
      {collection.items.length === 0 ? (
        ...
        <ul className="wv-collection-items">
          {collection.items.map((item) => (
            <CollectionItemRow key={item.id} collectionId={collection.id} item={item} />
          ))}
        </ul>
      )}
      <p className="wv-ctx-muted">
        Created {formatUtcDateTime(collection.createdAt)} · updated {formatUtcDateTime(collection.updatedAt)}
      </p>
    </Section>
  );
}
  ...
  return (
    <li className="wv-collection-items__row">
      <div className="wv-collection-items__head">
        ...
        {canGo ? (
          <button type="button" className="wv-ctx-link wv-truncate" onClick={goTo}>
            {item.title}
          </button>
        ) : (
          <span className="wv-truncate">{item.title}</span>
        )}
        ...
        <IconButton
          ...
          size="sm"
          onClick={() => void actions.removeFromCollection(collectionId, item.id)}
        />
      </div>
      {item.position ? (
        <span className="wv-ctx-muted wv-num">
          {formatCoordinates(item.position.latitude, item.position.longitude, 3)}
        </span>
      ) : null}
      {editing ? (
        <form
          ...
          onSubmit={(e) => {
            e.preventDefault();
            void actions.setItemNote(collectionId, item.id, note);
            setEditing(false);
          }}
        >
          <textarea
        ...
        <p className="wv-collection-items__note">{item.note}</p>
      ) : null}
    </li>
  );
}
