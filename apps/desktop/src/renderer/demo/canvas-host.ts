...
import type { AttributionEntry, FeatureUpdate, LensDefinition, PickResult, RenderFeature, RenderMode, RendererEvents, ViewState } from '@worldview/render-core';
...
const STYLE_COLORS: Record<string, string> = {
  aircraft: '#3ec5d5', vessel: '#62d9e6', satellite: '#c9a0ff', earthquake: '#e8903a', 'earthquake.shallow': '#e0605c', 'earthquake.intermediate': '#e8903a', 'earthquake.deep': '#d6b64a',
  fire: '#e0605c', 'weather-alert': '#d6b64a', 'weather-station': '#8b97ff', camera: '#a3b3c4', transit: '#35c98a', infrastructure: '#7f8d9e', launch: '#c9a0ff', sensor: '#35c98a', trail: '#62d9e6',
};
...
export class CanvasRendererHost implements RendererHostLike {
  ...
  private readonly listeners: Listeners = { viewChanged: new Set(), pick: new Set(), hover: new Set(), ready: new Set(), error: new Set(), frame: new Set() };
  ...
  setMode(_mode: RenderMode): void { /* only 2D is available in the demo host */ }
  activeMode(): '2D' { return '2D'; }
  supportsMode(mode: '2D' | '3D'): boolean { return mode === '2D'; }

  getView(): ViewState {
    return { center: { ...this.center }, zoom: this.zoom, altitudeM: zoomToAltitudeM(this.zoom, this.center.latitude), headingDegrees: 0, pitchDegrees: -90, bounds: this.bounds() };
  }

  flyTo(target: { position: GeoPosition; altitudeM?: number; zoom?: number; bounds?: GeoBounds }, opts?: { durationMs?: number }): Promise<void> {
    const toZoom = target.bounds ? this.zoomForBounds(target.bounds) : target.zoom ?? (target.altitudeM !== undefined ? altitudeToZoom(target.altitudeM, target.position.latitude) : this.zoom);
    ...
    const to = { lat: target.position.latitude, lon: target.position.longitude, zoom: Math.max(MIN_ZOOM, Math.min(MAX_ZOOM, toZoom)) };
    if (this.animation !== null) cancelAnimationFrame(this.animation);
    if (duration === 0 || typeof requestAnimationFrame !== 'function') { this.center = { latitude: to.lat, longitude: to.lon }; this.zoom = to.zoom; this.redraw(); this.scheduleViewChanged(0); return Promise.resolve(); }
    return new Promise((resolve) => {
      ...
      const step = (t: number) => {
        ...
        else { this.animation = null; this.scheduleViewChanged(0); resolve(); }
      };
      this.animation = requestAnimationFrame(step);
    });
  }

  select(featureId: string | null): void { this.selectedId = featureId; this.redraw(); }
  setLens(lens: LensDefinition): void { this.lens = lens; this.redraw(); }
  setAttribution(entries: AttributionEntry[]): void { this.attribution = entries; this.redraw(); }

  setFeatures(update: FeatureUpdate): void {
    if (update.replaceLayers?.length) for (const [id, f] of [...this.features]) if (update.replaceLayers.includes(f.layer)) this.features.delete(id);
  ...
  private scale(): number { return TILE * Math.pow(2, this.zoom); }
  ...
  private bounds(): GeoBounds {
    const nw = this.unproject(0, 0), se = this.unproject(this.width, this.height);
    const s = this.scale();
    const coversWorld = this.width >= s;
    return { west: coversWorld ? -180 : nw.longitude, east: coversWorld ? 180 : se.longitude, north: nw.latitude, south: se.latitude };
  }
  ...
  private onPointerUp(e: PointerEvent): void {
    ...
    this.drag = null;
    if (!drag) return;
    if (!drag.moved) { const p = this.local(e); this.emit('pick', this.pick(p.x, p.y)); }
  ...
  private onWheel(e: WheelEvent): void {
    ...
    this.center = { latitude: Math.max(-85, Math.min(85, this.center.latitude + (before.latitude - after.latitude))), longitude: ((this.center.longitude + (before.longitude - after.longitude) + 540) % 360) - 180 };
    this.redraw();
    this.scheduleViewChanged();
  }
  ...
  private pick(x: number, y: number): PickResult | null {
    ...
    for (const f of this.features.values()) {
      ...
      const g = f.geometry;
      const pos = g.kind === 'point' ? g.position : g.kind === 'cluster' ? g.position : g.kind === 'polygon' ? centroid(g.rings[0] ?? []) : null;
      if (!pos) continue;
  ...
  private scheduleViewChanged(delayMs = 120): void {
    ...
    this.viewTimer = setTimeout(() => { this.viewTimer = null; this.emit('viewChanged', this.getView()); }, delayMs);
  }
  ...
  private draw(): void {
    ...
    this.emit('frame', { fps: dt > 0 ? Math.min(60, Math.round(1000 / Math.max(dt, 16.7))) : 60, featureCount: this.features.size });
  }
  ...
  private drawGraticule(ctx: CanvasRenderingContext2D): void {
    ...
    for (let lon = -180; lon <= 180; lon += 30) {
      const a = this.project(85, lon), b = this.project(-85, lon);
      ctx.beginPath(); ctx.moveTo(a.x, a.y); ctx.lineTo(b.x, b.y); ctx.stroke();
    }
    for (let lat = -60; lat <= 60; lat += 30) {
      const a = this.project(lat, -180), b = this.project(lat, 180);
      const s = this.scale();
      if (this.width < s) { ctx.beginPath(); ctx.moveTo(0, a.y); ctx.lineTo(this.width, b.y); ctx.stroke(); }
      else { ctx.beginPath(); ctx.moveTo(a.x, a.y); ctx.lineTo(b.x, b.y); ctx.stroke(); }
    }
    ctx.strokeStyle = '#243040';
    const eqA = this.project(0, -180), eqB = this.project(0, 180);
    ctx.beginPath(); ctx.moveTo(Math.min(0, eqA.x), eqA.y); ctx.lineTo(Math.max(this.width, eqB.x), eqB.y); ctx.stroke();
  }
  ...
  private drawDensity(ctx: CanvasRenderingContext2D, f: RenderFeature): void {
    ...
    const nw = this.project(b.north, b.west), se = this.project(b.south, b.east);
    ...
    ctx.globalAlpha = 1;
    if (f.style.label && se.x - nw.x > 24) { ctx.fillStyle = '#e6edf3'; ctx.font = '11px Inter, system-ui, sans-serif'; ctx.textAlign = 'center'; ctx.fillText(f.style.label, (nw.x + se.x) / 2, (nw.y + se.y) / 2 + 4); }
  }
  ...
  private drawPolygon(ctx: CanvasRenderingContext2D, f: RenderFeature): void {
    ...
    for (const ring of f.geometry.rings) {
      ctx.beginPath();
      ring.forEach((p, i) => { const s = this.project(p.latitude, p.longitude); if (i === 0) ctx.moveTo(s.x, s.y); else ctx.lineTo(s.x, s.y); });
      ctx.closePath();
      ctx.fillStyle = color; ctx.globalAlpha = f.style.selected ? 0.35 : 0.18; ctx.fill();
      ctx.globalAlpha = 1; ctx.strokeStyle = color; ctx.lineWidth = f.style.selected ? 2 : 1; ctx.stroke();
    }
  }
  ...
  private drawLine(ctx: CanvasRenderingContext2D, f: RenderFeature): void {
    ...
    ctx.beginPath();
    f.geometry.positions.forEach((p, i) => { const s = this.project(p.latitude, p.longitude); if (i === 0) ctx.moveTo(s.x, s.y); else ctx.lineTo(s.x, s.y); });
  ...
  private drawCluster(ctx: CanvasRenderingContext2D, f: RenderFeature): void {
    ...
    ctx.fillStyle = '#e6edf3'; ctx.font = '600 11px Inter, system-ui, sans-serif'; ctx.textAlign = 'center';
  ...
  private drawPoint(ctx: CanvasRenderingContext2D, f: RenderFeature): void {
    ...
    if (f.style.icon === 'aircraft' || f.style.icon === 'vessel') {
      ...
      ctx.beginPath(); ctx.arc(p.x, p.y, Math.max(2, size / 2), 0, Math.PI * 2);
      ctx.fillStyle = color; ctx.fill();
      if (f.style.styleClass.startsWith('earthquake')) { ctx.strokeStyle = color; ctx.lineWidth = 1; ctx.globalAlpha = 0.35; ctx.beginPath(); ctx.arc(p.x, p.y, size, 0, Math.PI * 2); ctx.stroke(); }
    }
    ctx.globalAlpha = 1;
    if (selected || hovered) {
      ctx.beginPath(); ctx.arc(p.x, p.y, Math.max(6, size) + 5, 0, Math.PI * 2);
      ctx.strokeStyle = selected ? '#e6edf3' : '#62d9e6'; ctx.lineWidth = selected ? 2 : 1; ctx.stroke();
    }
  ...
  private drawAttribution(ctx: CanvasRenderingContext2D): void {
    const text = this.attribution.filter((a) => a.onScreen).map((a) => a.text).join(' · ');
    if (!text) return;
...
function centroid(ring: GeoPosition[]): GeoPosition | null {
  ...
  let lat = 0, lon = 0;
  for (const p of ring) { lat += p.latitude; lon += p.longitude; }
  return { latitude: lat / ring.length, longitude: lon / ring.length };
}

export function createCanvasHost(): CanvasRendererHost { return new CanvasRendererHost(); }
