...
export class DemoClient implements WorldClient {
  ...
  tick(nowMs: number): void {
    ...
    this.emit('world.changed', { added: [], updated: updated.map((o) => o.id), removed: [], refreshed: [], at: new Date(nowMs).toISOString(), objectCount: this.allObjects().length, objects: updated, freshness: [] });
  ...
  on<E extends EventChannel>(event: E, listener: (payload: WorldEvents[E]) => void): () => void {
    ...
    return () => { set?.delete(listener as Listener<EventChannel>); };
  }
  ...
  private handle<C extends RequestChannel>(channel: C, request: RequestOf<C>): ResponseOf<RequestChannel> {
    ...
    switch (channel) {
      case 'app.info': return { version: '0.1.0-demo', channel: 'dev', commit: 'demo', demoMode: true, platform: 'browser' };
      case 'app.openExternal': { const { url } = request as RequestOf<'app.openExternal'>; return { opened: this.openExternalHook ? this.openExternalHook(url) : false }; }
      ...
      case 'world.get': { const { objectId } = request as RequestOf<'world.get'>; return this.findObject(objectId) ?? null; }
      case 'world.track': { const { objectId } = request as RequestOf<'world.track'>; return this.track(objectId, nowMs); }
      case 'world.events': { const q = request as WorldQuery; const items = [...this.events.values()].filter((e) => !q.eventTypes || q.eventTypes.includes(e.type)).slice(0, q.limit ?? 500); return { items, total: items.length, truncated: false, basis: 'live', evaluatedAt: iso } satisfies WorldQueryResult<WorldEvent>; }
      case 'world.event': { const { eventId } = request as RequestOf<'world.event'>; return this.events.get(eventId) ?? null; }
      ...
      case 'world.related': {
        ...
        if (eventId) { const ev = this.events.get(eventId); return { objects: ev ? ev.objectIds.map((id) => this.findObject(id)).filter((o): o is WorldObject => !!o) : [], events: [] }; }
        return { objects: [], events: objectId ? [...this.events.values()].filter((e) => e.objectIds.includes(objectId)) : [] };
      }
      case 'world.whatChanged': {
        const { region, time } = request as RequestOf<'world.whatChanged'>;
        const within = (e: WorldEvent) => e.geometry?.type === 'Point' && regionContains(region, { latitude: e.geometry.coordinates[1], longitude: e.geometry.coordinates[0] }) && e.startAt >= time.start && e.startAt <= time.end;
        const newEvents = [...this.events.values()].filter(within);
        return { region, time, newEvents, endedEvents: [], statusChanges: [], countChanges: [{ objectType: 'earthquake', before: 0, after: newEvents.length }], newAlerts: newEvents.filter((e) => (e.severity ? SEVERITY_ORDER[e.severity] : 0) >= SEVERITY_ORDER.MODERATE) };
      }
      case 'world.viewport': return undefined;

      case 'sources.list': return this.sources;
      case 'sources.manifest': return null; // demo has no provider manifests loaded; the panel falls back to health meta
      case 'sources.setEnabled': { const { providerId, enabled } = request as RequestOf<'sources.setEnabled'>; this.setEnabled(providerId, enabled, nowMs); return undefined; }
      case 'sources.refresh': { const { providerId } = request as RequestOf<'sources.refresh'>; this.refresh(providerId, nowMs); return undefined; }
      case 'sources.connection': return connectionFrom(this.sources, nowMs);
      case 'sources.settings.get': { const { providerId } = request as RequestOf<'sources.settings.get'>; return this.providerSettings.get(providerId) ?? {}; }
      case 'sources.settings.set': { const { providerId, settings } = request as RequestOf<'sources.settings.set'>; this.providerSettings.set(providerId, settings); return undefined; }

      case 'credentials.has': { const { key } = request as RequestOf<'credentials.has'>; return { present: this.credentials.has(key) }; }
      case 'credentials.set': { const { key } = request as RequestOf<'credentials.set'>; this.credentials.add(key); /* value intentionally discarded: the demo never stores secrets */ return undefined; }
      case 'credentials.delete': { const { key } = request as RequestOf<'credentials.delete'>; this.credentials.delete(key); return undefined; }

      case 'history.query': return this.queryObjects({ ...(request as WorldQuery) }, nowMs, true);
      case 'history.availability': return this.availability(nowMs);
      case 'timeline.get': return this.timelineNow(nowMs);
      case 'timeline.set': {
      ...
      case 'search.query': return this.search(request as RequestOf<'search.query'>);
      case 'lenses.list': return [...BUILT_IN_LENSES, ...this.customLenses];
      case 'lenses.save': { const lens = request as LensDefinition; this.customLenses = [...this.customLenses.filter((l) => l.id !== lens.id), { ...lens, builtIn: false }]; const all = [...BUILT_IN_LENSES, ...this.customLenses]; this.emit('lenses.changed', all); return all; }
      case 'lenses.delete': { const { id } = request as RequestOf<'lenses.delete'>; this.customLenses = this.customLenses.filter((l) => l.id !== id); const all = [...BUILT_IN_LENSES, ...this.customLenses]; this.emit('lenses.changed', all); return all; }

      case 'collections.list': return this.collections;
      case 'collections.save': { const c = request as Collection; this.collections = [...this.collections.filter((x) => x.id !== c.id), c].sort((a, b) => a.createdAt.localeCompare(b.createdAt)); return this.collections; }
      case 'collections.delete': { const { id } = request as RequestOf<'collections.delete'>; this.collections = this.collections.filter((c) => c.id !== id); return this.collections; }
      case 'collections.export': {
        ...
        const path = this.downloadHook(`${c.name.replace(/[^a-z0-9-]+/gi, '_')}.worldview-collection.json`, 'application/json', new TextEncoder().encode(JSON.stringify({ format: 'worldview-collection/1', collection: c }, null, 2)));
        return path ? { path } : { cancelled: true };
      }
      case 'collections.import': return { imported: null, issues: ['Import needs a file picker; not available in the browser demo'] };

      case 'watchzones.list': return this.watchzones;
      case 'watchzones.save': { const z = request as WatchZone; this.watchzones = [...this.watchzones.filter((x) => x.id !== z.id), z]; return this.watchzones; }
      case 'watchzones.delete': { const { id } = request as RequestOf<'watchzones.delete'>; this.watchzones = this.watchzones.filter((z) => z.id !== id); return this.watchzones; }
      ...
      case 'offline.status': return this.offlineStatus(nowMs);
      case 'offline.installPack': return { installed: null, issues: ['Worldpack installation needs a file picker; not available in the browser demo'] };
      case 'offline.removePack': case 'offline.setPackEnabled': return this.offlineStatus(nowMs);
      ...
      case 'export.objects': {
        ...
        const body = format === 'csv'
          ? ['id,type,latitude,longitude,observedAt,freshness', ...result.items.map((o) => `${o.id},${o.type},${o.position?.latitude ?? ''},${o.position?.longitude ?? ''},${o.observedAt},${o.freshness}`)].join('\n')
          : format === 'geojson'
            ? JSON.stringify({ type: 'FeatureCollection', features: result.items.filter((o) => o.position).map((o) => ({ type: 'Feature', id: o.id, geometry: { type: 'Point', coordinates: [o.position!.longitude, o.position!.latitude] }, properties: { ...o.properties, type: o.type, observedAt: o.observedAt, freshness: o.freshness, attribution: o.provenance.attribution } })) })
            : JSON.stringify({ recorded: true, exportedAt: iso, objects: result.items }, null, 2);
        const path = this.downloadHook(`worldview-export.${format}`, format === 'csv' ? 'text/csv' : 'application/json', new TextEncoder().encode(body));
      ...
      case 'diagnostics.export': {
        if (!this.downloadHook) return { cancelled: true };
        const path = this.downloadHook('worldview-diagnostics.json', 'application/json', new TextEncoder().encode(JSON.stringify({ redacted: true, ...this.diagnostics(nowMs) }, null, 2)));
        return path ? { path, redacted: true } : { cancelled: true };
      }

      case 'updater.state': return this.updater;
      case 'updater.check': this.updater = { ...this.updater, lastCheckedAt: iso }; this.emit('updater.changed', this.updater); return this.updater;
      case 'updater.install': return this.updater;
    }
  ...
  private rebuildMovers(nowMs: number): void {
    ...
    for (const o of [...buildAircraft(nowMs, this.startMs), ...buildVessels(nowMs, this.startMs), buildSatellite(nowMs, this.startMs)]) next.set(o.id, o);
    this.movers = next;
  }

  private allObjects(): WorldObject[] { return [...this.staticObjects.values(), ...this.movers.values()]; }

  private findObject(id: string): WorldObject | undefined { return this.staticObjects.get(id) ?? this.movers.get(id); }
  ...
  private visibleObjects(nowMs: number): WorldObject[] {
    ...
    return [...this.staticObjects.values()].filter((o) => o.type === 'earthquake' && o.observedAt <= cursor && this.enabledProvider(o.provenance.providerId)).map((o) => ({ ...o, freshness: 'HISTORICAL' as const }));
  }

  private enabledProvider(providerId: string): boolean { return this.sources.find((s) => s.providerId === providerId)?.enabled ?? true; }
  ...
  private queryObjects(q: WorldQuery, nowMs: number, historical = false): WorldQueryResult<WorldObject> {
    let items = historical ? this.visibleObjects(nowMs).filter((o) => o.type === 'earthquake') : this.visibleObjects(nowMs);
    ...
    if (q.text) { const t = q.text.toLowerCase(); items = items.filter((o) => o.id.toLowerCase().includes(t) || Object.values(o.labels).some((l) => l.toLowerCase().includes(t))); }
    const total = items.length;
    const limit = q.limit ?? 1000;
    return { items: items.slice(0, limit), total, truncated: total > limit, basis: historical ? 'historical' : 'live', evaluatedAt: new Date(nowMs).toISOString() };
  }
  ...
  private track(objectId: string, nowMs: number): ResponseOf<'world.track'> {
    ...
    for (let t = from; t <= nowMs; t += stepMs) {
      const at = o.type === 'aircraft' ? buildAircraft(t, this.startMs).find((a) => a.id === objectId) : o.type === 'vessel' ? buildVessels(t, this.startMs).find((v) => v.id === objectId) : buildSatellite(t, this.startMs);
      if (at?.position) points.push({ observedAt: new Date(t).toISOString(), latitude: at.position.latitude, longitude: at.position.longitude, ...(at.position.altitudeM !== undefined ? { altitudeM: at.position.altitudeM } : {}) });
    }
    return points;
  }

  private availability(nowMs: number): TimelineState['availability'] {
    const eq = [...this.staticObjects.values()].filter((o) => o.type === 'earthquake').map((o) => Date.parse(o.observedAt));
    if (eq.length === 0) return [];
    return [{ objectType: 'earthquake', ranges: [{ start: new Date(Math.min(...eq)).toISOString(), end: new Date(nowMs).toISOString() }] }];
  }

  private timelineNow(nowMs: number): TimelineState {
    const iso = new Date(nowMs).toISOString();
    const t = this.timeline;
    const range: TimeRange = t.mode === 'LIVE' ? { start: new Date(nowMs - (Date.parse(t.range.end) - Date.parse(t.range.start))).toISOString(), end: iso } : t.range;
  ...
  private setEnabled(providerId: string, enabled: boolean, nowMs: number): void {
    ...
    const to: ProviderStatus = enabled ? (e.meta.credentialsRequired.some((k) => !this.credentials.has(k)) ? 'AUTH_REQUIRED' : 'LIVE') : 'DISABLED';
    e.enabled = enabled;
  ...
  private refresh(providerId: string, nowMs: number): void {
    ...
    e.health = to === 'LIVE' && from !== to ? { ...rest, status: to, lastAttempt: iso, lastSuccess: iso, credentialState: 'present' } : { ...e.health, status: to, lastAttempt: iso };
  ...
  private search({ text, limit = 12 }: RequestOf<'search.query'>): SearchResult[] {
    ...
    for (const p of DEMO_PLACES) {
      ...
      if (score) out.push({ kind: 'place', id: `place:${p.id}`, title: p.name, subtitle: `${p.region} · recorded place index`, position: p.position, bounds: p.bounds, source: 'local-index', score });
    }
    for (const o of this.allObjects()) {
      ...
      const r: SearchResult = { kind: 'object', id: o.id, title, subtitle: `${o.type} · ${o.provenance.sourceName}`, source: 'world-state', score: title.toLowerCase().startsWith(q) ? 80 : 50 };
      if (o.position) r.position = o.position;
      out.push(r);
    }
    for (const e of this.events.values()) {
      if (!e.title.toLowerCase().includes(q)) continue;
      const r: SearchResult = { kind: 'event', id: e.id, title: e.title, subtitle: `${e.type} · recorded`, source: 'world-state', score: 55 };
      if (e.geometry?.type === 'Point') r.position = { latitude: e.geometry.coordinates[1], longitude: e.geometry.coordinates[0] };
      out.push(r);
    }
  ...
  private buildFeed(nowMs: number): FeedItem[] {
    ...
    for (const e of this.events.values()) {
      const pos: GeoPosition | undefined = e.geometry?.type === 'Point' ? { latitude: e.geometry.coordinates[1], longitude: e.geometry.coordinates[0] } : undefined;
      const item: FeedItem = { id: `feed:${e.id}`, at: e.startAt, eventId: e.id, title: e.title, subtitle: e.summary, severity: e.severity ?? 'INFO', type: e.type, recorded: true };
    ...
    if (alert?.position) items.push({ id: 'feed:alert:hsw', at: alert.observedAt, objectId: alert.id, title: 'High Surf Warning — Oahu north shores', subtitle: 'NWS Honolulu · until this evening', severity: 'MODERATE', type: 'weather-alert', position: alert.position, recorded: true });
    const fire = this.staticObjects.get('fire-detection:firms:viirs-noaa20-20260921-0530-3412');
    if (fire?.position) items.push({ id: 'feed:fire:viirs', at: fire.observedAt, objectId: fire.id, title: 'VIIRS fire detection near Chatsworth, CA', subtitle: 'NOAA-20 · brightness 341 K · FRP 18 MW', severity: 'MINOR', type: 'wildfire-cluster', position: fire.position, recorded: true });
    items.push({ id: 'feed:source:aisstream', at: new Date(nowMs - 25 * 60_000).toISOString(), title: 'AISStream went OFFLINE', subtitle: 'websocket closed; reconnecting with backoff', severity: 'MINOR', type: 'source-status-change', recorded: true });
    items.push({ id: 'feed:source:celestrak', at: new Date(nowMs - 2 * 3600_000).toISOString(), title: 'CelesTrak is STALE', subtitle: 'serving cached elements (3 h old)', severity: 'INFO', type: 'source-status-change', recorded: true });
    return items.sort((a, b) => b.at.localeCompare(a.at));
  }

  private offlineStatus(nowMs: number): OfflineStatus {
    return { connection: connectionFrom(this.sources, nowMs), packs: [], capabilities: { localMap: false, localSearch: true, history: true, collections: true, localAircraft: this.enabledProvider(DEMO_PROVIDERS.aircraft.id) } };
  }

  private diagnostics(nowMs: number): DiagnosticsSnapshot {
    return {
      app: { version: '0.1.0-demo', channel: 'dev', commit: 'demo', demoMode: true, startedAt: new Date(this.startMs).toISOString() },
      runtime: { electron: 'not running (browser demo)', chrome: 'browser', node: 'not available in renderer', platform: 'browser', arch: 'unknown' },
      providers: this.sources,
      database: { status: 'ok', backend: 'in-memory (demo)', sizeBytes: 0, partitions: 1, message: 'History is served from recorded fixtures' },
      offline: this.offlineStatus(nowMs),
      renderer: { active: this.settings.renderMode === '3D' ? '3D' : '2D', webgl2: false, gpu: 'canvas (demo host)' },
      sidecars: [{ id: 'go2rtc', status: 'not-configured' }, { id: 'readsb', status: 'not-configured', message: 'Demo aircraft are a recorded track' }],
  ...
  feedSize(minimum?: SeverityClass): number { return this.feed.filter((f) => SEVERITY_ORDER[f.severity] >= (minimum ? SEVERITY_ORDER[minimum] : 0)).length; }
}
