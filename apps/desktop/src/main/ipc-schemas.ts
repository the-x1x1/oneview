import { s, boundsSchema, positionSchema, regionSchema, timeRangeSchema, worldQuerySchema, type Schema } from '@worldview/world-model';
import type { RequestChannel, RequestOf, Collection, WatchZone, CameraSourceInput, WorldSubscription, TimelineState } from '@worldview/ipc-contract';
...
const voidSchema: Schema<void> = {
  kind: 'void',
  parse: (value, path = '') => (value === undefined || value === null ? { ok: true, value: undefined } : { ok: false, issues: [{ path, message: 'expected no payload' }] }),
};
...
const httpsUrl = s.refine(s.string({ min: 8, max: 2048 }), (v) => {
  let u: URL;
  try { u = new URL(v); } catch { return 'invalid url'; }
...
const cameraUrl = s.refine(s.string({ min: 8, max: 2048 }), (v) => {
  let u: URL;
  try { u = new URL(v); } catch { return 'invalid url'; }
  return ['rtsp:', 'rtsps:', 'http:', 'https:'].includes(u.protocol) ? undefined : 'camera urls must be rtsp(s):// or http(s)://';
});
...
const renderingRuleSchema = s.object({
  ...
  sizeBy: s.optional(s.object({ property: s.string({ min: 1, max: 128 }), min: s.number(), max: s.number(), scale: s.tuple([s.number({ min: 0, max: 512 }), s.number({ min: 0, max: 512 })]) })),
  colorBy: s.optional(s.object({ property: s.string({ min: 1, max: 128 }), bands: s.array(s.object({ upTo: s.number(), suffix: s.string({ min: 1, max: 32 }) }), { max: 32 }) })),
  basePriority: s.number({ min: 0, max: 1000 }),
  densityCellDeg: s.optional(s.object({ global: s.optional(s.number({ min: 0.01, max: 90 })), continental: s.optional(s.number({ min: 0.01, max: 90 })), regional: s.optional(s.number({ min: 0.01, max: 90 })), local: s.optional(s.number({ min: 0.01, max: 90 })) })),
...
}, { strict: true }) as unknown as Schema<LensDefinition>;
...
}, { strict: true }) as unknown as Schema<Collection>;
...
export const watchZoneSchema = s.object({
  ...
  enabled: s.boolean(),
  createdAt: iso,
}, { strict: true }) as unknown as Schema<WatchZone>;
...
}, { strict: true }) as unknown as Schema<WorldSubscription>;
...
}, { strict: true }) as unknown as Schema<CameraSourceInput>;
...
}, { strict: true }) as unknown as Schema<Partial<Pick<TimelineState, 'mode' | 'cursor' | 'speed' | 'range'>>>;
...
export const REQUEST_SCHEMAS: RequestSchemas = {
  ...
  'world.track': s.object({ objectId: id, time: s.optional(timeRangeSchema) }, { strict: true }) as Schema<RequestOf<'world.track'>>,
  'world.events': worldQuerySchema,
  'world.event': eventIdRequest,
  'world.subscribe': subscriptionSchema,
  'world.related': s.object({ objectId: s.optional(id), eventId: s.optional(id) }, { strict: true }) as Schema<RequestOf<'world.related'>>,
  'world.whatChanged': s.object({ region: regionSchema, time: timeRangeSchema }, { strict: true }) as Schema<RequestOf<'world.whatChanged'>>,
  'world.viewport': s.object({ bounds: boundsSchema, zoom: s.number({ min: 0, max: 30 }) }, { strict: true }) as Schema<RequestOf<'world.viewport'>>,
  ...
  'history.availability': s.object({ objectTypes: s.optional(s.array(objectType, { max: 64 })) }, { strict: true }) as Schema<RequestOf<'history.availability'>>,
  'timeline.get': voidSchema,
  'timeline.set': timelinePatchSchema,

  'search.query': s.object({ text: s.string({ max: 500 }), bias: s.optional(positionSchema), limit: s.optional(s.number({ min: 1, max: 200, integer: true })) }, { strict: true }) as Schema<RequestOf<'search.query'>>,
  ...
  'feed.recent': s.object({ limit: s.optional(s.number({ min: 1, max: 500, integer: true })), minimumSeverity: s.optional(severity) }, { strict: true }) as Schema<RequestOf<'feed.recent'>>,
  ...
  'export.objects': s.object({ query: worldQuerySchema, format: s.enum(['geojson', 'json', 'csv'] as const) }, { strict: true }),
...
export function schemaFor(channel: string): Schema<unknown> | undefined {
  return Object.prototype.hasOwnProperty.call(REQUEST_SCHEMAS, channel) ? (REQUEST_SCHEMAS[channel as RequestChannel] as Schema<unknown>) : undefined;
}
