...
test('router: every channel rejects a malformed payload and accepts a well-formed one', async () => {
  ...
  const bad: Partial<Record<(typeof REQUEST_CHANNELS)[number], unknown>> = {
    ...
    'sources.definitions.setEnabled': { file: '../settings.json', enabled: true },
    'sources.definitions.draft': { url: 'http://example.org/stations.json' },
    'sources.definitions.save': { id: '../escape', definition: {} },
