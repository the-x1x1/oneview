...
test('router: runtime events fan out to attached windows; targeted events reach one window; destroyed windows are dropped', () => {
  ...
  const settings: AppSettings = { renderMode: '2D', basemapId: 'x', terrainId: 'y', activeLensId: 'overview', reducedMotion: false, textScale: 1, updater: { automatic: false, prerelease: false }, demoMode: false, privacy: { telemetry: false }, providers: {} };
