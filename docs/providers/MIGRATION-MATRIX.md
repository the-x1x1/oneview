...
- **Known gaps** ("usgs-earthquakes known gaps: …", "… a non-numeric magnitude drops the field, not the event", "… which malformed rows each side refuses …", "… a reviewed definition still caps retention at seven days"):
  ...
  - **Retention.** The provider sets no retention cap, and earthquakes are kept indefinitely (`history-store/src/retention.ts`). A definition's policy carried one — seven days unless it named a number — and could not say "none" until A8 landed: a reviewed definition now sets `"maxRetentionSeconds": null` and the registry record below does. The licence audit does not compare this field.
...
**Landed at integration (2026-09-23, `develop`):** A1 (the `{TOKEN}` placeholder is kept
literal in the path; `param` never renames it), A2 (`ID_VALUE` is any non-blank string up to
256 characters), A7 (`connectors/` in the test roots and the type-check include) and A8
(`"maxRetentionSeconds": null` on a reviewed definition means no cap). `migration.test.ts`
asserts the fixed behaviour where it asserted the defects. A3–A6 go to the refactor pass and
the roadmap's deferred list; FIRMS stays HYBRID until A4's `concat` builds its id.
