...
Every bespoke provider, classified against what the connector layer ([ADR-013](../adr/ADR-013-connector-architecture.md)) can carry today, with the evidence for each call. Written by phase [`provider-migration`](../roadmap/phases/provider-migration.md) against `develop @ e7622a3`. Nothing here changes what the app does: every bespoke provider stays registered and on until the integrator retires one, provider by provider.

## What the classes mean

| Class       | Meaning                                                                                                                                                                                                                                                                                                                      |
| ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **MIGRATE** | A definition on a Wave 1 connector produces the same external ids, positions, observation times and payload values as the provider's normalizer on the provider's own fixtures. Any remaining difference is listed, and none is read by an event rule or identity, or weakens a safety check.                                |
...
**Evidence.** Every claim below is asserted by `connectors/examples/migrated/migration.test.ts` (test names are quoted) or by the shared suite through a definition's sidecar (`pnpm connector:test`). A difference the test asserts is a _known gap_: if an amendment closes one, that assertion fails, and the row here is updated with it.
...
Sixteen providers: the fifteen packages under `providers/` (`cctv-public` registers three) — the same sixteen ids the runtime reserves against definition ids (`providerIds()` in `providers/registry`).
...
- **Known gaps** ("usgs-earthquakes known gaps: …", "… a non-numeric magnitude drops the field, not the event"):
  ...
  - A non-numeric magnitude drops the field instead of rejecting the event (`malformed-rows.geojson`: the definition admits `hv74012345` without a magnitude; the provider refuses it). An out-of-range magnitude is kept.
...
- **What stops it:**
  ...
  - **A bad key reads as "no fires".** FIRMS answers `Invalid MAP_KEY.` with status 200; the provider reports AUTH, the `csv` connector reads a header row and no records and stays LIVE ("nasa-firms: … an empty, healthy catalogue to the csv connector"; A5 `requiredColumns`).
...
- **What the definition carries** (`aisstream-feed.json`, suite 10/10): MMSI, position (sea surface, altitude 0), `time_utc` (V8's `Date.parse` reads AISStream's Go-style `2026-09-21 08:00:03.123456789 +0000 UTC`), name, `navStatus`, and the static-data fields `imo`, `callSign`, `shipType`, `destination`, `draughtM` — equal to the provider's for the full-length MMSIs ("aisstream-io: positions, times and names match; …").
- **What it cannot:**
  - **motion.** ITU "not available" sentinels would be shown as values: `TrueHeading 511` → 151°, `COG 360` → 0°, `SOG 102.3 kn` → 52.6 m/s, where the provider drops them or falls back to the course ("aisstream-io: why motion is left out of the example — …"). The example leaves speed, course and heading out on purpose (A3 `between`);
  ...
  - the provider's **data watchdog** (a socket that opens and sends nothing is recycled after 90 s), **viewport subscription** (bounds in the frame, re-subscribed on pan, split at the antimeridian) and client-side bounds filtering (A5);
  ...
  - within one batch the newest record for an MMSI wins, so a static-data frame replaces that vessel's position report in the same batch (4 observations for the fixture's 5 frames). The next position report restores it.
...
2. In the definition, set `"review": "bundled"` and open the policy the `usgs-earthquakes` record already grants (a `bundled` definition may; a `user-configured` one may not):
   ...
   Without it the definition stays fail-closed (commercial use unknown, no export or packs) and the record must say so instead — and retiring the bespoke provider would then take earthquakes out of exports and packs.
...
| A5  | `websocket-json.ts`, `csv.ts` connectors                                         | Socket: an error-message condition → AUTH, a data-silence timeout, viewport bounds in the subscribe frame with re-subscribe. CSV: `requiredColumns`, so an error body is MALFORMED rather than an empty catalogue.                                                                                     | AIS, FIRMS                                                                          |
...
| A7  | `tools/dev/run-tests.mjs`, `tools/dev/tsconfig.tsx-loader.json`, `tsconfig.json` | Add `connectors` to the test roots and `connectors/**` to the loader and type-check includes. `migration.test.ts` lives where ownership puts it, and today neither `pnpm test` nor `pnpm typecheck` reaches it.                                                                                        | This matrix's evidence in the gate                                                  |
...
The last line needs a tsx loader configuration that covers `connectors/` for the `@worldview/*` paths — until A7, point `TSX_TSCONFIG_PATH` at a copy of `tools/dev/tsconfig.tsx-loader.json` with `"../../connectors/**/*"` added to `include`.
