...
| `ontario`        | Ontario highway cameras (Ontario 511) — needs your own free developer key since 2026-09 (Credentials → Ontario 511 developer key; 511on.ca/developers)                 | Open Government Licence – Ontario          | 120 s         |
...
its own source, polled every minute, and its rows are not kept in history (only the
addresses change). Trafikverket's API takes a key: register at
