...
- 2026-09-24 amendment (definitions in the app, for phase `source-health-ui`): the drafter moves from `tools/connector-validator/src/draft.ts` to `packages/connector-runtime/src/draft.ts` (`draftDefinition`, `slugFromUrl`; `pnpm connector:add` imports it from there). `loadDefinitionsFrom` also returns `files: DefinitionFile[]` — every file read, accepted or not, with its id and connector when it validated and why it was refused. `SourceHealthEntry.meta` gains `connector?` and `definitionFile?` (`bundled/<file>` for a shipped one). The runtime's `ConnectorDefinitions` (`packages/runtime/src/support/definitions.ts`) owns the operator's folder while the app runs: a reload stops and forgets the source of a file that went away, registers a new file's source (disabled unless the operator's switch or the file says otherwise), and stops and re-registers one whose definition changed — a source whose file is unchanged is not touched; `ProviderHost.unregister(id)` is what takes a source out (poll cancelled, socket, listener and overlays closed, settings watchers dropped, its Source Health entry removed; its settings stay with the id). A draft is one GET in the main process under the definition URL policy (https, a public host, no credentials in the URL, redirects not followed, 8 MiB); a save writes `<id>.json` with `review: user-configured` and `enabled: false` forced, never over an existing file (`wx`) or an id another source holds, then reloads.
- 2026-09-24 amendment (broker sources, from phase `mqtt`, M1 and M2): `ConnectorProviderDefinition.mqtt?: MqttSpec` (`packages/connector-sdk/src/mqtt.ts`: `mqttSpecSchema`, `MQTT_PRESETS`, the caps, `checkTopicFilter`) — topics (1–16 filters, `+` a whole level, `#` whole and last, QoS 0/1), port, TLS, username, the password by credential reference, client id, preset (`rtl_433`, `owntracks`, `meshtastic`), `itemsPath`, message filters, `flushMs`, payload and rate caps, keep-alive, and a fixed-position table (≤ 1024). There is no host field: the broker is 127.0.0.1 or the operator's `brokerHost` (ADR-003 local-endpoint policy). A definition with `mqtt` has no `endpoint`, `websocket` or `file`, and its credential must be declared. The shared suite (`runConnectorSuite`) runs a definition with `mqtt` through `testing.FixtureMqtt`: each fixture body is one message `{ topic, payload, retained? }` or a run of them, and the checks are the socket mode's plus the broker equivalents of the HTTP ones (unreachable broker, refused CONNACK, payload cap) and Local endpoint / No transport.
- 2026-09-24 amendment (pushed sources and manifest descriptions, from phases `ingest` (A1) and `telemetry` (R5)): `runConnectorSuite` runs a pushed source — a `local-process` provider with `subscribe` and no `query`, and no `endpoint`, `websocket` or `file` — through `testing.FixtureLocalAccess`'s listener: each fixture body is one push with the definition's credential as the bearer; Successful parse / Empty / Malformed (4xx) / Auth failure (401) / Rate limit (429 with Retry-After, then accepted a minute later) / Oversized payload (413) / Cancellation (the listener closed) / Mapping error (4xx, nothing emitted), and Timeout is "the listener opened". `definitionToManifest` builds the description with `manifestDescription`: ` Connector: <name>.` kept whole within the manifest's 500 characters, the operator's text shortened with an ellipsis when it must be.
