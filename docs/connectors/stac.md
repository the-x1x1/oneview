...
the scene payload (below). The full example, with every payload field spelled out, is
...
A rolling window appears in Sources as **Time window (days)** (`windowDays`): the operator's
number wins over the definition's. Unreadable values fall back to the definition's window;
out-of-range ones are clamped. **The window never reaches past how long scenes are kept**:
`imagery-scene` keeps a scene a month (live a day, recent a week), so the setting runs 1–30
and a longer window in the definition is cut to 30 days, with a validation warning — a
longer search would only fetch scenes the world refuses as expired. A definition with its
own `freshness` moves that limit: without `expireSeconds` scenes are kept while the source
lists them, and the window may run to 366 days.
...
round of conditional requests. Catalogue scenes are often years old: on the type's default
freshness anything older than a month is refused as expired, and validation warns — give
the definition a `freshness` without `expireSeconds` (the Capella example does) to keep
them. `boundsQuery` is refused for a static catalogue (it is read
...
| `_stac.href`                | The item's `self` link, else its own file in a static catalogue (https only)                                          |
...
connector fills in the time, the centre and the footprint —
...
— and every `imagery-scene` payload key (ADR-002, 2026-09-23 amendment) it does not map
itself; the Scene section of the context panel reads these:
...
| `instrument`      | the first of `properties.instruments`                                            |
| `capturedAt`      | `properties.datetime`, else `properties.start_datetime` (the observation's time) |
| `cloudCoverPct`   | `properties.eo:cloud_cover`                                                      |
...
the start is the scene's time. A key the item lacks is left out. The examples spell the
mapping out and add their own: Earth Search's `processingLevel` is the literal `L2A` (the
collection is that level), Capella's is `sar:product_type` with `instrumentMode` from
`sar:instrument_mode`; both add `footprintSimplified` and a `name` label (`properties.title`,
else the id).
...
`imagery-scene` (ADR-002, 2026-09-23 amendment); validation refuses any other. On screen the
scene is the `imagery-scene` style with the imagery glyph at its centre (a point, a marker
regionally, an icon locally), in the Overview and Space lenses, with a Scene section in the
context panel; searches for "imagery" or "scenes" find it. The footprint is on every
observation and object, but the presentation step draws an object's geometry only when the
object has no position, so footprints are not on the map yet (see
[the phase brief](../roadmap/phases/stac.md)).
