...
| `credential`      | `{ "name": "<key of credentials>", "as": "open" \| "query", "param": "<name>" }` — resolved by the host. `open` (default): handed to the connector for the subscribe frame only. `query`: put in the URL the host dials as `param=<secret>` (`token` by default) — Traccar's `?token=` — and the frame gets no secret; the connector never sees a URL with it. |
