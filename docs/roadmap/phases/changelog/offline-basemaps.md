...
- **Build your own offline basemap** (guide in `docs/OFFLINE-BASEMAPS.md`):
  ...
  2D styles draw, plus a world pack holding it for Settings → Offline packs → Install
  offline pack. The map does not draw a pack's basemap yet; see the amendment requests in
  the phase brief. The tool finds Java 21+ and the Protomaps basemap jar on the machine
  (`--java`/`JAVA_HOME`/`PATH`, `--jar`/`ONEVIEW_PLANETILER_JAR`/`PATH`). It recognises the
  jar by its contents without running it, and refuses a stock Planetiler jar, whose
  ...
  **It downloads nothing:** not Java, not Planetiler, not the extract, not the profile's
  ...
  already be on disk, and the extract's header must say it meets the region. Planetiler
  runs with every download, refresh and Wikidata switch off, without `PLANETILER_*`
  settings from the environment, and not at all when a Java options variable sets a
  `planetiler.*` property. `--dry-run` checks everything and prints the Java command. The
  output is checked before it is packed: vector tiles, the Protomaps layers, and tiles
  present. The report next to the pack records:
  - the extract's SHA-256 and the page it came from (`--osm-url`, never fetched);
  - the Java version and the jar's SHA-256;
  - the full command, how long it took, and what came out. The pack is filed under the registry record `osm-protomaps-planetiler`, credited
...
- **Martin tile sources, read side** (`packages/offline/src/basemaps/martin.ts`): a Martin
  ...
  Martin basemap yet: the renderer descriptor, catalog entry, CSP origin and package export
  it needs are amendment requests.
