...
Every file GEV ships that is *not* ordinary MIT source code — 3D models, event imagery, bundled datasets, icons, curated camera catalogs, test fixtures, documentation media — with its provenance, licence and a decision:
...
| `config/cctv_sources.shinjuku.json` | 1,658 | Three *fake* Tokyo cameras whose `url` points at Google's public sample-video bucket (`gtv-videos-bucket/sample/*.mp4`, licence of the bucket not stated). Misleading in a product; replace with a WORLDVIEW-owned sample clip for pipeline tests. |
...
## 5. Things that are *not* assets but look like them
