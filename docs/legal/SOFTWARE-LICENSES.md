...
- `integration` distinguishes _how_ the code enters the product. The distinction matters for copyleft: `external-service` (readsb) is never distributed; `sidecar` (go2rtc) is a separate binary that may be optionally downloaded; `adapted-source` (GEV, skylight) is compiled into WORLDVIEW.
