...
const opts = {
  ...
  sourceQuality: 'authoritative' as const,
  origin: 'local' as const,
};

test('parseReadsbAircraftJson: seconds → ms, message counter, shape errors', () => {
  assert.deepEqual(parseReadsbAircraftJson({ now: 1789977600.4, messages: 12, aircraft: [] }), {
    rows: [],
    nowMs: 1789977600400,
    messages: 12,
  });
...
test('normalize: ground, military, non-ICAO and stale-position rules match adsb-remote', () => {
  const ground = aircraftRowToDraft(
    { hex: 'a9e0f1', alt_baro: 'ground', lat: 21.32, lon: -157.93, seen_pos: 5.8, dbFlags: 3 },
    opts,
  );
...
test('duplicate normalizer stays in step with providers/adsb-remote (row rules are byte-identical)', () => {
  ...
  const rules = (src: string) =>
    src.slice(
      src.indexOf('export const FOOT_TO_M'),
      src.indexOf('/** adsb.lol v2 envelope') >= 0
        ? src.indexOf('/** adsb.lol v2 envelope')
        : src.indexOf('/** readsb aircraft.json envelope'),
    );
