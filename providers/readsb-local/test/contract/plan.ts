...
const fixtures = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', '..', '..', '..', 'fixtures', 'readsb-local');
...
export const plan = definePlan({
  ...
  expectations: {
    ...
    verify: (obs) => {
      ...
      if (hal.observedAt !== '2026-09-21T07:59:59.700Z') return `observedAt should be now − seen_pos (seconds), got ${hal.observedAt}`;
      if (hal.payload['rssiDb'] !== -9.8 || hal.payload['callsign'] !== 'HAL9' || hal.payload['typeCode'] !== 'A21N') return 'HAL9 payload wrong';
      if (hal.position?.altitudeDatum !== 'barometric' || hal.position.altitudeM !== 3665.2) return 'barometric altitude wrong';
      ...
      if (!ground || ground.payload['onGround'] !== true || ground.position?.altitudeDatum !== 'ground') return 'ground row wrong';
