...
export const plan = definePlan({
  ...
  expectations: {
    ...
    verify: (obs) => {
      ...
      if (hnl.observedAt !== '2026-09-01T00:00:00.000Z')
        return `observedAt should be the dataset date, got ${hnl.observedAt}`;
      if (hnl.provenance.origin !== 'local') return 'origin should be local';
      if (
        hnl.payload['iata'] !== 'HNL' ||
        hnl.payload['icao'] !== 'PHNL' ||
        hnl.payload['countryCode'] !== 'US' ||
        hnl.payload['municipality'] !== 'Honolulu' ||
        hnl.payload['type'] !== 'large_airport'
      )
        return 'PHNL payload wrong';
