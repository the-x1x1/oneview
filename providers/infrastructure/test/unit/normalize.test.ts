...
const feature = (props: Record<string, unknown>, coords: unknown = [-157.92, 21.32], geometryType = 'Point') => ({
  type: 'Feature',
...
test('feature rejections: geometry, coordinates, icao, name; unknown optional values dropped', () => {
  ...
  assert.equal(
    airportFeatureToDraft({ type: 'Feature', geometry: null, properties: null }, '2026-09-01T00:00:00.000Z', opts),
    'missing properties',
  );
  assert.equal(
    airportFeatureToDraft(
      feature(
        hnl,
        [
          [0, 0],
  ...
  assert.equal(
    airportFeatureToDraft(feature(hnl, [-157.92, 121.32]), '2026-09-01T00:00:00.000Z', opts),
    'invalid coordinates',
  );
  assert.equal(
    airportFeatureToDraft(feature({ ...hnl, icao: 'ph-nl' }), '2026-09-01T00:00:00.000Z', opts),
    'invalid icao',
  );
  assert.equal(airportFeatureToDraft(feature({ ...hnl, name: '' }), '2026-09-01T00:00:00.000Z', opts), 'missing name');
  const loose = airportFeatureToDraft(
    feature({ ...hnl, iata: 'HONO', type: 'spaceport', countryCode: 'USA' }),
    '2026-09-01T00:00:00.000Z',
    opts,
  );
...
test('collection: datasetDate handling, duplicates, non-collections', () => {
  const ok = normalizeAirportCollection(
    {
      type: 'FeatureCollection',
      datasetDate: '2025-05-05',
      features: [feature(hnl), feature(hnl), feature({ ...hnl, icao: 'PHOG', iata: 'OGG' }, [-156.43, 20.9])],
    },
    SEED_AIRPORTS_MANIFEST,
    { ...opts, hash: () => 'a'.repeat(64) },
  );
  ...
  assert.equal(
    normalizeAirportCollection({ type: 'Topology' }, SEED_AIRPORTS_MANIFEST, opts),
    'not a FeatureCollection',
  );
  assert.equal(
    normalizeAirportCollection({ type: 'FeatureCollection' }, SEED_AIRPORTS_MANIFEST, opts),
    'FeatureCollection has no features array',
  );
  assert.equal(
    normalizeAirportCollection(
      { type: 'FeatureCollection', datasetDate: 'soon', features: [] },
      SEED_AIRPORTS_MANIFEST,
      opts,
    ),
    'invalid datasetDate',
...
test('bundled dataset: 87 unique ICAO codes, every feature admitted, Hawaii present', () => {
  ...
  for (const code of ['PHNL', 'PHOG', 'PHKO', 'PHTO', 'PHLI'])
    assert.ok(
      r.observations.some((o) => o.externalId === code),
      code,
    );
  for (const o of r.observations) {
    assert.match(String(o.payload['countryCode']), /^[A-Z]{2}$/);
    assert.equal(o.payload['type'], 'large_airport');
    for (const v of [o.position!.latitude, o.position!.longitude])
      assert.equal(
        Math.abs(Number((v * 100).toFixed(6)) % 1),
        0,
        `${o.externalId} coordinate ${v} not rounded to 0.01°`,
      );
  }
});
...
test('provider: reads the granted file, works with the app offline, MALFORMED on a bad file', async () => {
  ...
  const ctx = testing.createFixtureContext({
    providerId: 'worldview-seed-airports',
    clock: new testing.VirtualClock(Date.parse('2026-09-21T08:00:10.000Z')),
    online: false,
    local: new testing.FixtureLocalAccess(files),
  });
  ...
  await assert.rejects(
    ...
    (e: ProviderError) => e.code === 'MALFORMED',
  );
});
