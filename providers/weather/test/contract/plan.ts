...
export const plan = definePlan({
  ...
  fixtures: {
    normal: () => ({
      status: 200,
      body: body('normal.geojson'),
      headers: { 'content-type': 'application/geo+json', etag: ETAG },
    }),
  ...
  expectations: {
    ...
    verify: (obs) => {
      ...
      const flood = obs.find(
        (o) => o.externalId === 'urn:oid:2.49.0.1.840.0.8ea5b47ac7ab2ff0f8f3d5d6c9c0d5c8f4e3b4b6.001.1',
      );
      if (!flood) return 'Flash Flood Warning missing';
      if (flood.observedAt !== '2026-09-21T07:45:00.000Z')
        return `sent (CDT) not normalized to UTC: ${flood.observedAt}`;
      if (flood.effectiveFrom !== '2026-09-21T07:45:00.000Z' || flood.effectiveUntil !== '2026-09-21T10:45:00.000Z')
        return 'effective window wrong';
      if (
        flood.geometry?.type !== 'Polygon' ||
        !flood.position ||
        Math.abs(flood.position.latitude - 30.35) > 0.1 ||
        Math.abs(flood.position.longitude + 97.72) > 0.1
      )
        return 'polygon/centroid wrong';
      if (
        flood.payload['severity'] !== 'SEVERE' ||
        flood.payload['event'] !== 'Flash Flood Warning' ||
        flood.payload['certainty'] !== 'Likely' ||
        flood.payload['urgency'] !== 'Immediate'
      )
        return 'CAP fields wrong';
      if (
        JSON.stringify(flood.payload['sameCodes']) !== '["048453","048491"]' ||
        JSON.stringify(flood.payload['ugcCodes']) !== '["TXC453","TXC491"]'
      )
        return 'geocode lists wrong';
      if (
        typeof flood.payload['instruction'] !== 'string' ||
        flood.payload['senderName'] !== 'NWS Austin/San Antonio TX'
      )
        return 'instruction/sender missing';
      ...
      if (svr?.payload['messageType'] !== 'Update' || !svr.quality.flags?.includes('update'))
        return 'Update message type not flagged';
      ...
      if (
        redFlag?.effectiveFrom !== '2026-09-21T18:00:00.000Z' ||
        redFlag.effectiveUntil !== '2026-09-23T03:00:00.000Z'
      )
        return 'onset/ends precedence wrong';
      if (obs.some((o) => (o.payload['description'] as string).length > 2000)) return 'description not truncated';
      if (!obs.every((o) => o.quality.sourceQuality === 'authoritative' && o.rawPayloadHash))
        return 'quality/hash missing';
      return undefined;
    },
