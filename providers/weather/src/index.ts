...
export class NwsAlertsProvider extends PollingProvider {
  ...
  protected async fetchOnce(request: ProviderQuery): Promise<{ observations: Observation[]; cacheAgeMs?: number }> {
    ...
    try {
      ...
      res.invalidate();
      throw new ProviderError('MALFORMED', 'NWS alerts response is not valid JSON', { retryable: false });
    }
    ...
    if (result.rejected.some((r) => r.index === -1)) {
      res.invalidate();
      throw new ProviderError('MALFORMED', 'NWS alerts response is not a FeatureCollection', { retryable: false });
    }
    ...
    const zoneOnly = result.rejected.filter(
      (r) => r.reason === REJECT_ZONE_ONLY || r.reason === REJECT_NO_GEOMETRY,
    ).length;
    const hard = result.rejected.length - zoneOnly;
    if (result.total > 0 && result.observations.length === 0 && hard === result.total) {
      res.invalidate();
      assertAtomicAdmission(result.total, 0, 'NWS alerts feed');
    }
    if (result.rejected.length) {
      this.context.logger.info('NWS alerts skipped', {
        total: result.total,
        admitted: result.observations.length,
        fromZones: result.fromZones,
        zoneOnly,
        unresolvedZones: result.zonesNeeded.length,
        rejected: hard,
        sample: result.rejected
          .filter((r) => r.reason !== REJECT_ZONE_ONLY)
          .slice(0, 3)
          .map((r) => r.reason),
      });
    }
...
export function mapUpstreamError(err: unknown): ProviderError {
  if (err instanceof ProviderError && err.code === 'AUTH' && err.httpStatus === 403) {
    return new ProviderError(
      'HTTP_4XX',
      'api.weather.gov refused the request (HTTP 403): an identifying User-Agent with contact information is required — set the provider contact setting',
      { httpStatus: 403, retryable: false, cause: err },
    );
  }
