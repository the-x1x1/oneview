...
import { alertUrn, featureToDraft, isoOrUndefined, mapSeverity, normalizeNwsAlerts, toGeometry, REJECT_NO_GEOMETRY, REJECT_ZONE_ONLY } from './normalize.js';
...
const opts = { receivedAt: new Date(NOW).toISOString(), nowMs: NOW, sourceRef: 'https://api.weather.gov/alerts/active?status=actual&message_type=alert,update' };

test('the normal feed yields the seven polygon alerts and explains the two skipped ones', () => {
  const r = normalizeNwsAlerts(load('normal.geojson'), { ...opts, hash: (s) => 'c'.repeat(63) + String(s.length % 10) });
...
test('timestamps: local offsets → UTC; onset/effective and ends/expires precedence; expiry grace', () => {
  ...
  const feature = (over: Record<string, unknown>, geometry: unknown = { type: 'Polygon', coordinates: [[[-97, 30], [-96, 30], [-96, 31], [-97, 31], [-97, 30]]] }) => ({
    id: 'https://api.weather.gov/alerts/urn:oid:2.49.0.1.840.0.abc.001.1', type: 'Feature', geometry,
    properties: { id: 'urn:oid:2.49.0.1.840.0.abc.001.1', event: 'Flood Warning', sent: '2026-09-21T02:00:00-05:00', effective: '2026-09-21T02:00:00-05:00', expires: '2026-09-21T05:00:00-05:00', severity: 'Severe', certainty: 'Likely', urgency: 'Expected', areaDesc: 'X', senderName: 'NWS X', description: 'd', messageType: 'Alert', category: 'Met', geocode: { SAME: ['012345'], UGC: ['TXC001'] }, ...over },
  });
  ...
  const withOnset = featureToDraft(feature({ onset: '2026-09-21T04:00:00-05:00', ends: '2026-09-22T00:00:00-05:00' }), opts);
  ...
  const future = featureToDraft(feature({ sent: '2026-09-21T04:00:00-05:00', expires: '2026-09-21T09:00:00-05:00' }), opts);
  assert.ok(typeof future !== 'string' && future.observedAt === '2026-09-21T08:05:00.000Z', 'observedAt clamped to now; payload keeps sent');
...
test('geometry policy: polygons only; zone-only and geometry-less alerts reject with distinct reasons', () => {
  ...
  assert.equal(toGeometry({ type: 'Polygon', coordinates: [[[-97, 95], [-96, 30], [-96, 31], [-97, 95]]] }), 'invalid polygon coordinates');
  assert.equal(toGeometry({ type: 'Polygon', coordinates: [[[-97, 30], [-96, 30], [-97, 30]]] }), 'polygon ring too short');
  const multi = toGeometry({ type: 'MultiPolygon', coordinates: [[[[-97, 30], [-96, 30], [-96, 31], [-97, 30]]], [[[-90, 40], [-89, 40], [-89, 41], [-90, 40]]]] });
  ...
  assert.deepEqual(rows.rejected.map((x) => x.reason), ['invalid polygon coordinates', 'unsupported geometry LineString', 'expired', 'invalid sent timestamp', 'invalid alert id', 'feature not an object', 'duplicate alert urn:oid:2.49.0.1.840.0.8ea5b47ac7ab2ff0f8f3d5d6c9c0d5c8f4e3b4b6.001.1']);
...
test('severity mapping and alert ids', () => {
  ...
  assert.equal(alertUrn({ id: 'https://api.weather.gov/alerts/urn:oid:2.49.0.1.840.0.abc.001.1', properties: {} }), 'urn:oid:2.49.0.1.840.0.abc.001.1');
  assert.equal(alertUrn({ properties: { id: 'urn:oid:2.49.0.1.840.0.abc.001.1' } }), 'urn:oid:2.49.0.1.840.0.abc.001.1');
