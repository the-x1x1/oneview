...
const byUrl = (fintraffic: string, nsw: string) => (req: { url: string }) => {
  if (req.url === FINTRAFFIC_STATIONS_URL) return { status: 200, body: body(fintraffic), headers: { 'content-type': 'application/geo+json' } };
  if (req.url === NSW_CAMERAS_URL) return { status: 200, body: body(nsw), headers: { 'content-type': 'application/json' } };
...
export const plan = definePlan({
  ...
  expectations: {
    objectTypes: ['camera'],
    minObservations: 9,
    expectObjectIds: ['camera:public-cameras:fintraffic:C0150101', 'camera:public-cameras:fintraffic:C1400301', 'camera:public-cameras:nsw:1', 'camera:public-cameras:nsw:4'],
    verify: (obs) => {
      ...
      if (obs.some((o) => o.externalId === 'fintraffic:C0999901' || o.externalId === 'fintraffic:C0150103')) return 'non-gathering station / out-of-collection preset leaked';
      if (obs.some((o) => o.externalId === 'nsw:5')) return 'off-host NSW frame URL was admitted';
      for (const o of obs) {
        const url = String(o.payload['frameUrl']);
        const host = new URL(url).hostname;
        if (o.payload['pack'] === 'fintraffic' && host !== 'weathercam.digitraffic.fi') return `Fintraffic frame off-host: ${host}`;
        if (o.payload['pack'] === 'nsw' && host !== 'webcams.transport.nsw.gov.au') return `NSW frame off-host: ${host}`;
        ...
        if (ref !== `public:${String(o.payload['pack'])}:${String(o.externalId).split(':')[1]}`) return `${o.externalId}: media ref ${String(ref)} malformed`;
        if (typeof o.payload['attribution'] !== 'string' || !/CC BY 4\.0/.test(o.payload['attribution'])) return `${o.externalId}: pack attribution missing`;
      ...
      if (bridge?.payload['headingDegrees'] !== 0 || bridge.payload['direction'] !== 'N') return 'NSW direction N not mapped';
      ...
      if (ousley?.payload['headingDegrees'] !== 225 || ousley.payload['region'] !== 'WOLLONGONG') return 'NSW S-W / region not mapped';
      const hexham = obs.find((o) => o.externalId === 'nsw:4');
      if (hexham?.payload['headingDegrees'] !== undefined || hexham?.payload['name'] !== 'Pacific Highway Hexham') return 'NSW camera without direction must fall back to title';
      return undefined;
    },
