...
test('provider sends the Digitraffic-User header and only contacts catalog hosts', async () => {
  const { provider, ctx } = await providerWith((req) => ({
    status: 200,
    body: body(req.url === FINTRAFFIC_STATIONS_URL ? 'fintraffic-stations.geojson' : 'nsw-traffic-cam.json'),
  }));
  ...
  assert.ok(
    ctx.http.requests.every((r) => /^https:\/\/(tie\.digitraffic\.fi|data\.livetraffic\.com)\//.test(r.url)),
    'catalog hosts only; frames are never fetched by the provider',
  );
});
...
test('stale-served catalogs age the observations and mark origin cached', async () => {
  ...
  const ctx = testing.createFixtureContext({
    ...
    clock,
    settings: { packs: { fintraffic: false } },
  });
  const http = {
