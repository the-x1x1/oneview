 ...
 * could confirm: Caltrans (California), the City of Austin, New York City DOT and Iowa DOT.
 *
 ...
 * `nyc-dot-webcams`, `iowa-dot-cameras`), the most restrictive of the camera records:
...
export const PUBLIC_CAMERAS_UNVERIFIED_MANIFEST: ProviderManifest = {
  ...
  version: '0.1.0',
  description:
    'Traffic cameras that agencies publish on their own sites but under no licence we could confirm: Caltrans (California), the City of Austin, New York City DOT and Iowa DOT. Off by default; switching it on is your decision. Frames are shown as served; nothing is detected, recognised or retained.',
  objectTypes: ['camera'],
  categories: ['cameras'],
  transport: 'http',
  ...
  dataPolicy: {
    ...
    attributionText:
      'Caltrans — cwwp2.dot.ca.gov (courtesy); City of Austin, TX — data.austintexas.gov (courtesy); NYC DOT — webcams.nyctmc.org (courtesy); Iowa DOT (courtesy). Image licences not confirmed.',
  },
  attribution: {
    text: 'Caltrans — cwwp2.dot.ca.gov (courtesy); City of Austin, TX — data.austintexas.gov (courtesy); NYC DOT — webcams.nyctmc.org (courtesy); Iowa DOT (courtesy). Image licences not confirmed.',
  },
  commercialReview: 'manual-review-required',
  enabledByDefault: false,
  // Catalogue hosts only; the camera gateway keeps the frame hosts.
  allowedHosts: ['cwwp2.dot.ca.gov', 'data.austintexas.gov', 'webcams.nyctmc.org', 'services.arcgis.com'],
  settings: [
    {
      key: 'packs.caltrans',
