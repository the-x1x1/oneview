...
export const fintrafficPack: CatalogPack = {
  ...
  request: {
    url: FINTRAFFIC_STATIONS_URL,
    headers: { Accept: 'application/geo+json, application/json', 'Digitraffic-User': DIGITRAFFIC_USER },
...
export function normalizeFintraffic(payload: unknown, opts: PackNormalizeOptions): PackNormalizeResult {
  ...
  features.forEach((raw: unknown, index: number) => {
    ...
    if (!STATION_ID.test(stationId)) {
      total++;
      rejected.push({ index, reason: 'invalid station id' });
      return;
    }
    ...
    const lon = coords?.[0];
    const lat = coords?.[1];
    if (!isValidLatLon(lat, lon) || !isLikelyFinland(lat, lon as number)) {
      total++;
      rejected.push({ index, reason: 'invalid coordinates' });
      return;
    }
    const alt =
      typeof coords?.[2] === 'number' && Number.isFinite(coords[2]) && coords[2] > 0
        ? Math.min(1400, coords[2])
        : undefined;
    const name = stationName(p.name, stationId);
    const region =
      typeof p.municipality === 'string' && p.municipality.trim()
        ? p.municipality.trim().slice(0, 80)
        : typeof p.province === 'string' && p.province.trim()
          ? p.province.trim().slice(0, 80)
          : undefined;
    ...
    for (const rawPreset of presets) {
      ...
      if (!PRESET_ID.test(presetId) || !presetId.startsWith(stationId)) {
        rejected.push({ index, reason: `invalid preset id for ${stationId}` });
        continue;
      }
      if (seen.has(presetId)) {
        rejected.push({ index, reason: `duplicate preset ${presetId}` });
        continue;
      }
      seen.add(presetId);
      const view = presetId.slice(stationId.length);
      const presentation =
        typeof preset.presentationName === 'string' && preset.presentationName.trim()
          ? preset.presentationName.trim().slice(0, 80)
          : undefined;
      ...
      drafts.push(
        draftFromCamera(
          ...
          opts,
          { station: stationId, preset: rawPreset as JsonValue },
        ),
      );
    }
  });
