...
export function isLikelyCsv(text: string): boolean {
  ...
  const header = (nl === -1 ? trimmed : trimmed.slice(0, nl)).trim().toLowerCase().split(',').map((f) => f.trim());
...
export function isKeyRejection(text: string): boolean {
  return /invalid\s+map_?key|map_?key\s+(is\s+)?(invalid|missing|expired)|exceeded.*transaction/i.test(text.slice(0, 500));
}
...
export function parseFirmsCsv(text: string): ParsedFirmsCsv | undefined {
  ...
  const iLat = idx('latitude')!, iLon = idx('longitude')!, iFrp = idx('frp')!, iConf = idx('confidence')!;
  const iBright = idx('bright_ti4') ?? idx('brightness');
  const iBright2 = idx('bright_ti5') ?? idx('bright_t31');
  const iScan = idx('scan'), iTrack = idx('track'), iDayNight = idx('daynight');
  const iDate = idx('acq_date')!, iTime = idx('acq_time')!, iSat = idx('satellite'), iInst = idx('instrument'), iVer = idx('version');
  ...
  for (let i = headerIndex + 1; i < lines.length; i++) {
    ...
    if (parts.length < header.length) { rejected.push({ index, reason: `truncated row (${parts.length}/${header.length} columns)` }); continue; }
    const latitude = num(parts[iLat]);
    const longitude = num(parts[iLon]);
    if (latitude === undefined || longitude === undefined || Math.abs(latitude) > 90 || Math.abs(longitude) > 180) { rejected.push({ index, reason: 'invalid coordinates' }); continue; }
    const acqDate = parts[iDate] ?? '';
    const acqTime = parts[iTime] ?? '';
    if (!Number.isFinite(acquisitionMsUtc(acqDate, acqTime))) { rejected.push({ index, reason: `invalid acquisition time ${acqDate} ${acqTime}` }); continue; }
    const row: FirmsRow = {
      latitude, longitude,
    ...
    const frp = num(parts[iFrp]); if (frp !== undefined && frp >= 0) row.frp = frp;
    const brightness = iBright !== undefined ? num(parts[iBright]) : undefined; if (brightness !== undefined && brightness > 0) row.brightness = brightness;
    const brightness2 = iBright2 !== undefined ? num(parts[iBright2]) : undefined; if (brightness2 !== undefined && brightness2 > 0) row.brightnessSecondary = brightness2;
    const scan = iScan !== undefined ? num(parts[iScan]) : undefined; if (scan !== undefined && scan > 0) row.scan = scan;
    const track = iTrack !== undefined ? num(parts[iTrack]) : undefined; if (track !== undefined && track > 0) row.track = track;
    rows.push(row);
  }
...
export function acquisitionMsUtc(acqDate: string, acqTime: string | number): number {
  ...
  const year = Number(acqDate.slice(0, 4)), month = Number(acqDate.slice(5, 7)), day = Number(acqDate.slice(8, 10));
  const hours = Number(hhmm.slice(0, 2)), minutes = Number(hhmm.slice(2, 4));
