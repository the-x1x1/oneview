...
export {
  FIRMS_HOST,
  FIRMS_AREA_BASE,
  FIRMS_KEY_PLACEHOLDER,
  FIRMS_CREDENTIAL,
  FIRMS_CREDENTIAL_KEY,
  MIN_AREA_PADDING_DEG,
  firmsAreaUrl,
  firmsRequest,
  boundsToArea,
  formatArea,
...
export class FirmsProvider extends PollingProvider {
  ...
  private async fetchSource(
    ...
    if (!parsed) {
      res.invalidate();
      if (isKeyRejection(text))
        throw new ProviderError('AUTH', `FIRMS rejected the MAP_KEY for ${source}`, { retryable: false });
      throw new ProviderError('MALFORMED', `FIRMS ${source}: response is not FIRMS CSV`, { retryable: false });
    }
    if (parsed.total > 0 && parsed.rows.length === 0) {
      res.invalidate();
      assertAtomicAdmission(parsed.total, 0, `FIRMS ${source}`);
    }
    ...
    if (rejected)
      this.context.logger.warn('rejected FIRMS rows', {
        source,
        count: rejected,
        sample: [...parsed.rejected, ...result.rejected].slice(0, 3).map((r) => r.reason),
      });
