...
test('MODIS rows normalize with numeric confidence bands and 1 km accuracy', () => {
  ...
  assert.deepEqual(
    r.observations.map((o) => o.payload['confidence']),
...
test('external ids are deterministic per source pixel and minute; duplicates and bad confidence are rejected', () => {
  ...
  assert.equal(
    detectionExternalId('VIIRS_SNPP_NRT', { ...row, acqTime: '45' }),
    'VIIRS_SNPP_NRT:2026-09-21T0045:38.99488:-121.67046',
  );
  const parsed = parseFirmsCsv(body('malformed-rows.csv'))!;
  const r = normalizeFirmsRows(parsed.rows, {
    receivedAt,
    source: 'VIIRS_SNPP_NRT',
    hash: (s) => 'f'.repeat(63) + String(s.length % 10),
  });
  assert.equal(r.observations.length, 1);
  assert.deepEqual(
    r.rejected.map((x) => x.reason),
    ['unknown confidence value "x"', 'duplicate detection VIIRS_SNPP_NRT:2026-09-21T0340:-33.6121:150.21044'],
  );
