 ...
 */
const perSource = (file?: (source: string) => string): FixtureResponder => (req) => {
  ...
  if (req.credential?.key !== FIRMS_CREDENTIAL_KEY && !/\/csv\/[A-Za-z0-9]{16,}\//.test(req.url)) return { status: 401, body: 'no MAP_KEY' };
  return { status: 200, body: body(file ? file(parsed.source) : NORMAL_BY_SOURCE[parsed.source]!), headers: { 'content-type': 'text/csv' } };
};
...
export const plan = definePlan({
  ...
  expectations: {
    ...
    verify: (obs) => {
      ...
      if (ca.payload['confidence'] !== 'high' || ca.payload['frpMw'] !== 14.53 || ca.payload['brightnessK'] !== 331.62 || ca.payload['dayNight'] !== 'night') return 'payload mapping wrong';
      if (ca.payload['satellite'] !== 'N' || ca.payload['instrument'] !== 'VIIRS' || ca.payload['source'] !== 'VIIRS_SNPP_NRT') return 'source/satellite fields wrong';
      ...
      if (low?.payload['confidence'] !== 'low' || !low.quality.flags?.includes('low-confidence')) return 'low confidence not mapped';
      ...
      if (!obs.every((o) => o.provenance.sourceRef?.includes('/api/area/csv/{MAP_KEY}/') || o.provenance.sourceRef?.includes('/api/area/csv/'))) return 'sourceRef missing';
      if (obs.some((o) => /MAP_KEY=[^{]/.test(o.provenance.sourceRef ?? ''))) return 'sourceRef leaks the key';
      return undefined;
    },
    verifyHealth: (h) => (h.objectCount === 19 && h.credentialState === 'present' ? undefined : `objectCount ${h.objectCount} credentialState ${h.credentialState}`),
  },
});
