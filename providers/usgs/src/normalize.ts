...
const MAG_TYPES = new Set(['md', 'ml', 'ms', 'mw', 'me', 'mi', 'mb', 'mlg', 'mwr', 'mww', 'mwc', 'mwb', 'mb_lg', 'mh', 'mun', 'mint']);
...
export function normalizeUsgsFeed(payload: unknown, opts: NormalizeOptions): NormalizeResult {
  ...
  features.forEach((raw, index) => {
    ...
    if (typeof draft === 'string') { if (draft !== 'below-min-magnitude') rejected.push({ index, reason: draft }); return; }
    if (seen.has(draft.externalId)) { rejected.push({ index, reason: `duplicate id ${draft.externalId}` }); return; }
...
export function featureToDraft(raw: unknown, minMag: number, opts: NormalizeOptions): ObservationDraft | string {
  ...
  const coords = f.geometry?.type === 'Point' && Array.isArray(f.geometry.coordinates) ? f.geometry.coordinates : undefined;
  ...
  if (depthKm !== undefined && depthKm !== null && (typeof depthKm !== 'number' || !Number.isFinite(depthKm))) return 'invalid depth';
  const mag = p['mag'];
  if (mag !== null && mag !== undefined && (typeof mag !== 'number' || !Number.isFinite(mag) || mag > 10 || mag < -5)) return 'invalid magnitude';
  ...
  const place = text(p['place']); if (place) payload['place'] = place;
  const title = text(p['title']); if (title) payload['title'] = title;
  const status = text(p['status'], 16); if (status) payload['status'] = status;
  const type = text(p['type'], 32); if (type) payload['eventType'] = type;
  const alert = text(p['alert'], 8); if (alert) payload['alert'] = alert;
  const tsunami = num(p['tsunami']); if (tsunami !== undefined) payload['tsunami'] = tsunami === 1;
  const sig = num(p['sig']); if (sig !== undefined) payload['significance'] = sig;
  const felt = num(p['felt']); if (felt !== undefined) payload['felt'] = felt;
  const cdi = num(p['cdi']); if (cdi !== undefined) payload['cdi'] = cdi;
  const mmi = num(p['mmi']); if (mmi !== undefined) payload['mmi'] = mmi;
  const nst = num(p['nst']); if (nst !== undefined) payload['stations'] = nst;
  const gap = num(p['gap']); if (gap !== undefined) payload['gap'] = gap;
  const rms = num(p['rms']); if (rms !== undefined) payload['rms'] = rms;
  const dmin = num(p['dmin']); if (dmin !== undefined) payload['dmin'] = dmin;
  const net = text(p['net'], 8); if (net) payload['network'] = net;
  const updated = epochToIso(p['updated'], 'ms'); if (updated) payload['updatedAt'] = updated;
  const url = text(p['url'], 500); if (url && /^https:\/\/earthquake\.usgs\.gov\//.test(url)) payload['detailUrl'] = url;
  const ids = text(p['ids'], 500); if (ids) payload['aliases'] = ids.split(',').filter(Boolean);
  ...
  const draft: ObservationDraft = {
    ...
    observedAt: time,
    position: { latitude: lat as number, longitude: lon as number, ...(typeof depthKm === 'number' ? { altitudeM: -depthKm * 1000, altitudeDatum: 'msl' as const } : {}) },
    payload,
