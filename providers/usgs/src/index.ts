...
export class UsgsEarthquakeProvider extends PollingProvider {
  ...
  protected async fetchOnce(request: ProviderQuery): Promise<{ observations: Observation[]; cacheAgeMs?: number }> {
    ...
    try {
      ...
      res.invalidate();
      throw new ProviderError('MALFORMED', 'USGS feed is not valid JSON', { retryable: false });
    }
    ...
    if (result.rejected.some((r) => r.index === -1)) {
      res.invalidate();
      throw new ProviderError('MALFORMED', 'USGS feed is not a FeatureCollection', { retryable: false });
    }
    // Atomic admission: rows filtered by minMagnitude are not rejections, so only count hard rejections.
    if (result.total > 0 && result.observations.length === 0 && result.rejected.length === result.total) {
      res.invalidate();
      assertAtomicAdmission(result.total, 0, 'USGS feed');
    }
    if (result.rejected.length)
      this.context.logger.warn('rejected USGS rows', {
  ...
  async historical(request: HistoricalProviderQuery): Promise<Observation[]> {
    const params = new URLSearchParams({
      ...
      orderby: 'time',
      limit: String(Math.min(request.limit ?? 2000, 20_000)),
    });
    if (request.bounds) {
      params.set('minlatitude', String(request.bounds.south));
      params.set('maxlatitude', String(request.bounds.north));
      params.set('minlongitude', String(request.bounds.west));
      params.set('maxlongitude', String(request.bounds.east));
    }
    ...
    try {
      ...
      throw new ProviderError('MALFORMED', 'USGS FDSN response is not valid JSON', { retryable: false });
    }
    const result = normalizeUsgsFeed(payload, {
