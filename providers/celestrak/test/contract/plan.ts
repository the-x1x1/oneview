...
export const plan = definePlan({
  ...
  expectations: {
    ...
    verify: (obs) => {
      ...
      if (
        iss.payload['name'] !== 'ISS (ZARYA)' ||
        iss.payload['noradId'] !== 25544 ||
        iss.payload['intlDesignator'] !== '1998-067A'
      )
        return 'ISS payload wrong';
      if (
        iss.position?.altitudeDatum !== 'orbit' ||
        !(iss.position.altitudeM > 350_000 && iss.position.altitudeM < 500_000)
      )
        return `ISS altitude implausible: ${String(iss.position?.altitudeM)}`;
      if (Math.abs(iss.position.latitude) > 51.7) return 'ISS latitude exceeds inclination';
      if (iss.observedAt !== '2026-09-21T03:12:34.123Z' || iss.effectiveFrom !== iss.observedAt)
        return 'observedAt/effectiveFrom must equal the element epoch';
      if (iss.effectiveUntil !== '2026-09-28T03:12:34.123Z') return 'effectiveUntil must be epoch + 7 d';
      if (
        typeof iss.payload['periodMinutes'] !== 'number' ||
        Math.abs((iss.payload['periodMinutes'] as number) - 92.9) > 0.2
      )
        return 'periodMinutes wrong';
      if (typeof iss.payload['speedMps'] !== 'number' || Math.abs((iss.payload['speedMps'] as number) - 7660) > 60)
        return 'speedMps implausible';
      if (typeof iss.payload['headingDegrees'] !== 'number') return 'headingDegrees missing';
      const geo = obs.find((o) => o.externalId === '41866');
      if (!geo || !(geo.position!.altitudeM > 35_000_000 && geo.position!.altitudeM < 36_500_000))
        return 'GOES 16 not at GEO altitude';
      ...
      if (!obs.every((o) => o.quality.flags?.includes('propagated') && o.quality.sourceQuality === 'authoritative'))
        return 'quality flags missing';
      if (!obs.every((o) => o.rawPayloadHash && /^[0-9a-f]{64}$/.test(o.rawPayloadHash)))
        return 'rawPayloadHash missing';
      return undefined;
    },
