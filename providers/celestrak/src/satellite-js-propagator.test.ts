...
const skipReason = satelliteJs
  ? false
  : 'satellite.js is not installed in this environment (no registry access); SGP4 path verified against the typed shim only';
...
test('SGP4 through satellite.js: ISS position is plausible and heading is derived', { skip: skipReason }, async () => {
  ...
  const ommOnly = { ...ISS };
  delete ommOnly.line1;
  delete ommOnly.line2;
...
test('a satrec with error ≠ 0 or a false position yields undefined and is cached per element set', async () => {
  ...
  const satrec = (fields: Record<string, unknown>) =>
    fields as unknown as ReturnType<SatelliteJsModule['twoline2satrec']>;
