...
export const SETTINGS = {
  cameras: [
    { cameraId: '3f2a9c1e7b04', name: 'Garage door', position: { latitude: 48.1374, longitude: 11.5755, altitudeM: 520 }, headingDegrees: 270, gateway: 'direct', kind: 'mjpeg' },
    { cameraId: 'a1b2c3d4e5f6', name: 'Roof (RTSP via go2rtc)', position: { latitude: 48.1380, longitude: 11.5760 }, headingDegrees: 45, gateway: 'go2rtc', kind: 'rtsp' },
...
export const plan = definePlan({
  ...
  expectations: {
    objectTypes: ['camera'],
    minObservations: 3,
    expectObjectIds: ['camera:cameras-local:3f2a9c1e7b04', 'camera:cameras-local:a1b2c3d4e5f6', 'camera:cameras-local:0f0e0d0c0b0a'],
    verify: (obs) => {
      ...
      if (garage.payload['headingDegrees'] !== 270 || garage.payload['gateway'] !== 'direct') return 'heading/gateway not carried';
      ...
      if (still?.position !== undefined || !still?.quality.flags?.includes('position-unknown')) return 'positionless camera must be flagged';
      if (!Array.isArray(still.payload['media']) || still.payload['media'].length !== 1) return 'still cameras get a snapshot ref only';
