...
test('provider: without bounds or homePosition the query is skipped, health explains why', async () => {
  ...
  const ctx = testing.createFixtureContext({ providerId: 'adsb-lol', clock: new testing.VirtualClock(NOW + 5000), responder: () => ({ status: 200, body: envelope([]) }) });
...
test('provider: viewport bounds drive the endpoint; settings change is picked up', async () => {
  const p = new AdsbLolProvider();
  const ctx = testing.createFixtureContext({ providerId: 'adsb-lol', clock: new testing.VirtualClock(NOW + 5000), responder: () => ({ status: 200, body: envelope([{ hex: 'a4b2c3', lat: 21.5, lon: -157.9, alt_baro: 3000, seen_pos: 1 }]) }) });
  await p.initialize(ctx);
  await p.start();
  const obs = await p.query({ signal: new AbortController().signal, background: true, bounds: { west: -158.5, south: 20.9, east: -157.3, north: 21.8 } });
...
test('provider: a feed where every row is unusable (other than missing positions) is MALFORMED; Mode S-only feeds are empty and LIVE', async () => {
  let body = envelope([{ hex: 'a6c6d6', type: 'mode_s', flight: 'HAL22' }, { hex: 'a6c6d7', type: 'mode_s' }]);
  const p = new AdsbLolProvider();
  const ctx = testing.createFixtureContext({ providerId: 'adsb-lol', clock: new testing.VirtualClock(NOW + 5000), settings: { homePosition: { latitude: 21.32, longitude: -157.92 } }, responder: () => ({ status: 200, body }) });
  await p.initialize(ctx);
  await p.start();
  ...
  await assert.rejects(p.query({ signal: new AbortController().signal, background: true }), (e: { code?: string }) => e.code === 'MALFORMED');
});
