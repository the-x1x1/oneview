...
export const plan = definePlan({
  ...
  expectations: {
    ...
    verify: (obs) => {
      ...
      if (
        hal.payload['callsign'] !== 'HAL457' ||
        hal.payload['registration'] !== 'N383HA' ||
        hal.payload['typeCode'] !== 'A332'
      )
        return 'HAL457 identity payload wrong';
      if (hal.position?.altitudeDatum !== 'barometric' || Math.abs((hal.position.altitudeM ?? 0) - 10668) > 1)
        return 'barometric altitude not converted to metres';
      if (
        hal.payload['speedMps'] !== 241.89 ||
        hal.payload['headingDegrees'] !== 92.4 ||
        hal.payload['verticalSpeedMps'] !== -0.33
      )
        return `HAL457 motion payload wrong (${String(hal.payload['speedMps'])}, ${String(hal.payload['headingDegrees'])}, ${String(hal.payload['verticalSpeedMps'])})`;
      if (hal.observedAt !== '2026-09-21T07:59:59.200Z')
        return `observedAt should be now − seen_pos, got ${hal.observedAt}`;
      ...
      if (
        !ground ||
        ground.payload['onGround'] !== true ||
        ground.position?.altitudeM !== 0 ||
        ground.position.altitudeDatum !== 'ground'
      )
        return 'on-ground row not mapped to ground datum';
      const mil = obs.find((o) => o.externalId === 'ae1234');
      if (!mil || mil.payload['military'] !== true) return 'military flag (dbFlags & 1) missing';
      if (obs.some((o) => o.externalId !== 'ae1234' && o.payload['military'] !== false))
        return 'civil rows must carry military=false';
      ...
      if (!tisb || tisb.payload['icao24'] !== undefined || !tisb.quality.flags?.includes('non-icao-address'))
        return 'TIS-B non-ICAO address must not carry icao24';
      if (obs.some((o) => o.externalId === 'a6c6d6')) return 'Mode S row without position leaked';
      const heli = obs.find((o) => o.externalId === 'a2b2c2');
      if (!heli?.quality.flags?.includes('emergency:general') || heli.payload['squawk'] !== '7700')
        return 'emergency flag/squawk missing';
