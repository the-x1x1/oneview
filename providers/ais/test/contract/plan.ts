...
export const plan = definePlan({
  ...
  expectations: {
    ...
    expectObjectIds: [
      ...
      'vessel:mmsi:338987654',
      'vessel:mmsi:002320001',
      'vessel:mmsi:431009876',
    ],
    verify: (obs) => {
      ...
      if (trader.observedAt !== '2026-09-21T08:00:03.123Z')
        return `time_utc with nanoseconds not parsed: ${trader.observedAt}`;
      if (trader.position?.altitudeDatum !== 'sea-surface' || trader.position.latitude !== 21.3044)
        return 'position wrong';
      if (
        trader.payload['speedMps'] !== 5.86 ||
        trader.payload['headingDegrees'] !== 252 ||
        trader.payload['courseDegrees'] !== 254.3
      )
        return `motion payload wrong (${String(trader.payload['speedMps'])}/${String(trader.payload['headingDegrees'])}/${String(trader.payload['courseDegrees'])})`;
      if (
        trader.payload['navStatus'] !== 0 ||
        trader.payload['name'] !== 'KALIHI TRADER' ||
        trader.payload['mmsi'] !== '366123456'
      )
        return 'identity payload wrong';
      if (trader.quality.sourceQuality !== 'crowdsourced' || trader.rawPayloadHash !== undefined)
        return 'quality/raw retention wrong';
      const fallback = obs.find((o) => o.externalId === '338987654');
      if (
        !fallback ||
        fallback.payload['headingDegrees'] !== 96.5 ||
        !fallback.quality.flags?.includes('heading-from-cog')
      )
        return 'TrueHeading 511 must fall back to COG';
      ...
      if (anchored.payload['speedMps'] !== undefined || anchored.payload['courseDegrees'] !== undefined)
        return 'SOG 102.3 / COG 360 sentinels leaked';
      if (
        anchored.payload['headingDegrees'] !== 14 ||
        anchored.payload['navStatusText'] !== 'at anchor' ||
        !anchored.quality.flags?.includes('turning-right')
      )
        return 'anchored vessel payload wrong';
      ...
      if (
        stat.payload['imo'] !== '9312345' ||
        stat.payload['callSign'] !== 'WDK4421' ||
        stat.payload['shipTypeText'] !== 'cargo' ||
        stat.payload['lengthM'] !== 162 ||
        stat.payload['beamM'] !== 25 ||
        stat.payload['draughtM'] !== 8.4 ||
        stat.payload['destination'] !== 'HONOLULU'
      )
        return 'static payload wrong';
    ...
    verifyHealth: (h) =>
      h.objectCount === 4 && h.credentialState === 'present'
        ? undefined
