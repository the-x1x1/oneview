...
export async function runProviderChecklist(
  ...
  const run = async (name: (typeof CHECKLIST)[number], fn: () => Promise<string> | string): Promise<void> => {
    ...
    try {
      ...
      checks.push({
        check: name,
        status: detail.startsWith('SKIP:') ? 'SKIP' : 'PASS',
        detail,
  ...
  const need = <T>(v: T | undefined, what: string): T => {
    if (v === undefined) throw new Error(`${what} unavailable (earlier check failed)`);
    return v;
  };
  ...
  await run('Manifest', () => {
    ...
    const m = r.value;
    manifest = m;
    if (m.id !== plan.providerDir && !m.id.startsWith(plan.providerDir) && !plan.aliases?.includes(m.id))
      fail(`manifest.id ${m.id} does not match providers/${plan.providerDir} (declare it in plan.aliases)`);
    if (plan.profile === 'local' && (m.transport === 'http' || m.transport === 'websocket'))
      fail(`profile 'local' but transport is ${m.transport}`);
    ...
    if (rec && rec.commercialReview !== m.commercialReview)
      fail(`commercialReview mismatch: manifest=${m.commercialReview} registry=${rec.commercialReview}`);
  ...
  await run('Data Policy', () => {
    ...
    if (rec) {
      ...
      for (const k of [
        ...
        if (rec.dataPolicy[k] !== mp[k])
          diffs.push(`${k}: manifest=${String(mp[k])} registry=${String(rec.dataPolicy[k])}`);
      }
  ...
  await run('Initialization', async () => {
    ...
    ctx = testing.createFixtureContext({
      ...
      responder: (req) => {
        switch (scenario) {
          case 'empty':
            return plan.fixtures.empty ? plan.fixtures.empty(req) : { status: 200, body: '' };
          case 'stale':
            return plan.fixtures.stale ? plan.fixtures.stale(req) : plan.fixtures.normal(req);
          case 'timeout':
            return { error: 'timeout' };
          case 'malformed':
            return plan.fixtures.malformed![malformedIndex]!(req);
          case 'rate':
            return { status: 429, headers: { 'retry-after': '45' } };
          case 'auth':
            return { status: 401 };
          case 'server-error':
            return { status: 503 };
          default:
            return plan.fixtures.normal(req);
        }
      },
      local: scenarioLocalAccess(
        plan,
        m,
        () => scenario,
        () => malformedIndex,
  ...
  await run('Fixture', async () => {
    ...
    if (normal.length < plan.expectations.minObservations)
      fail(`expected ≥${plan.expectations.minObservations} observations, got ${normal.length}`);
  ...
  await run('Normalization', () => {
    ...
    normal.forEach((o, i) => {
      ...
      if (!m.objectTypes.includes(o.objectType))
        fail(`observation[${i}].objectType ${o.objectType} not declared in manifest`);
      if (ids.has(o.id)) fail(`duplicate observation id ${o.id}`);
      ids.add(o.id);
      types.add(o.objectType);
      if (Date.parse(o.observedAt) > Date.parse(o.receivedAt) + 5 * 60_000)
        fail(`observation[${i}] observedAt is in the future relative to receivedAt`);
      if (o.rawPayloadHash && !m.dataPolicy.rawPayloadRetentionAllowed)
        fail('rawPayloadHash present but raw retention not allowed');
    });
  ...
  await run('Cancellation', async () => {
    ...
    try {
      ...
      if (!(err instanceof ProviderError) || err.code !== 'CANCELLED')
        fail(
          `expected CANCELLED ProviderError, got ${err instanceof Error ? `${err.name}: ${err.message}` : String(err)}`,
        );
    }
    const h = await p.health();
  ...
  await run('Timeout', async () => {
    ...
    try {
      ...
      if (!(err instanceof ProviderError) || err.code !== 'TIMEOUT')
        fail(
          `expected TIMEOUT ProviderError, got ${err instanceof Error ? `${err.name}: ${err.message}` : String(err)}`,
        );
    } finally {
      scenario = 'normal';
    }
    const h = await p.health();
    if (!['DEGRADED', 'ERROR', 'STALE'].includes(h.status))
      fail(`expected DEGRADED/ERROR after timeout, got ${h.status}`);
  ...
  await run('Stale Detection', async () => {
    ...
    if (![...classes].every((c) => c === 'STALE' || c === 'RECENT'))
      fail(`stale observations classified as ${[...classes].join(',')}`);
  ...
  await run('Attribution', () => {
    ...
    for (const o of normal)
      if (o.provenance.attribution !== m.attribution.text)
        fail(`observation ${o.id} attribution "${o.provenance.attribution}" ≠ manifest`);
    if (m.dataPolicy.attributionRequired && !m.dataPolicy.attributionText)
      fail('attributionRequired but attributionText missing');
  ...
  await run('World Mapping', () => {
    ...
    const meta = {
      snapshot: true,
      providerId: m.id,
      ...(m.refreshPolicy.freshness ? { freshness: m.refreshPolicy.freshness } : {}),
    };
    ...
    for (const id of plan.expectations.expectObjectIds ?? [])
      if (!stateA.has(id)) fail(`expected object ${id} missing`);
    const authoritative = idsA.filter(isAuthoritativeId).length;
    for (const obj of stateA.all()) {
      if (!obj.freshness || obj.confidence < 0 || obj.confidence > 1)
        fail(`object ${obj.id} freshness/confidence invalid`);
  ...
  await run('Malformed Feed', async () => {
    ...
    for (malformedIndex = 0; malformedIndex < plan.fixtures.malformed.length; malformedIndex++) {
      ...
      try {
        ...
        for (const o of obs) {
          const r = observationSchema.parse(o);
          if (!r.ok) fail(`malformed[${malformedIndex}] leaked invalid observation`);
        }
        outcomes.push(`#${malformedIndex}: admitted ${obs.length} valid rows`);
      } catch (err) {
        if (!(err instanceof ProviderError))
          return fail(
            `malformed[${malformedIndex}] threw a non-ProviderError: ${err instanceof Error ? err.message : String(err)}`,
          );
  ...
  await run('Auth Failure', async () => {
    ...
    try {
      ...
      if (!(err instanceof ProviderError) || err.code !== 'AUTH')
        fail(`expected AUTH, got ${err instanceof Error ? err.message : String(err)}`);
    } finally {
      scenario = 'normal';
    }
    const h = await p.health();
  ...
  await run('Offline', async () => {
    ...
    const m = need(manifest, 'manifest');
    if (!p.query) return 'SKIP: subscription provider';
    if (!local && m.transport === 'local-process')
      return 'SKIP: local-process transport keeps loopback access while offline (runtime HttpClient policy); fixture http cannot model it';
    if (local) {
    ...
    try {
      ...
      if (!(err instanceof ProviderError) || err.code !== 'OFFLINE')
        fail(`expected OFFLINE, got ${err instanceof Error ? err.message : String(err)}`);
    } finally {
      c.setOnline(true);
    }
    const h = await p.health();
 ...
 */
function scenarioLocalAccess(
  plan: ProviderTestPlan,
  manifest: ProviderManifest,
  scenario: () => Scenario,
  malformedIndex: () => number,
): testing.FixtureLocalAccess {
...
class ScenarioLocalAccess extends testing.FixtureLocalAccess {
  constructor(
    ...
    private readonly plan: ProviderTestPlan,
    private readonly scenario: () => Scenario,
    private readonly malformedIndex: () => number,
  ) {
  ...
  override async readGrantedFile(file: string, opts?: { maxBytes?: number }): Promise<Uint8Array> {
    ...
    const responder =
      s === 'empty'
        ...
        : s === 'stale'
          ? f.stale
          : s === 'malformed'
            ? f.malformed?.[this.malformedIndex()]
            : undefined;
    ...
    if (opts?.maxBytes !== undefined && bytes.byteLength > opts.maxBytes)
      throw new ProviderError('TOO_LARGE', `file exceeded ${opts.maxBytes} bytes`, { retryable: false });
    return bytes;
  }
}
...
export function formatReport(report: ValidationReport): string {
  ...
  return [
    `Provider ${report.providerId} v${report.version}`,
    ...lines,
    `${report.summary.pass} pass, ${report.summary.fail} fail, ${report.summary.skip} skip → ${report.passed ? 'PASS' : 'FAIL'}`,
  ].join('\n');
}
