...
const release = (
  JSON.parse(readFileSync(path.join(root, 'apps', 'desktop', 'package.json'), 'utf8')) as { version: string }
).version;
...
if (report.artifactHashes.length)
  writeFileSync(path.join(outDir, 'SHA256SUMS.txt'), sha256SumsText(report.artifactHashes));
