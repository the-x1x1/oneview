...
test('boundary: identity resolution merges only on authoritative object identifiers (ADR-011)', () => {
  ...
  for (const [type, rule] of Object.entries(authoritativeRules)) {
    ...
    if (result)
      assert.match(
        result.rule,
        /^(?:aircraft\.icao24|vessel\.mmsi|satellite\.norad|earthquake\.usgs|airport\.icao)$/,
        `${type} joins on an authoritative id`,
      );
  }
});

test('boundary: no channel searches for a person, and no recognition runs on a frame', () => {
  const personShaped = REQUEST_CHANNELS.filter((c) =>
    /person|people|face|facial|biometr|plate|alpr|owner|subject/i.test(c),
  );
  ...
  const analysisApis =
    /\b(?:detectFaces|faceDetect|recognize[A-Z]|FaceDetector|readPlates?|plateRecognit|tesseract|opencv|\bocr\(|classifyFrame|detectObjects)\b/;
  const offenders = SOURCES.filter((f) => analysisApis.test(readFileSync(f, 'utf8')));
  assert.deepEqual(
    offenders.map((f) => path.relative(root, f)),
    [],
    'no frame-analysis API is called anywhere',
  );
});
...
test('boundary: no module that handles frame bytes writes them to disk', () => {
  ...
  for (const file of present) {
    ...
    assert.ok(
      !/\bwriteFile(?:Sync)?\s*\(|\bcreateWriteStream\s*\(|\bappendFile/.test(text),
      `${file} must not persist frames`,
    );
  }
...
test('every pnpm script entry point is tracked by git', () => {
  const scripts = (
    JSON.parse(readFileSync(path.join(root, 'package.json'), 'utf8')) as { scripts: Record<string, string> }
  ).scripts;
