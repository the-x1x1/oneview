 ...
 */
export type PathName = 'home' | 'appData' | 'userData' | 'sessionData' | 'temp' | 'exe' | 'module' | 'desktop' | 'documents' | 'downloads' | 'music' | 'pictures' | 'videos' | 'recent' | 'logs' | 'crashDumps';

export interface Event { preventDefault(): void; readonly defaultPrevented: boolean }

export interface RenderProcessGoneDetails { reason: 'clean-exit' | 'abnormal-exit' | 'killed' | 'crashed' | 'oom' | 'launch-failed' | 'integrity-failure'; exitCode: number }
export interface Details { readonly reason: string }
...
export interface App {
  ...
  on(event: 'child-process-gone', listener: (event: Event, details: Details & { type: string; name?: string; exitCode: number }) => void): this;
...
export interface WindowOpenHandlerDetails { url: string; frameName: string; disposition: string; referrer?: { url: string; policy: string } }
...
export interface WebContents {
  ...
  on(event: 'console-message', listener: (event: Event, level: number, message: string, line: number, sourceId: string) => void): this;
...
export interface HeadersReceivedResponse { cancel?: boolean; responseHeaders?: Record<string, string | string[]>; statusLine?: string }
...
export interface WebRequest {
  ...
  onHeadersReceived(filter: { urls: string[] }, listener: ((details: OnHeadersReceivedListenerDetails, callback: (response: HeadersReceivedResponse) => void) => void) | null): void;
}

export type PermissionType = 'clipboard-read' | 'clipboard-sanitized-write' | 'display-capture' | 'fullscreen' | 'geolocation' | 'idle-detection' | 'media' | 'mediaKeySystem' | 'midi' | 'midiSysex' | 'notifications' | 'pointerLock' | 'keyboardLock' | 'openExternal' | 'speaker-selection' | 'storage-access' | 'top-level-storage-access' | 'window-management' | 'unknown' | 'fileSystem';

export interface PermissionRequestDetails { requestingUrl?: string; isMainFrame?: boolean; externalURL?: string; mediaTypes?: Array<'video' | 'audio'> }
export interface PermissionCheckDetails { embeddingOrigin?: string; requestingUrl?: string; isMainFrame?: boolean; mediaType?: 'video' | 'audio' | 'unknown' }

export interface Session {
  readonly webRequest: WebRequest;
  setPermissionRequestHandler(handler: ((webContents: WebContents, permission: PermissionType, callback: (granted: boolean) => void, details: PermissionRequestDetails) => void) | null): void;
  setPermissionCheckHandler(handler: ((webContents: WebContents | null, permission: string, requestingOrigin: string, details: PermissionCheckDetails) => boolean) | null): void;
  setDevicePermissionHandler(handler: ((details: { deviceType: 'hid' | 'serial' | 'usb'; origin: string }) => boolean) | null): void;
...
export interface IpcMain {
  ...
  on(channel: string, listener: (event: { sender: WebContents; senderFrame: WebFrameMain | null; reply(channel: string, ...args: unknown[]): void }, ...args: unknown[]) => void): this;
  removeAllListeners(channel?: string): this;
}
export declare const ipcMain: IpcMain;

export interface IpcRendererEvent { readonly sender: IpcRenderer; readonly ports: unknown[] }
...
export interface Shell {
  openExternal(url: string, options?: { activate?: boolean; workingDirectory?: string; logUsage?: boolean }): Promise<void>;
...
export interface OpenDialogOptions { title?: string; defaultPath?: string; buttonLabel?: string; filters?: FileFilter[]; properties?: Array<'openFile' | 'openDirectory' | 'multiSelections' | 'showHiddenFiles' | 'createDirectory' | 'promptToCreate' | 'dontAddToRecent'>; message?: string }
export interface OpenDialogReturnValue { canceled: boolean; filePaths: string[] }
export interface SaveDialogOptions { title?: string; defaultPath?: string; buttonLabel?: string; filters?: FileFilter[]; message?: string; nameFieldLabel?: string; showsTagField?: boolean; properties?: Array<'showHiddenFiles' | 'createDirectory' | 'treatPackageAsDirectory' | 'showOverwriteConfirmation' | 'dontAddToRecent'> }
export interface SaveDialogReturnValue { canceled: boolean; filePath: string }
export interface MessageBoxOptions { type?: 'none' | 'info' | 'error' | 'question' | 'warning'; buttons?: string[]; defaultId?: number; title?: string; message: string; detail?: string; noLink?: boolean }
...
export interface NotificationConstructorOptions { title: string; body?: string; silent?: boolean; icon?: string; urgency?: 'normal' | 'critical' | 'low'; timeoutType?: 'default' | 'never'; toastXml?: string }
...
export declare const Menu: { setApplicationMenu(menu: Menu | null): void; buildFromTemplate(template: Array<Record<string, unknown>>): Menu };

export interface NativeTheme { shouldUseDarkColors: boolean; themeSource: 'system' | 'light' | 'dark' }
export declare const nativeTheme: NativeTheme;

export interface CrashReporterStartOptions { submitURL?: string; uploadToServer?: boolean; compress?: boolean; ignoreSystemCrashHandler?: boolean; rateLimit?: boolean; extra?: Record<string, string> }
