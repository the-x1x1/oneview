...
export async function runBackendConformance(
  ...
  assert.deepEqual(
    avail.map((a) => a.objectType),
    ['aircraft', 'earthquake', 'vessel'],
  );
  ...
  assert.deepEqual(
    exact.map((r) => r.objectId),
    ['aircraft:icao24:abc123'],
    'cursor equal to observedAt is included',
  );
  const bounded = await backend.objectsAt(iso(D2, 30), {
    objectTypes: ['aircraft'],
    lookbackSeconds: 600,
    bounds: { west: 150, south: -40, east: 160, north: -30 },
  });
  assert.deepEqual(
    bounded.map((r) => r.objectId),
    ['aircraft:icao24:def456'],
  );
  const all = await backend.objectsAt(iso(D3, 5 * 3600), { lookbackSeconds: 3 * 86_400 });
  assert.deepEqual(all.map((r) => r.objectId).sort(), [
    'aircraft:icao24:abc123',
    'aircraft:icao24:def456',
    'earthquake:usgs:q-2026-09-19',
    'earthquake:usgs:q-2026-09-20',
    'earthquake:usgs:q-2026-09-21',
  ]);
  ...
  const counts = await backend.counts({
    range: { start: D1, end: iso(D3, 86_400) },
    objectTypes: ['aircraft', 'earthquake'],
  });
  assert.deepEqual(counts, [
    { objectType: 'aircraft', objects: 2, rows: 15 },
    { objectType: 'earthquake', objects: 3, rows: 3 },
  ]);
  const regionCounts = await backend.counts({
    range: { start: D1, end: iso(D3, 86_400) },
    region: { kind: 'circle', center: { latitude: 35, longitude: -118 }, radiusM: 10_000 },
  });
  assert.deepEqual(regionCounts, [{ objectType: 'earthquake', objects: 3, rows: 3 }]);
  const inRange = await backend.observationsInRange({
    range: { start: iso(D2, 0), end: iso(D2, 10) },
    objectTypes: ['aircraft'],
    providerIds: ['opensky'],
  });
