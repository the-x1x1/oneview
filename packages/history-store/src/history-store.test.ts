...
const D1 = '2026-09-19T08:00:00.000Z',
  D2 = '2026-09-20T08:00:00.000Z',
  D3 = '2026-09-21T08:00:00.000Z';
...
async function makeStore(overrides: Partial<ConstructorParameters<typeof HistoryStore>[0]> = {}) {
  ...
  const store = new HistoryStore({
    ...
    policies: policies({
      [AIRCRAFT_PROVIDER]: OPEN_POLICY,
      [QUAKE_PROVIDER]: OPEN_POLICY,
      'no-raw': NO_RAW_POLICY,
      'no-retain': NO_RETAIN_POLICY,
      capped: CAPPED_POLICY,
    }),
    ...overrides,
  });
  await store.open();
  ...
  for (const day of [D1, D2, D3]) {
    out.push(
      batch(AIRCRAFT_PROVIDER, [
        aircraftObs('abc123', iso(day, 0), 50.0, 8.0),
        aircraftObs('abc123', iso(day, 10), 50.1, 8.1),
        aircraftObs('abc123', iso(day, 20), 50.2, 8.2),
        aircraftObs('def456', iso(day, 0), -33.9, 151.2),
        aircraftObs('def456', iso(day, 10), -33.8, 151.3),
      ]),
    );
...
test('partition layout: keys, paths and round-trip parsing', () => {
  ...
  assert.deepEqual(partitionKeyFor('vessel', 'ais', '2026-12-31T00:00:00.000Z', 60), {
    objectType: 'vessel',
    providerId: 'ais',
    day: '2026-12-31',
    slot: '0000',
...
test('ndjson backend: conformance suite (append, list, read, availability, objectsAt, track, counts, rewrite, delete)', async () => {
  ...
  await runBackendConformance(backend, {
    afterWrite: async () => {
      ...
      const index = JSON.parse(await fs.readFile(path.join(dataDir, 'history', 'index.json'), 'utf8')) as {
        version: number;
        partitions: unknown[];
      };
...
test('store: writes for two providers/types across three days; availability exact; objectsAt/lookback; track ordering; snapshot identity', async () => {
  ...
  assert.equal(
    receipts.reduce((n, r) => n + r.queued, 0),
    18,
  );
  await store.flush();
  ...
  assert.deepEqual(avail, [
    {
      objectType: 'aircraft',
      ranges: [
        { start: iso(D1, 0), end: iso(D1, 20) },
        { start: iso(D2, 0), end: iso(D2, 20) },
        { start: iso(D3, 0), end: iso(D3, 20) },
      ],
    },
    {
      objectType: 'earthquake',
      ranges: [
        { start: iso(D1, 4 * 3600), end: iso(D1, 4 * 3600) },
        { start: iso(D2, 4 * 3600), end: iso(D2, 4 * 3600) },
        { start: iso(D3, 4 * 3600), end: iso(D3, 4 * 3600) },
      ],
    },
  ]);
  ...
  assert.deepEqual(
    ...
    [
      ['aircraft:icao24:abc123', iso(D2, 10)],
      ['aircraft:icao24:def456', iso(D2, 10)],
    ],
  );
  assert.equal((await store.objectsAt(iso(D2, 15), { objectTypes: ['aircraft'], lookbackSeconds: 4 })).length, 0);
  assert.equal(
    (await store.objectsAt(iso(D2, 15), { objectTypes: ['aircraft'], lookbackSeconds: 5 })).length,
    2,
  ...
  const result = await store.queryObjects({
    objectTypes: ['aircraft', 'earthquake'],
    time: { start: D2, end: iso(D2, 5 * 3600) },
    region: { kind: 'bounds', bounds: { west: -130, south: 30, east: -110, north: 40 } },
  });
  assert.equal(result.basis, 'historical');
  assert.deepEqual(
    result.items.map((o) => o.id),
    ['earthquake:usgs:q-2026-09-20'],
  );
  ...
  const rows = await store.observationsInRange({
    objectTypes: ['earthquake'],
    time: { start: D1, end: iso(D3, 86_400) },
  });
...
test('store: policy enforcement — no normalized retention writes nothing, raw hash stripped, maxRetentionSeconds caps', async () => {
  ...
  const noPolicy = store.writeBatch(
    batch('unknown-provider', [aircraftObs('aaa111', iso(D3, 0), 1, 1, {}, 'unknown-provider')]),
  );
  ...
  const camera = store.writeBatch(
    batch(AIRCRAFT_PROVIDER, [{ ...aircraftObs('eee555', iso(D3, 0), 5, 5), objectType: 'camera' }]),
  );
  ...
  assert.equal(
    parts.some((p) => p.providerId === 'no-retain'),
    false,
  );
  ...
  assert.deepEqual(
    sweep.deleted.map((p) => p.providerId),
    ['capped'],
    'capped partition older than 1h deleted, others kept',
  );
...
test('retention: tier selection and composable downsampling keep first/last and exact every-10th', () => {
  ...
  assert.deepEqual(
    t2.rows.map((r) => r.seq),
    [0, 10, 20, 30, 40, 50, 59],
    'tier 2 is exactly every 10th of the original plus the last point',
  );
  const direct = downsampleRows(
    rows.map((r) => ({ ...r })),
    TRACK_DOWNSAMPLE_TIERS,
    2,
  );
  assert.deepEqual(
    direct.rows.map((r) => r.seq),
    t2.rows.map((r) => r.seq),
    'tier 1 → tier 2 equals direct tier 2',
  );
...
test('store: retention sweep deletes expired partitions and downsamples aircraft in tiers while the index keeps original counts', async () => {
  ...
  assert.ok(
    fresh.rewritten.every((r) => r.partition.id !== partitionId(key)),
    'younger than 5 min: untouched',
  );
  ...
  assert.equal(
    (await store.sweepRetention(Date.parse(iso(D3, 59 + 6 * 60)))).rewritten.length,
    0,
    'same tier: no rewrite',
  );
  ...
  assert.deepEqual(
    track.map((t) => t.observedAt),
    [0, 10, 20, 30, 40, 50, 59].map((s) => iso(D3, s)),
  );
  assert.ok(
    (await backend.readPartition(key)).rows.every((r) => r.rawPayloadHash !== undefined),
    'raw hash kept inside 24h',
  );
  ...
  assert.ok(
    (await backend.readPartition(key)).rows.every((r) => r.rawPayloadHash === undefined),
    'raw hash stripped after 24h',
  );
  assert.deepEqual(
    s3.deleted.map((p) => p.id),
    [partitionId(oldKey)],
    'aircraft partition older than 30d deleted; indefinite earthquake kept',
  );
...
test('ndjson backend: malformed lines are counted and skipped, never fatal', async () => {
  ...
  store.writeBatch(
    batch(AIRCRAFT_PROVIDER, [aircraftObs('abc123', iso(D3, 0), 50, 8), aircraftObs('abc123', iso(D3, 10), 50.1, 8.1)]),
  );
  await store.flush();
...
test('ndjson backend: index rebuilt when index.json is missing and reconciled when files are ahead of it', async () => {
  ...
  assert.deepEqual(await reopened.availability(['aircraft']), [
    { objectType: 'aircraft', ranges: [{ start: iso(D3, 0), end: iso(D3, 10) }] },
  ]);
  ...
  await fs.appendFile(
    path.join(dataDir, 'history', 'aircraft', '2026', '09', '21', 'opensky-0800.ndjson'),
    JSON.stringify(row) + '\n',
  );
  const stale = JSON.parse(committed) as { partitions: Array<Record<string, unknown>> };
  stale.partitions.push({
    ...stale.partitions[0]!,
    id: 'aircraft/2026-09-22/opensky-0800',
    day: '2026-09-22',
    minObservedAt: iso(D3, 86_400),
    maxObservedAt: iso(D3, 86_400),
  });
...
test('store: sweep never destroys partitions of a provider whose policy is unknown right now', async () => {
  ...
  assert.deepEqual(report.skipped, [
    { partition: 'aircraft/2026-09-20/capped-0800', reason: 'no data policy for provider' },
  ]);
...
test('store: bounded queue drops with a count and a log line when the backend is slow; writes never block', async () => {
  ...
  const slow: HistoryBackend = wrapBackend(inner, {
    append: async (key: PartitionKey, rows: HistoryRow[]) => {
      appends++;
      await gate;
  ...
  const mk = (n: number, offset: number) =>
    Array.from({ length: n }, (_, i) =>
      aircraftObs(`a${String(offset + i).padStart(5, '0')}`, iso(D3, offset + i), 10, 10),
    );
  ...
  assert.ok(
    sink.records.some((r) => r.level === 'warn' && r.message.includes('queue full')),
    'overflow is logged',
  );
  release!();
  await store.flush();
...
test('timeline controller: replay advances at 60x, auto-live at now, jump-to-live, availability from the store only', async () => {
  ...
  const timeline = new TimelineController({
    ...
    clock,
    rangeSeconds: 6 * 3600,
    onChange: (s) => states.push(`${s.mode}@${s.cursor}`),
  });
  ...
  store.writeBatch(
    batch(AIRCRAFT_PROVIDER, [
      aircraftObs('abc123', iso(D3, 0), 50, 8),
      aircraftObs('abc123', iso(D3, 600), 50.1, 8.1),
    ]),
  );
  await store.flush();
