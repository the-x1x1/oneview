import { boundsContain, parseObjectId, regionContains, type GeoBounds, type GeoRegion, type IsoTimestamp, type TimeRange } from '@worldview/world-model';
...
export function reduceLatestPerObject(rows: Iterable<HistoryRow>, into: Map<string, HistoryRow> = new Map()): Map<string, HistoryRow> {
...
export function sortByObservedAt(rows: HistoryRow[]): HistoryRow[] {
  return rows.sort((a, b) => (a.observedAt < b.observedAt ? -1 : a.observedAt > b.observedAt ? 1 : a.observationId < b.observationId ? -1 : a.observationId > b.observationId ? 1 : 0));
}

export async function scanObjectsAt(scanner: PartitionScanner, cursor: IsoTimestamp, opts: ObjectsAtOptions): Promise<HistoryRow[]> {
  ...
  for (const meta of await scanner.listPartitions(filter)) {
    ...
    reduceLatestPerObject(rows.filter((r) => inRange(r, range) && rowInBounds(r, opts.bounds)), latest);
  }
...
export function availabilityFromMetas(metas: PartitionMeta[], objectTypes: string[] | undefined, gapToleranceMs: number): TypeAvailability[] {
  ...
  for (const m of metas) {
    ...
    if (!list) { list = []; byType.set(m.objectType, list); }
    list.push(m);
  }
...
export function rangeFilter(query: RangeQuery): PartitionFilter {
  return { overlapping: query.range, ...(query.objectTypes ? { objectTypes: query.objectTypes } : {}), ...(query.providerIds ? { providerIds: query.providerIds } : {}) };
}
...
export function reduceCounts(rows: Iterable<HistoryRow>, into: Map<string, { objects: Set<string>; rows: number }> = new Map()): Map<string, { objects: Set<string>; rows: number }> {
  for (const r of rows) {
    let c = into.get(r.objectType);
    if (!c) { c = { objects: new Set(), rows: 0 }; into.set(r.objectType, c); }
...
export function countsToList(map: Map<string, { objects: Set<string>; rows: number }>, objectTypes: string[] | undefined): TypeCounts[] {
...
export async function scanCounts(scanner: PartitionScanner, query: RangeQuery): Promise<TypeCounts[]> {
  ...
  for (const meta of await scanner.listPartitions(rangeFilter(query))) {
    const { rows } = await scanner.readPartition(meta);
    reduceCounts(rows.filter((r) => rowMatchesRange(r, query)), acc);
  }
