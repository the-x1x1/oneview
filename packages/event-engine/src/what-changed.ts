...
export async function whatChanged(
  input: { region: GeoRegion; time: TimeRange },
  ...
  for (const e of sources.events.all()) {
    if (e.type === EventTypes.SourceStatusChange) {
      if (inRange(e.startAt)) statusEvents.push(e);
      continue;
    }
  ...
  const before: WorldObject[] | undefined =
    sources.history && bounds
      ? (await sources.history.objectsAt(time.start, { bounds })).filter(
          (o) => o.position && regionContains(region, o.position),
        )
      : undefined;
  ...
  const after: WorldObject[] = useHistoryAfter
    ? (await sources.history!.objectsAt(time.end, { bounds: bounds! })).filter(
        (o) => o.position && regionContains(region, o.position),
      )
  ...
  if (before) {
    ...
    for (const prev of before) {
      ...
      if (from !== undefined && to !== undefined && from !== to)
        statusChanges.push({ objectId: prev.id, from, to, at: next.observedAt });
    }
  ...
  const beforeCounts = before
    ? countByType(before)
    : countByType(liveInRegion(sources.state, region).filter((o) => knownAt(o, start)));
  ...
  return {
    ...
    statusChanges,
    countChanges,
    newAlerts: newEvents.filter((e) => e.type === EventTypes.WeatherAlert),
  };
}
