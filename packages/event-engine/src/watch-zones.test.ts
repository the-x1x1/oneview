...
const oahuCircle: WatchZone = { id: 'wz-oahu', name: 'Oahu', geometry: { kind: 'circle', center: { latitude: 21.44, longitude: -158.0 }, radiusM: 60_000 }, eventTypes: ['earthquake', 'weather-alert', 'wildfire-cluster'], minimumSeverity: 'MODERATE', notifications: { inApp: true, desktop: true }, enabled: true, createdAt: iso(-DAY) };
const japanPolygon: WatchZone = { id: 'wz-japan', name: 'Japan', geometry: { kind: 'polygon', polygon: [[129, 31], [146, 31], [146, 45], [129, 45], [129, 31]] }, eventTypes: [], notifications: { inApp: true, desktop: false }, enabled: true, createdAt: iso(-DAY) };
const trafficBounds: WatchZone = { id: 'wz-hnl', name: 'HNL approach', geometry: { kind: 'bounds', bounds: { west: -158.1, south: 21.2, east: -157.7, north: 21.5 } }, eventTypes: ['watch-zone-entry'], notifications: { inApp: true, desktop: false }, enabled: true, createdAt: iso(-DAY) };
...
test('WatchZoneEvaluator: circle + polygon intersection, severity gate, enter once, 6 h dedupe', () => {
  ...
  const quakeOahu = eventFrom({ id: 'event:earthquake:usgs:o1', type: 'earthquake', title: 'M5.0 earthquake — Oahu', startAt: iso(-HOUR), severity: 'MODERATE', geometry: { type: 'Point', coordinates: [-157.9, 21.4] }, objectIds: ['earthquake:usgs:o1'], summary: 'Magnitude 5.0.' });
  ...
  assert.deepEqual(hit.notification, { id: hit.event.id, title: 'Oahu: M5.0 earthquake — Oahu', body: 'M5.0 earthquake — Oahu intersects watch zone "Oahu".', severity: 'MODERATE', eventId: 'event:earthquake:usgs:o1', watchZoneId: 'wz-oahu' });
  ...
  const weak = eventFrom({ id: 'event:earthquake:usgs:o2', type: 'earthquake', startAt: iso(0), severity: 'MINOR', geometry: { type: 'Point', coordinates: [-157.9, 21.4] } });
  assert.equal(ev.evaluateEvent(weak).length, 0, 'below minimumSeverity');
  const wrongType = eventFrom({ id: 'event:launch:x:1', type: 'launch', startAt: iso(0), severity: 'EXTREME', geometry: { type: 'Point', coordinates: [-157.9, 21.4] } });
  assert.equal(ev.evaluateEvent(wrongType).length, 0, 'event type not subscribed by the circle zone');
  const outside = eventFrom({ id: 'event:earthquake:usgs:o3', type: 'earthquake', startAt: iso(0), severity: 'EXTREME', geometry: { type: 'Point', coordinates: [-155.3, 19.4] } });
  assert.equal(ev.evaluateEvent(outside).length, 0, 'Big Island is outside the Oahu circle');

  const japanQuake = eventFrom({ id: 'event:earthquake:usgs:j1', type: 'earthquake', startAt: iso(0), severity: 'INFO', geometry: { type: 'Point', coordinates: [140, 36] } });
  ...
  const alert = eventFrom({ id: 'event:weather-alert:jma:1', type: 'weather-alert', startAt: iso(0), severity: 'SEVERE', geometry: { type: 'Polygon', coordinates: [[[135, 33], [137, 33], [137, 35], [135, 35], [135, 33]]] } });
  assert.equal(ev.evaluateEvent(alert)[0]!.zone.id, 'wz-japan', 'polygon geometry vs polygon zone');
  const noGeometry = eventFrom({ id: 'event:source-status-change:x:1', type: 'source-status-change', startAt: iso(0), severity: 'INFO' });
  assert.equal(ev.evaluateEvent(noGeometry).length, 0);
  const entry = eventFrom({ id: 'event:watch-zone-entry:wz-japan:x', type: 'watch-zone-entry', startAt: iso(0), severity: 'SEVERE', geometry: { type: 'Point', coordinates: [140, 36] } });
  ...
  assert.equal(ev.evaluateEvent({ ...quakeOahu, id: 'event:earthquake:usgs:o9' }).length, 0, 'disabled zones are skipped');
});
...
test('WatchZoneEvaluator: aircraft/vessel entries fire on outside→inside transitions only', () => {
  ...
  const plane = (lat: number, lon: number, at = iso(0)) => obj({ id: 'aircraft:icao24:a1b2c3', type: 'aircraft', providerId: 'adsb-lol', observedAt: at, lat, lon, altitudeM: 900, labels: { callsign: 'HAL45' } });
  ...
  assert.equal(ev.evaluateObjects([plane(21.3, -157.9)]).length, 0, 're-entry inside the 6 h dedupe window is suppressed');
  ...
  const ship = obj({ id: 'vessel:mmsi:366123456', type: 'vessel', lat: 21.3, lon: -157.87, labels: { name: 'Aloha Star' } });
  ...
  clock.advance(7 * HOUR);
  assert.equal(ev.evaluateObjects([obj({ id: 'aircraft:icao24:ffffff', type: 'aircraft', lat: 21.3, lon: -157.9 })]).length, 0, 'MINOR entries are gated by minimumSeverity');
});
