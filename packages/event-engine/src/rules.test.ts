...
test('earthquakeRule: deterministic ids, severity by magnitude, title/summary from known data only', () => {
  ...
  assert.equal(
    e.summary,
    'Magnitude 5.7 (mww), depth 10 km, 2026-09-21 11:00 UTC. Status: reviewed. Tsunami flag set by the source.',
  );
  ...
  // severity bands
  assert.deepEqual([2.9, 3, 4.4, 4.5, 5.4, 5.5, 6.9, 7, undefined].map(magnitudeSeverity), [
    'INFO',
    'MINOR',
    'MINOR',
    'MODERATE',
  ...
  const noMag = earthquakeRule.evaluate(
    [obj({ id: 'earthquake:usgs:x1', type: 'earthquake', lat: 1, lon: 1, properties: { place: 'Somewhere' } })],
    ctxAt(T0),
  )[0]!;
  assert.equal(noMag.title, 'Earthquake — Somewhere');
  assert.equal(noMag.severity, 'INFO');
  const recorded = earthquakeRule.evaluate(
    [
      obj({
        id: 'earthquake:usgs:x2',
        type: 'earthquake',
        lat: 1,
        lon: 1,
        origin: 'recorded',
        properties: { magnitude: 4 },
      }),
    ],
    ctxAt(T0),
  )[0]!;
  assert.equal(recorded.provenance.origin, 'recorded', 'recorded inputs stay labelled recorded');
  const scoped = earthquakeRule.evaluate(
    [obj({ id: 'earthquake:emsc:abc', type: 'earthquake', lat: 1, lon: 1, properties: { magnitude: 4 } })],
    ctxAt(T0),
  )[0]!;
...
test('earthquakeRule: aftershock linking is nearest larger earlier ≥5.5 within 100 km / 7 days', () => {
  ...
  assert.equal(
    store.get('event:earthquake:usgs:main')!.properties!['mainshockEventId'],
    undefined,
    'mainshock itself is not an aftershock of the older 7.0 (outside 7 days)',
  );
  ...
  for (const e3 of earthquakeRule.evaluate([quake('late-after', 3.5, 36.0, 140.0, iso(-HOUR))], ctxAt(T0 - HOUR)))
    store3.upsert(e3);
...
test('wildfireClusterRule: stable ids, false-merge safety, severity thresholds, hull geometry', () => {
  const near = [
    fire('a', 34.1, -118.5, iso(-6 * HOUR), 12),
    fire('b', 34.12, -118.52, iso(-5 * HOUR), 30),
    fire('c', 34.11, -118.48, iso(-4 * HOUR), 8),
  ];
  const far = [fire('d', 34.5, -118.5, iso(-5 * HOUR), 5)]; // ~44 km north → separate cluster
  ...
  const chain = wildfireClusterRule.evaluate(
    [fire('c1', 50, 10, iso(0)), fire('c2', 50.04, 10, iso(0)), fire('c3', 50.08, 10, iso(0))],
    ctxAt(T0),
  );
...
test('wildfireClusterRule: identity survives the earliest detection expiring; merges end the absorbed cluster', () => {
  const store = new EventStore();
  const a = fire('a', 34.1, -118.5, iso(-30 * HOUR), 5);
  ...
  const east = fire('e', 45.0, 10.1, iso(-4 * HOUR)); // ~7.9 km apart → two clusters
...
test('weatherAlertRule: severity from payload, endAt from validUntil, geometry passthrough', () => {
  ...
  const alert = obj({
    id: 'weather-alert:nws:abc-123',
    type: 'weather-alert',
    providerId: 'nws-alerts',
    observedAt: iso(-HOUR),
    validUntil: iso(5 * HOUR),
    geometry: polygon,
    labels: { title: 'High Surf Warning issued September 21' },
    properties: {
      event: 'High Surf Warning',
      severity: 'Severe',
      areaDesc: 'Oahu North Shore',
      senderName: 'NWS Honolulu HI',
      effectiveFrom: iso(-HOUR),
      urgency: 'Expected',
      certainty: 'Likely',
    },
  });
  ...
  assert.equal(
    e!.summary,
    'High Surf Warning for Oahu North Shore from 2026-09-21 11:00 UTC until 2026-09-21 17:00 UTC. Severity: Severe. Issued by NWS Honolulu HI.',
  );
  assert.deepEqual(
    [undefined, 'Unknown', 'Minor', 'Moderate', 'Severe', 'Extreme'].map(
      (s) =>
        weatherAlertRule.evaluate(
          [
            obj({
              id: 'weather-alert:nws:s',
              type: 'weather-alert',
              lat: 1,
              lon: 1,
              properties: s ? { severity: s } : {},
            }),
          ],
          ctxAt(T0),
        )[0]!.severity,
    ),
    ['INFO', 'INFO', 'MINOR', 'MODERATE', 'SEVERE', 'EXTREME'],
  );
  const plain = weatherAlertRule.evaluate(
    [
      obj({
        id: 'weather-alert:nws:p',
        type: 'weather-alert',
        lat: 21.3,
        lon: -157.8,
        properties: { expires: iso(HOUR) },
      }),
    ],
    ctxAt(T0),
  )[0]!;
...
test('launchRule: INFO event with NET time and pad position', () => {
  const l = obj({
    id: 'launch:ll2:abc',
    type: 'launch',
    providerId: 'launch-library',
    lat: 28.6,
    lon: -80.6,
    labels: { name: 'Starlink Group 9-1' },
    properties: {
      net: iso(3 * HOUR),
      windowEnd: iso(5 * HOUR),
      vehicle: 'Falcon 9',
...
test('sourceStatusRule: notable transitions only, throttled to one per provider per 10 minutes', () => {
  const health = (status: ProviderHealth['status']): ProviderHealth => ({
  ...
  const change = (from: ProviderHealth['status'], to: ProviderHealth['status']): SourceChange => ({
    ...
    entry: {
      ...
      locality: 'remote',
      enabled: true,
      health: { ...health(to), lastError: { code: 'NETWORK', message: 'fetch failed', at: iso(0) } },
      transitions: [],
      meta: {
        attribution: 'USGS',
  ...
  assert.equal(
    e.summary,
    'USGS Earthquakes changed from live to offline at 2026-09-21 12:00 UTC. Last error: NETWORK.',
  );
  assert.deepEqual(e.properties, {
    providerId: 'usgs-earthquakes',
    from: 'LIVE',
