...
const f = (i: number, layer = 'fire', priority = i % 5): RenderFeature => ({ id: `f${i}`, geometry: { kind: 'point', position: { latitude: i * 0.001, longitude: 0 } }, style: { styleClass: layer }, interactive: true, priority, layer });
...
test('dense budget: caps per band, keeps highest priority (stable by id), scales under low power', () => {
  ...
  assert.ok(r.kept.every((k) => k.priority >= 3), 'lowest priorities dropped first');
  assert.deepEqual(r.kept.map((k) => k.id), budget.apply([...many].reverse(), 'local').kept.map((k) => k.id), 'order-independent');
  ...
  assert.equal(new DenseBudget().scaled(0).capFor('local'), Math.round(DEFAULT_DENSE_CAPS.local * 0.05), 'never scales to zero');
});
...
test('native dense adapter: replaces the layer wholesale on the active renderer within budget; reports support by count', () => {
  ...
  const adapter = new NativeDenseAdapter({ kind: 'points', renderer, layer: 'fire', budget: new DenseBudget({ local: 3 }), nativeLimit: 100 });
  assert.equal(adapter.supports(100), true);
  assert.equal(adapter.supports(101), false, 'above the native limit the host should look for a dense renderer (deck.gl) — none today');
...
test('benchmark harness: deterministic synthetic objects and a well-formed report at small sizes', () => {
  const a = syntheticObjects(50, 7), b = syntheticObjects(50, 7);
  assert.deepEqual(a, b);
  assert.notDeepEqual(syntheticObjects(50, 8), a);
  const r1 = mulberry32(3), r2 = mulberry32(3);
  assert.equal(r1(), r2());
  let t = 0;
  ...
  assert.ok(report.cases.find((c) => c.band === 'global' && c.objects === 1_000)!.density > 0, 'aircraft/vessels aggregate to density at global zoom');
