...
test('cameras: a registered camera is restored into its gateway after a restart', async () => {
  ...
  try {
    ...
    const stored = JSON.parse(await fs.readFile(path.join(dataDir, 'cameras.json'), 'utf8')) as {
    ...
    const providerSettings = JSON.parse(
      await fs.readFile(path.join(dataDir, 'provider-settings.json'), 'utf8'),
    ...
    try {
      ...
      assert.deepEqual(
        list.map((c) => c.name),
        ['Driveway'],
        'the gateway knows the camera again',
      );
  ...
  try {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        ...
        items: [
          {
            id: 'aaaaaaaaaaaa',
            cameraId: 'aaaaaaaaaaaa',
            objectId: 'camera:cameras-local:aaaaaaaaaaaa',
            name: 'Good',
            url: 'http://10.0.0.9/snapshot.jpg',
            kind: 'snapshot',
            registeredAt: '2026-09-21T08:00:00.000Z',
          },
          {
            id: 'bbbbbbbbbbbb',
            cameraId: 'bbbbbbbbbbbb',
            objectId: 'camera:cameras-local:bbbbbbbbbbbb',
            name: 'File URL',
            url: 'file:///etc/passwd',
            kind: 'snapshot',
            registeredAt: '2026-09-21T08:00:00.000Z',
          ...
          {
            id: 'cccccccccccc',
            cameraId: 'cccccccccccc',
            objectId: 'camera:cameras-local:cccccccccccc',
            name: 'Inline creds',
            url: 'http://u:p@10.0.0.9/snapshot.jpg',
            kind: 'snapshot',
            registeredAt: '2026-09-21T08:00:00.000Z',
          },
          {
            id: 'not-hex',
            cameraId: 'not-hex',
            objectId: 'camera:cameras-local:x',
            name: 'Bad id',
            url: 'http://10.0.0.9/a.jpg',
            kind: 'snapshot',
            registeredAt: '2026-09-21T08:00:00.000Z',
          },
          {
            id: 'dddddddddddd',
            cameraId: 'dddddddddddd',
            objectId: 'camera:cameras-local:dddddddddddd',
            name: 'Bad kind',
            url: 'http://10.0.0.9/a.jpg',
            kind: 'websocket',
            registeredAt: '2026-09-21T08:00:00.000Z',
    ...
    try {
      ...
      assert.deepEqual(
        list.map((c) => c.name),
        ['Good'],
        'only the valid entry is restored',
      );
    } finally {
      await h.dispose();
    }
