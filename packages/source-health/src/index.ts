...
export class SourceHealthRegistry {
  ...
  private readonly listeners: { change: Set<Listener<'change'>>; connection: Set<Listener<'connection'>> } = { change: new Set(), connection: new Set() };
  private networkOnline = true;
  private lastConnection: ConnectionState | undefined;

  constructor(private readonly clock: Clock = systemClock, private readonly maxTransitions = 20) {}

  register(manifest: ProviderManifest, opts: { enabled: boolean; locality?: SourceLocality }): SourceHealthEntry {
    const locality = opts.locality ?? (manifest.transport === 'local-process' || manifest.transport === 'hardware' ? 'local' : manifest.transport === 'filesystem' ? 'bundled' : 'remote');
  ...
  get(providerId: string): SourceHealthEntry | undefined { return this.entries.get(providerId); }
  list(): SourceHealthEntry[] { return [...this.entries.values()].sort((a, b) => a.name.localeCompare(b.name)); }

  connection(): ConnectionSnapshot {
    let remoteLive = 0, remoteTotal = 0, localLive = 0;
    for (const e of this.entries.values()) {
      if (!e.enabled) continue;
      if (e.locality === 'remote') { remoteTotal++; if (LIVE_LIKE.has(e.health.status)) remoteLive++; }
    ...
    return { state, networkOnline: this.networkOnline, remoteLive, remoteTotal, localLive, at: new Date(this.clock.now()).toISOString() };
  }
