...
export type GeoRegion =
  ...
  | { kind: 'admin'; /** Stable admin id such as `iso3166-1:US` or `iso3166-2:US-HI` */ regionId: string; bounds?: GeoBounds };
...
export function isValidBounds(b: GeoBounds): boolean {
  return (
    Number.isFinite(b.west) && Number.isFinite(b.east) && Number.isFinite(b.south) && Number.isFinite(b.north) &&
    b.south <= b.north && Math.abs(b.south) <= 90 && Math.abs(b.north) <= 90 && Math.abs(b.west) <= 180 && Math.abs(b.east) <= 180
  );
}
...
export function bearingDegrees(a: { latitude: number; longitude: number }, b: { latitude: number; longitude: number }): number {
  const φ1 = a.latitude * DEG, φ2 = b.latitude * DEG, Δλ = (b.longitude - a.longitude) * DEG;
...
export function boundsIntersect(a: GeoBounds, b: GeoBounds): boolean {
  ...
  const aSplit = a.west > a.east, bSplit = b.west > b.east;
...
export function circleBounds(center: GeoPosition, radiusM: number): GeoBounds {
  ...
  return { west: normalizeLongitude(center.longitude - dLon), south, east: normalizeLongitude(center.longitude + dLon), north };
}
...
export function pointInPolygon(p: { latitude: number; longitude: number }, ring: Array<[number, number]>): boolean {
  ...
  for (let i = 0, j = n - 1; i < n; j = i++) {
    const [xi, yi] = ring[i]!;
    const [xj, yj] = ring[j]!;
    const intersect = yi > p.latitude !== yj > p.latitude && p.longitude < ((xj - xi) * (p.latitude - yi)) / (yj - yi) + xi;
...
export function regionBounds(region: GeoRegion): GeoBounds | undefined {
  switch (region.kind) {
    case 'bounds': return region.bounds;
    case 'circle': return circleBounds(region.center, region.radiusM);
    case 'polygon': return polygonBounds(region.polygon);
    case 'admin': return region.bounds;
  }
}
...
export function regionContains(region: GeoRegion, p: { latitude: number; longitude: number }): boolean {
  switch (region.kind) {
    case 'bounds': return boundsContain(region.bounds, p);
    case 'circle': return haversineMeters(region.center, p) <= region.radiusM;
    case 'polygon': return pointInPolygon(p, region.polygon);
    case 'admin': return region.bounds ? boundsContain(region.bounds, p) : false;
  }
}
...
export function geometryCentroid(g: WorldGeometry): GeoPosition | undefined {
  ...
  switch (g.type) {
    case 'Point': pts.push(g.coordinates); break;
    case 'MultiPoint': case 'LineString': pts.push(...g.coordinates); break;
    case 'MultiLineString': case 'Polygon': for (const r of g.coordinates) pts.push(...r); break;
    case 'MultiPolygon': for (const poly of g.coordinates) for (const r of poly) pts.push(...r); break;
  }
...
export function positionToGeometry(p: GeoPosition): WorldGeometry {
  return p.altitudeM !== undefined ? { type: 'Point', coordinates: [p.longitude, p.latitude, p.altitudeM] } : { type: 'Point', coordinates: [p.longitude, p.latitude] };
}
