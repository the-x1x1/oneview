...
export class FakeWorldRenderer implements WorldRenderer {
  ...
  constructor(
    mode: '2D' | '3D',
    private readonly options: { withTerrain?: boolean; mountDelayMs?: number } = {},
  ) {
    this.capabilities = {
      mode,
      terrain: mode === '3D',
  ...
  setView(view: Partial<ViewState>, _opts?: { animate?: boolean; durationMs?: number }): void {
    ...
    if (view.zoom !== undefined && view.altitudeM === undefined)
      next.altitudeM = zoomToAltitudeM(view.zoom, next.center.latitude);
    if (view.altitudeM !== undefined && view.zoom === undefined)
      next.zoom = altitudeToZoom(view.altitudeM, next.center.latitude);
    this.view = next;
  }
  getView(): ViewState {
    return this.view;
  ...
  async flyTo(
    target: { position: { latitude: number; longitude: number }; altitudeM?: number; zoom?: number },
  ...
  listenerCount(event: keyof RendererEvents): number {
    return this.listeners[event]?.size ?? 0;
  }
  simulatePick(result: PickResult | null): void {
