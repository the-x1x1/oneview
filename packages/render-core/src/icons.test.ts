...
function recordingContext(): GlyphContext & { ops: string[] } {
  ...
  const ctx = {
    ops,
    fillStyle: '', strokeStyle: '', lineWidth: 1, lineJoin: 'miter', lineCap: 'butt', globalAlpha: 1,
    ...
    arc: (x: number, y: number, r: number) => { ops.push(`arc ${x.toFixed(2)},${y.toFixed(2)},${r.toFixed(2)}`); },
...
function fakeCanvas(width: number, height: number): GlyphCanvas & { ctx: ReturnType<typeof recordingContext> } {
  ...
  return { width, height, ctx, getContext: () => ctx, toDataURL: () => `data:image/png;base64,${Buffer.from(ctx.ops.join('|')).toString('base64').slice(0, 24)}` };
}
...
test('icons: every icon draws a distinct, non-empty path and stays inside the unit box', () => {
  ...
  for (const id of ICON_IDS) {
    ...
    for (const op of ctx.ops) {
      ...
      if (m) { assert.ok(Number(m[1]) >= 0 && Number(m[1]) <= 1 && Number(m[2]) >= 0 && Number(m[2]) <= 1, `${id}: ${op} inside box`); }
    }
...
test('icons: unknown ids fall back to the default disc; sprites are rendered once per icon', () => {
  const a = recordingContext(); drawGlyph(a, 'does-not-exist', 16);
  const b = recordingContext(); drawGlyph(b, 'default', 16);
