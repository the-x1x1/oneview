...
export class CameraError extends Error {
  ...
  constructor(code: CameraErrorCode, message: string, opts: { httpStatus?: number; retryable?: boolean; cause?: unknown } = {}) {
  ...
  toInfo(): { code: CameraErrorCode; message: string; httpStatus?: number } {
    return { code: this.code, message: this.message, ...(this.httpStatus !== undefined ? { httpStatus: this.httpStatus } : {}) };
  }
  ...
  get ipcCode(): 'INVALID_REQUEST' | 'NOT_FOUND' | 'UNAVAILABLE' | 'INTERNAL' | 'CANCELLED' {
    switch (this.code) {
      case 'INVALID_URL': case 'UNSUPPORTED_SCHEME': case 'UNSUPPORTED': return 'INVALID_REQUEST';
...
export function toCameraError(err: unknown): CameraError {
  ...
  if (/ENOTFOUND|EAI_AGAIN|ECONNREFUSED|ECONNRESET|EHOSTUNREACH|ENETUNREACH|fetch failed|network/i.test(message)) return new CameraError('NETWORK', message.slice(0, 200), { cause: err });
...
export function errorForStatus(status: number): CameraError | undefined {
  ...
  if (status >= 300 && status < 400) return new CameraError('UPSTREAM_ERROR', `upstream redirect (${status}) not followed`, { httpStatus: status, retryable: false });
  if (status === 401 || status === 403) return new CameraError('UPSTREAM_REFUSED', `upstream refused the request (HTTP ${status})`, { httpStatus: status, retryable: false });
  if (status === 404) return new CameraError('UPSTREAM_ERROR', 'upstream has no such resource (HTTP 404)', { httpStatus: status, retryable: false });
  if (status === 429) return new CameraError('UPSTREAM_ERROR', 'upstream rate limited (HTTP 429)', { httpStatus: status });
  if (status >= 500) return new CameraError('UPSTREAM_ERROR', `upstream error (HTTP ${status})`, { httpStatus: status });
  return new CameraError('UPSTREAM_ERROR', `unexpected upstream status ${status}`, { httpStatus: status, retryable: false });
}
