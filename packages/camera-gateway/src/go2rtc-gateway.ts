...
export class Go2rtcGateway implements CameraGateway {
  ...
  async status(): Promise<GatewayStatus> {
    ...
    const state: GatewayStatus['state'] =
      sidecar.status === 'running' ? 'ready' : sidecar.status === 'not-configured' ? 'not-configured' : 'unavailable';
  ...
  async register(source: CameraSourceInput): Promise<CameraRegistration> {
    ...
    if (source.position && isValidLatLon(source.position.latitude, source.position.longitude))
      record.position = {
        latitude: source.position.latitude,
        longitude: source.position.longitude,
      } satisfies GeoPosition;
    if (typeof source.headingDegrees === 'number' && Number.isFinite(source.headingDegrees))
      record.headingDegrees = ((source.headingDegrees % 360) + 360) % 360;
    this.cameras.set(cameraId, record);
    this.synced.delete(cameraId);
    if (this.opts.sidecar.isRunning()) await this.pushStream(record);
    this.logger.info('camera registered (go2rtc)', {
      cameraId,
      kind: record.kind,
  ...
  async syncStreams(): Promise<void> {
    ...
    for (const record of this.cameras.values()) {
      ...
      try {
        await this.pushStream(record);
      } catch (err) {
        this.logger.warn('go2rtc stream sync failed', { cameraId: record.cameraId, code: toCameraError(err).code });
      }
    }
  }
  ...
  async snapshot(cameraId: string): Promise<CameraSnapshot> {
    ...
    try {
      ...
      const res = await this.opts.fetch(
        `${this.opts.sidecar.apiBase()}/api/frame.jpeg?src=${encodeURIComponent(cameraId)}`,
      ...
      if (buf.byteLength > MAX_FRAME_BYTES)
        throw new CameraError('TOO_LARGE', 'frame exceeds size cap', { retryable: false });
  ...
  async stream(cameraId: string): Promise<CameraStreamDescriptor> {
    ...
    if ((this.opts.streamKind ?? 'hls') === 'webrtc')
      return { cameraId, kind: 'webrtc', url: `${api}/api/webrtc?src=${encodeURIComponent(cameraId)}` };
  ...
  async unregister(cameraId: string): Promise<void> {
    ...
    if (this.opts.sidecar.isRunning()) {
      try {
        await this.opts.fetch(`${this.opts.sidecar.apiBase()}/api/streams?src=${encodeURIComponent(cameraId)}`, {
          method: 'DELETE',
          signal: AbortSignal.timeout(5000),
        });
      } catch (err) {
        this.logger.warn('go2rtc stream removal failed', { cameraId, code: toCameraError(err).code });
      }
    }
  }
  ...
  restore(records: RegisteredCamera[]): void {
    for (const r of records) {
      ...
      try {
        parseCameraUrl(r.url, ['rtsp', 'rtsps', 'http', 'https']);
      } catch {
        continue;
      }
  ...
  private async pushStream(record: RegisteredCamera): Promise<void> {
    ...
    if (res.status < 200 || res.status >= 300)
      throw new CameraError('UPSTREAM_ERROR', `go2rtc refused the stream (HTTP ${res.status})`, {
