...
test('parseCameraUrl rejects non-camera and disallowed schemes with typed errors', () => {
  for (const bad of ['file:///etc/passwd', 'javascript:alert(1)', 'ftp://cam/x.jpg', 'data:image/jpeg;base64,xx']) {
    assert.throws(() => parseCameraUrl(bad, HTTP), (e: unknown) => e instanceof CameraError && e.code === 'UNSUPPORTED_SCHEME', bad);
  }
  assert.throws(() => parseCameraUrl('rtsp://cam/live', HTTP), (e: unknown) => e instanceof CameraError && e.code === 'UNSUPPORTED_SCHEME' && /go2rtc/.test(e.message));
  assert.throws(() => parseCameraUrl('not a url', HTTP), (e: unknown) => e instanceof CameraError && e.code === 'INVALID_URL');
  assert.throws(() => parseCameraUrl('', HTTP), (e: unknown) => e instanceof CameraError && e.code === 'INVALID_URL');
  assert.throws(() => parseCameraUrl('http://', HTTP), (e: unknown) => e instanceof CameraError);
...
test('image magic detection and MJPEG first-frame extraction', async () => {
  ...
  async function* body(): AsyncGenerator<Uint8Array> { for (const c of mjpegChunks(3)) yield c; }
  const frame = await firstJpegFrame(body(), 1024 * 1024);
  assert.deepEqual([...frame], [...JPEG_BYTES]);
  async function* split(): AsyncGenerator<Uint8Array> { const all = Buffer.concat(mjpegChunks(1)); for (let i = 0; i < all.length; i += 5) yield all.subarray(i, i + 5); }
  assert.deepEqual([...(await firstJpegFrame(split(), 1024))], [...JPEG_BYTES]);
  async function* noise(): AsyncGenerator<Uint8Array> { for (let i = 0; i < 10; i++) yield new Uint8Array(100); }
  await assert.rejects(firstJpegFrame(noise(), 500), (e: unknown) => e instanceof CameraError && e.code === 'TOO_LARGE');
});
