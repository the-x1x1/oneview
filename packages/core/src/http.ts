...
export class HttpClient {
  ...
  readonly stats: HttpClientStats = { requests: 0, coalesced: 0, cacheHits: 0, notModified: 0, staleServed: 0, failures: 0, rateLimitedWaits: 0 };
  ...
  private breaker(host: string): CircuitBreaker {
    ...
    if (!b) { b = new CircuitBreaker({ failureThreshold: 3, openMs: 30_000, maxOpenMs: 10 * 60_000 }, this.opts.clock); this.breakers.set(host, b); }
    return b;
  }

  async request(req: ProviderHttpRequest): Promise<ProviderHttpResponse> {
    let parsed: URL;
    try { parsed = new URL(req.url); } catch { throw new ProviderError('INTERNAL', `invalid url`, { retryable: false }); }
    if (parsed.protocol !== 'https:' && !(parsed.protocol === 'http:' && isLoopback(parsed.hostname))) {
      throw new ProviderError('HOST_NOT_ALLOWED', `only https (or http to loopback) is allowed`, { retryable: false });
    }
    if (!this.isHostAllowed(req.url)) throw new ProviderError('HOST_NOT_ALLOWED', `host ${parsed.hostname} is not in the allowlist`, { retryable: false });
  ...
  private async execute(req: ProviderHttpRequest, parsed: URL, key: string): Promise<ProviderHttpResponse> {
    ...
    const serveStale = (err: ProviderError): ProviderHttpResponse => {
      if (req.allowStale !== false && cached && staleMs > 0 && clock.now() - cached.storedAt <= staleMs) {
        ...
        this.logger.warn('serving stale response after failure', { host, code: err.code, ageMs: clock.now() - cached.storedAt });
        return toResponse(cached, { fromCache: true, stale: true, ageMs: clock.now() - cached.storedAt, latencyMs: 0 });
      }
      throw err;
    };

    if (this.opts.online && !this.opts.online()) return serveStale(new ProviderError('OFFLINE', 'application is offline'));
    if (!breaker.allow()) return serveStale(new ProviderError('NETWORK', `circuit open for ${host}; retry in ${breaker.retryInMs()}ms`, { retryAfterMs: breaker.retryInMs() }));

    const wait = this.limiter.tryAcquire(host);
    if (wait > 0) {
      this.stats.rateLimitedWaits++;
      if (wait > 10_000) return serveStale(new ProviderError('RATE_LIMITED', `client rate limit for ${host}`, { retryAfterMs: wait }));
      await (this.opts.sleep ?? sleep)(wait, req.signal);
    }

    const timeoutMs = Math.min(req.timeoutMs ?? this.opts.defaultTimeoutMs ?? 15_000, this.opts.maxTimeoutMs ?? 120_000);
    const maxBytes = Math.min(req.maxBytes ?? this.opts.defaultMaxBytes ?? 8 * 1024 * 1024, this.opts.hardMaxBytes ?? 64 * 1024 * 1024);
    ...
    const attemptOnce = async (): Promise<ProviderHttpResponse> => {
      ...
      try {
        ...
        if (res.status >= 300 && res.status < 400) {
          ...
          throw new ProviderError('HTTP_4XX', `redirect to ${location ? safeHost(location) : 'unknown'} not followed`, { httpStatus: res.status, retryable: false });
        }
        if (res.status === 429) {
        ...
        if (res.status === 401 || res.status === 403) throw new ProviderError('AUTH', `HTTP ${res.status}`, { httpStatus: res.status, retryable: false });
        if (res.status >= 500) throw new ProviderError('HTTP_5XX', `HTTP ${res.status}`, { httpStatus: res.status });
        if (res.status >= 400) throw new ProviderError('HTTP_4XX', `HTTP ${res.status}`, { httpStatus: res.status, retryable: false });
    ...
    try {
      const result = await withRetry(attemptOnce, {
        ...
        isRetryable: (e) => e instanceof ProviderError && e.retryable && e.code !== 'RATE_LIMITED' && e.code !== 'CANCELLED',
        sleep: this.opts.sleep ?? sleep,
        ...(req.signal ? { signal: req.signal } : {}),
        onRetry: (attempt, delayMs, e) => this.logger.debug('retrying request', { host, attempt, delayMs, code: e instanceof ProviderError ? e.code : 'unknown' }),
      });
...
async function readCapped(res: Response, maxBytes: number, signal: AbortSignal): Promise<Uint8Array> {
  ...
  if (Number.isFinite(declared) && declared > maxBytes) throw new ProviderError('TOO_LARGE', `content-length ${declared} exceeds ${maxBytes}`, { retryable: false });
  ...
  try {
    for (;;) {
      ...
      if (total > maxBytes) throw new ProviderError('TOO_LARGE', `response exceeded ${maxBytes} bytes`, { retryable: false });
      chunks.push(value);
    }
  } finally {
...
function classify(err: unknown): ProviderError {
  ...
  if (/ENOTFOUND|EAI_AGAIN|getaddrinfo/i.test(message)) return new ProviderError('DNS', 'dns lookup failed', { cause: err });
