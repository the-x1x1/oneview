...
export class RateLimiter {
  ...
  constructor(
    private readonly opts: { windowMs: number; max: number; globalMax?: number; maxKeys?: number },
  ...
  tryAcquire(key = 'default'): number {
    ...
    if (!bucket) {
      if (this.buckets.size >= (this.opts.maxKeys ?? 2000))
    ...
    if (this.opts.globalMax !== undefined && this.global.length >= this.opts.globalMax)
      return Math.max(1, this.global[0]! + this.opts.windowMs - now);
    bucket.push(now);
...
export class CircuitBreaker {
  ...
  constructor(
    private readonly opts: { failureThreshold: number; openMs: number; maxOpenMs: number },
    private readonly clock: Clock,
  ) {}
