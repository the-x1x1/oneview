...
test('Earth Search example: the shared suite, with scenes at their bbox centres and the payload the brief names', async () => {
  const r = await runConnectorSuite(example(EARTH_SEARCH), {
    ...
    verify: (obs) => {
      for (const o of obs) {
        if (o.objectType !== 'place' || o.payload['kind'] !== 'imagery-scene') return `${o.externalId}: not a scene`;
        if (o.geometry?.type !== 'Polygon') return `${o.externalId}: footprint missing`;
        if (!Array.isArray(o.payload['assets']) || !o.payload['assets'].includes('visual'))
      ...
      if (b.payload['cloudCoverPercent'] !== 41.02) return 'eo:cloud_cover';
      ...
      if (typeof c.payload['thumbnail'] !== 'string' || !c.payload['thumbnail'].endsWith('/preview.jpg'))
...
test('both examples load from connectors/examples/stac: user-configured, disabled, nothing opened', () => {
  ...
  for (const d of loaded.definitions) {
    ...
    assert.equal(d.objectType, 'place');
...
test('item records: thumbnail absolute and https only, asset keys, the collection-qualified key, anything not an object untouched', () => {
  const item = {
    type: 'Feature',
    id: 'a',
    collection: 'c',
...
test('static catalogue: children and items to the leaves, one item reached twice read once, other hosts and a missing branch', async () => {
  ...
  assert.equal(a.payload['itemUrl'], `${STATIC_ROOT}by-mode/spotlight/items/EXAMPLE_SAR_SP_20260105T103000.json`);
  assert.equal(a.payload['instrumentMode'], 'spotlight');
  assert.equal(a.payload['kind'], 'imagery-scene');
  ...
  assert.equal(b.payload['thumbnail'], undefined, 'an http thumbnail is not kept');
  const message = (await provider.health()).message ?? '';
...
test('validation: what a STAC definition may not say, each refused with the reason', () => {
  ...
  refuse({ ...es, objectType: 'sensor' }, /objectType must be "place"/);
...
test('defaults: a definition that names only the id gets the time, centre, footprint and kind filled in', async () => {
  const es = example(EARTH_SEARCH);
  ...
  assert.equal(c.payload['kind'], 'imagery-scene');
});
