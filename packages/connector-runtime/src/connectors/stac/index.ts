import { DEFAULT_FRESHNESS_POLICIES, ObjectTypes, type JsonValue, type Observation } from '@worldview/world-model';
import {
  PollingProvider,
  ProviderError,
 ...
 *
 * Object type: `imagery-scene` (ADR-002, 2026-09-23 amendment). A definition that leaves
 * the payload out gets the amendment's keys (`SCENE_PAYLOAD`); scenes are kept as long as
 * the type's freshness (or the definition's) allows, and the time window is capped to it so
 * the search never asks for scenes the world would refuse as expired. Everything reaches the
 * network through `ProviderContext.http`.
 */
 ...
 * gets for every key it does not map itself.
 */
export const SCENE_PAYLOAD: Readonly<Record<string, Field>> = Object.freeze({
  sceneId: 'id',
  collection: 'collection',
  platform: 'properties.platform',
  instrument: { path: 'properties.instruments', transform: 'first' },
  capturedAt: { path: 'properties.datetime', fallback: 'properties.start_datetime', transform: 'isoTimestamp' },
  cloudCoverPct: { path: 'properties.eo:cloud_cover', transform: 'number' },
  gsdM: { path: 'properties.gsd', transform: 'number' },
  processingLevel: 'properties.processing:level',
  sourceUrl: '_stac.href',
  thumbnailUrl: '_stac.thumbnail',
  assetKeys: '_stac.assets',
});
...
export function withStacDefaults(d: ConnectorProviderDefinition): ConnectorProviderDefinition {
  ...
  mapping.properties = { ...SCENE_PAYLOAD, ...(mapping.properties ?? {}) };
...
/**
 * How long the world keeps a scene, in ms: the definition's freshness if it has one (no
 * `expireSeconds` there means kept while the source lists it), else the object type's
 * default. Undefined: not expired by age.
 */
export function keptForMs(d: ConnectorProviderDefinition): number | undefined {
  const seconds = d.freshness ? d.freshness.expireSeconds : DEFAULT_FRESHNESS_POLICIES[d.objectType]?.expireSeconds;
  return seconds === undefined ? undefined : seconds * 1000;
}

/** The longest window worth searching: 366 days, or less when scenes are not kept that long. */
export function maxWindowMs(d: ConnectorProviderDefinition): number {
  const kept = keptForMs(d);
  return Math.max(MIN_WINDOW_MS, Math.min(MAX_WINDOW_MS, kept ?? MAX_WINDOW_MS));
}
...
export function stacManifest(d: ConnectorProviderDefinition): ProviderManifest {
  ...
  if (dt && 'rolling' in dt && !has(WINDOW_SETTING))
    settings.push({
      ...
      kind: 'number',
      min: 1,
      max: Math.max(1, Math.floor(maxWindowMs(d) / DAY_MS)),
      step: 1,
      defaultLabel: windowDaysLabel(Math.min(dt.rolling, maxWindowMs(d))),
    });
...
export class StacProvider extends PollingProvider {
  ...
  /**
   * The window this poll searches: the operator's setting, else the definition's, else seven
   * days — never longer than scenes are kept (`maxWindowMs`).
   */
  async windowMs(): Promise<number> {
    if (!('rolling' in this.datetime)) return 0;
    const days = await this.setting(WINDOW_SETTING);
    const wanted = days === undefined ? this.datetime.rolling : days * DAY_MS;
    return Math.min(maxWindowMs(this.definition), Math.max(MIN_WINDOW_MS, wanted));
  }
  ...
  private mapItems(
    ...
    for (const { item, base, own } of items) {
      const r = itemRecord(item, base, own === true);
...
export function validateStac(input: ConnectorProviderDefinition): ConnectorValidationResult {
  ...
  if (input.objectType !== ObjectTypes.ImageryScene)
    errors.push(`objectType must be "${ObjectTypes.ImageryScene}": a STAC item is a scene; got "${input.objectType}"`);
  const kept = keptForMs(d);
  const days = windowDaysLabel;
  ...
  if (mode === 'search') {
    ...
    if (kept !== undefined && 'rolling' in dt && dt.rolling > kept)
      warnings.push(
        `the time window (${days(dt.rolling)}) is longer than scenes are kept (${days(kept)}, freshness): it is cut to ${days(maxWindowMs(d))}`,
      );
    if (kept !== undefined && 'fixed' in dt)
      warnings.push(`scenes older than ${days(kept)} are refused as expired, whatever the fixed datetime asks for`);
  } else {
    warnings.push(
    ...
    if (kept !== undefined)
      warnings.push(
        `catalogue scenes older than ${days(kept)} are refused as expired (the type's freshness); give the definition a freshness without expireSeconds to keep them`,
      );
