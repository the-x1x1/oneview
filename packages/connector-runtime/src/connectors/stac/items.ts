 ...
 *   _stac.href                 the item's own page: its `self` link, else the file it was read from
 ...
 * file, or the search or item-collection page that held it), against which relative hrefs
 * resolve; `own` says `base` is the item's own file, and so a page about this item when it
 * has no `self` link. Anything that is not an object is passed through untouched for the
 * mapping to reject with its reason.
 */
export function itemRecord(raw: unknown, base: string, own = false): ItemRecord {
  ...
  const href = httpsHref(self?.['href'], base) ?? (own ? httpsHref(base, base) : undefined);
  if (href) stac['href'] = href;
  if (fp) {
