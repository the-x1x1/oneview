...
import {
  ...
  type GrantedFolderAccess,
  type Ogr2ogrAccess,
  type Ogr2ogrDetection,
  type Ogr2ogrRequest,
} from '../contract.js';
import { checkRelativePath, extensionOf } from '../path-policy.js';
import { GDAL_INPUT_EXTENSIONS } from '../gdal-import.js';

/**
 * The host side of amendments A2 and A3, written so the connectors' path and conversion
 * rules can be tested against a real file system, real links and a real child process. It
 * is what `packages/runtime/src/support/provider-storage.ts` would gain when the integrator
 * lands the amendments (and then this file is deleted); nothing in the connectors imports
 * it, and the package index does not export it.
 *
 * A2 — reads inside a granted folder: the relative path passes the connector's own policy,
 * then the folder's and the file's real paths (links, junctions and `..` resolved by the OS)
 * are compared, so a link that leads out of the folder is refused however it is spelt. Only
 * regular files are read (a FIFO would block forever); the handle opened is checked to be
 * the file that was checked (same device and inode) before a byte is read.
 *
 * A3 — ogr2ogr: found on PATH (an `.exe` on Windows: Node refuses to run `.bat`/`.cmd`
 * without a shell, and there is never a shell), run with a fixed argument list, a minimal
 * environment (no WORLDVIEW secrets), a timeout and a fresh temporary directory that is
 * always removed.
 */
export const HOST_MAX_FILE_BYTES = 32 * 1024 * 1024;

export interface GrantedFolderOptions {
  /** The folder the operator named in the source's setting; undefined or empty: nothing is granted. */
...
export function createGrantedFolderAccess(opts: GrantedFolderOptions): GrantedFolderAccess {
  const hostMax = opts.maxBytes ?? HOST_MAX_FILE_BYTES;
  return {
    async statGrantedFile(relative) {
      const { stat } = await resolveInside(opts.folder, relative);
      return { size: stat.size, mtimeMs: stat.mtimeMs };
    },
    async readGrantedFile(relative, readOpts) {
      const { real, stat } = await resolveInside(opts.folder, relative);
      const limit = Math.min(readOpts?.maxBytes ?? hostMax, hostMax);
      if (stat.size > limit)
      ...
      try {
        ...
        if (opened.ino !== stat.ino || opened.dev !== stat.dev)
          throw new ProviderError('HOST_NOT_ALLOWED', 'the file was replaced while it was opened', {
            retryable: false,
          });
...
export function childEnvironment(parent: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  for (const [k, v] of Object.entries(parent))
    if (v !== undefined && (ENV_KEEP.test(k) || ENV_KEEP_PREFIX.test(k)) && !/^ONEVIEW_/i.test(k)) out[k] = v;
  return out;
}
 ...
 */
async function datasetParts(resolved: Resolved, ext: string): Promise<Stats[]> {
...
export function createOgr2ogrAccess(opts: Ogr2ogrHostOptions): Ogr2ogrAccess {
  ...
  const checkInput = (input: string) => {
    ...
    if (!GDAL_INPUT_EXTENSIONS.includes(ext))
  ...
  return {
    ...
    async datasetStat(input): Promise<GrantedFileStat> {
      ...
      const resolved = await resolveInside(opts.folder, input, { allowDirectory: ext === 'gdb' });
      let size = 0;
    ...
    async toGeoJson(req: Ogr2ogrRequest): Promise<Uint8Array> {
      ...
      if (req.layer !== undefined && !LAYER_NAME.test(req.layer))
        throw new ProviderError('HOST_NOT_ALLOWED', 'the layer name is not allowed', { retryable: false });
      const resolved = await resolveInside(opts.folder, req.input, { allowDirectory: ext === 'gdb' });
