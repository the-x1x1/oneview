...
import { buildEmptyStyle, buildRasterStyle, buildWorldviewStyle, styleForBasemap, DARK_PALETTE, LIGHT_PALETTE, DEFAULT_GLYPHS_URL } from './styles/worldview-dark.js';
...
const PROTOMAPS_LAYERS = new Set(['earth', 'landuse', 'natural', 'water', 'buildings', 'roads', 'transit', 'boundaries', 'places', 'pois', 'physical_line', 'physical_point']);
...
test('style: worldview-dark is a valid style for the Protomaps basemap schema (unique ids, known sources and source-layers, glyphs for text)', () => {
  const style = buildWorldviewStyle({ sourceUrl: 'pmtiles://packs/europe.pmtiles', variant: 'dark', attribution: '© OpenStreetMap contributors', glyphs: DEFAULT_GLYPHS_URL });
  assert.deepEqual(validateStyle(style), []);
  assert.equal(style.version, 8);
  assert.ok(style.sources['basemap'] && style.sources['basemap'].type === 'vector' && style.sources['basemap'].url === 'pmtiles://packs/europe.pmtiles');
  const ids = style.layers.map((l) => l.id);
  assert.equal(new Set(ids).size, ids.length);
  for (const l of style.layers) if (l.type !== 'background') assert.ok(PROTOMAPS_LAYERS.has(l['source-layer']!), `${l.id} uses a Protomaps layer (${l['source-layer']})`);
  assert.equal(style.layers[0]!.type, 'background');
  assert.ok(ids.indexOf('water') < ids.indexOf('roads-highway') && ids.indexOf('roads-highway') < ids.indexOf('places-locality'), 'water under roads under labels');
  ...
  assert.equal((style.layers[0] as { paint: { 'background-color': string } }).paint['background-color'], DARK_PALETTE.background);
  const light = buildWorldviewStyle({ sourceUrl: 'pmtiles://x', variant: 'light', attribution: '', glyphs: DEFAULT_GLYPHS_URL });
  assert.equal((light.layers[0] as { paint: { 'background-color': string } }).paint['background-color'], LIGHT_PALETTE.background);
...
test('style: validation catches broken styles; raster/empty/basemap-descriptor styles are valid', () => {
  const broken: MapStyle = { version: 8, name: 'x', sources: { v: { type: 'vector', url: 'pmtiles://a' }, r: { type: 'raster', tiles: [] } }, layers: [
  ...
  assert.deepEqual(validateStyle(buildRasterStyle({ id: 'esri', tiles: ['https://t/{z}/{y}/{x}'], maxzoom: 19, attribution: 'Powered by Esri' })), []);
  assert.deepEqual(validateStyle(buildEmptyStyle()), []);
  const fromPack = styleForBasemap({ kind: 'pmtiles', id: 'pack', url: 'packs/na.pmtiles', styleId: 'worldview-dark', attribution: 'OSM' });
  assert.ok(typeof fromPack === 'object' && fromPack.sources['basemap']?.type === 'vector' && fromPack.sources['basemap'].url === 'pmtiles://packs/na.pmtiles');
  assert.equal(styleForBasemap({ kind: 'vector-style', id: 'ofm', styleUrl: 'https://tiles.openfreemap.org/styles/dark', attribution: 'OpenFreeMap' }), 'https://tiles.openfreemap.org/styles/dark');
  const esri = styleForBasemap({ kind: 'esri-world-imagery', id: 'esri', attribution: '' });
  assert.ok(typeof esri === 'object' && esri.sources['basemap']?.type === 'raster' && /Powered by Esri/.test(esri.sources['basemap'].attribution ?? ''));
...
test('overlay layers: one source per layer, unique ids, cluster options from rules, valid against the style validator', () => {
  ...
  assert.deepEqual(layers.map((l) => l.id), overlayLayerIds('aircraft'));
  assert.ok(interactiveLayerIds('aircraft').every((id) => layers.some((l) => l.id === id)));
  const style: MapStyle = { version: 8, name: 't', glyphs: DEFAULT_GLYPHS_URL, sources: { 'wv:aircraft': source }, layers };
  assert.deepEqual(validateStyle(style), []);
...
test('geojson: feature conversion carries style properties; circles and density cells become polygons', () => {
  const point: RenderFeature = { id: 'obj:a', objectId: 'a', geometry: { kind: 'point', position: { latitude: 10, longitude: 20, altitudeM: 900 } }, style: { styleClass: 'aircraft', icon: 'aircraft', rotationDegrees: 45, label: 'X', labelPriority: 60, freshness: 'STALE' }, interactive: true, priority: 50, layer: 'aircraft' };
  ...
  const circle: RenderFeature = { ...point, id: 'c', geometry: { kind: 'circle', center: { latitude: 0, longitude: 0 }, radiusM: 1000 } };
  ...
  const density: RenderFeature = { ...point, id: 'd', geometry: { kind: 'density', bounds: { west: 0, south: 0, east: 1, north: 1 }, count: 9, intensity: 0.5 }, style: { styleClass: 'fire.density' } };
  ...
  assert.equal(featureGeometry({ ...point, geometry: { kind: 'line', positions: [{ latitude: 0, longitude: 0 }] } }), undefined, 'degenerate lines dropped');
  assert.equal(featureGeometry({ ...point, geometry: { kind: 'circle', center: { latitude: 0, longitude: 0 }, radiusM: 0 } }), undefined);
});
...
test('source model: diff application marks only touched layers dirty; replaceLayers and layer moves', () => {
  ...
  const f = (id: string, layer: string, lat = 0): RenderFeature => ({ id, geometry: { kind: 'point', position: { latitude: lat, longitude: 0 } }, style: { styleClass: layer }, interactive: true, priority: 1, layer });
  assert.deepEqual(model.apply({ upsert: [f('a', 'aircraft'), f('b', 'vessel')], remove: [] }).sort(), ['aircraft', 'vessel']);
  ...
  assert.deepEqual(model.collection('moved').features.map((x) => x.properties.id), ['m1']);
...
test('picking: overlay features resolve to PickResults, MapLibre clusters to synthetic ids, non-interactive skipped', () => {
  const base = { layer: { id: 'wv:aircraft:symbol' }, source: 'wv:aircraft', geometry: { type: 'Point', coordinates: [20, 10, 500] } };
  const r = toPickResult([{ ...base, properties: { id: 'obj:a', objectId: 'a', interactive: true } }], { x: 1, y: 2 }, { lng: 0, lat: 0 })!;
  assert.deepEqual(r, { featureId: 'obj:a', objectId: 'a', position: { latitude: 10, longitude: 20, altitudeM: 500 }, screen: { x: 1, y: 2 } });
  assert.equal(toPickResult([{ ...base, properties: { id: 'obj:n', interactive: false } }], { x: 1, y: 2 }, { lng: 0, lat: 0 }), null);
  assert.equal(toPickResult([{ ...base, source: 'basemap', properties: { id: 'x' } }], { x: 1, y: 2 }, { lng: 0, lat: 0 }), null, 'basemap features are not picks');
  const cluster = toPickResult([{ ...base, properties: { cluster: true, cluster_id: 42, point_count: 7 } }], { x: 1, y: 2 }, { lng: 5, lat: 6 })!;
  assert.equal(cluster.featureId, 'mlcluster:aircraft:42');
  assert.equal(cluster.objectId, undefined);
  const poly = toPickResult([{ ...base, geometry: { type: 'Polygon', coordinates: [] }, properties: { id: 'event:z', eventId: 'z' } }], { x: 1, y: 2 }, { lng: 5, lat: 6 })!;
...
test('view: map ↔ ViewState round trip with pitch convention and altitude coupling', () => {
  ...
  assert.deepEqual(resolveMapFlyTarget({ position: { latitude: 1, longitude: 2 }, bounds: { west: 0, south: 0, east: 1, north: 1 } }, v), { kind: 'bounds', bounds: [0, 0, 1, 1] });
  assert.deepEqual(resolveMapFlyTarget({ position: { latitude: 1, longitude: 2 }, zoom: 30 }, v), { kind: 'center', center: [2, 1], zoom: 22 });
  assert.equal((resolveMapFlyTarget({ position: { latitude: 1, longitude: 2 } }, { ...v, zoom: 3 }) as { zoom: number }).zoom, 10, 'default fly zoom');
});

test('attribution markup, pmtiles protocol registration and icon registry', () => {
  const markup = attributionMarkup([{ id: 'b', text: 'Data <B>', onScreen: false }, { id: 'a', text: 'Esri', url: 'https://www.esri.com', onScreen: true }, { id: 'dup', text: 'Data <B>', onScreen: false }]);
  assert.deepEqual(markup, ['<a href="https://www.esri.com" target="_blank" rel="noopener">Esri</a>', 'Data &lt;B&gt;']);
