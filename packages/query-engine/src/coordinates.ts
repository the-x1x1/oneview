...
interface Component { value: number; axis: 'lat' | 'lon' | null }
...
function readDecimal(text: string): Component | undefined {
  ...
  return { value: letter === 'S' || letter === 'W' ? -magnitude : magnitude, axis: LAT_LETTERS.has(letter) ? 'lat' : 'lon' };
}

/** 21°18'25"N or 21 18 25 N or 21°18.5'N — degrees, optional minutes, optional seconds, mandatory hemisphere letter. */
const DMS_COMPONENT = /^(\d{1,3})\s*(?:°|\s)\s*(\d{1,2}(?:\.\d+)?)?\s*(?:['′]|\s)?\s*(\d{1,2}(?:\.\d+)?)?\s*(?:["″])?\s*([NSEW])$|^([NSEW])\s*(\d{1,3})\s*(?:°|\s)\s*(\d{1,2}(?:\.\d+)?)?\s*(?:['′]|\s)?\s*(\d{1,2}(?:\.\d+)?)?\s*(?:["″])?$/i;
...
function readDms(text: string): Component | undefined {
  ...
  if (!Number.isFinite(deg) || !Number.isFinite(min) || !Number.isFinite(sec) || min >= 60 || sec >= 60) return undefined;
...
export function parseCoordinates(text: string): ParsedCoordinate | UnsupportedCoordinate | undefined {
  ...
  if (MGRS_RE.test(t)) return { kind: 'unsupported', note: 'MGRS grid references are not supported; enter decimal degrees or DMS.' };
  if (UTM_RE.test(t)) return { kind: 'unsupported', note: 'UTM coordinates are not supported; enter decimal degrees or DMS.' };
...
function assemble(first: Component, second: Component, kind: 'decimal' | 'dms'): ParsedCoordinate | undefined {
  ...
  if (first.axis === 'lon' || second.axis === 'lat') { lat = second.value; lon = first.value; }
  else { lat = first.value; lon = second.value; }
