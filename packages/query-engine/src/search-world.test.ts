  ...
  const state = stateWith(clock, [
    {
      ...
      lat: 36.0,
      lon: 140.0,
      payload: { magnitude: 5.7, place: '20 km E of Tokyo, Japan' },
    },
    {
      providerId: 'usgs-earthquakes',
      objectType: 'earthquake',
      ...
      lat: 35.5,
      lon: 139.5,
      payload: { magnitude: 4.1, place: 'Near Yokohama, Japan' },
    },
    {
      providerId: 'usgs-earthquakes',
      objectType: 'earthquake',
      ...
      lat: 19.4,
      lon: -155.3,
      payload: { magnitude: 2.1, place: 'Kilauea, Hawaii' },
    },
    {
      providerId: 'adsb-lol',
      objectType: 'aircraft',
      ...
      lon: -157.9,
      altitudeM: 10_000,
      payload: { icao24: 'a1b2c3', callsign: 'UA123' },
    },
    {
      providerId: 'adsb-lol',
      objectType: 'aircraft',
      ...
      lon: -118.0,
      altitudeM: 2_000,
      payload: { icao24: 'c0ffee', callsign: 'HAL45' },
    },
    {
    ...
    },
    {
      ...
      lat: 51.5,
      lon: -0.12,
      payload: { name: 'Honolulu Street London' },
    },
  ]);
...
test('searchWorld: "ISS" is an object result first (present or not in live state)', () => {
  ...
  state.ingest(
    [
      {
        id: 'celestrak:25544:' + new Date(clock.now()).toISOString(),
        providerId: 'celestrak',
        externalId: '25544',
        objectType: 'satellite',
        observedAt: new Date(clock.now()).toISOString(),
        receivedAt: new Date(clock.now()).toISOString(),
        position: { latitude: 10, longitude: 20, altitudeM: 420_000 },
        payload: { noradId: 25544, name: 'ISS (ZARYA)' },
...
test('searchWorld: distance bias, limit, events and determinism', () => {
  ...
  assert.deepEqual(
    searchWorld('fires near Los Angeles', { state, gazetteer, now }),
    searchWorld('fires near Los Angeles', { state, gazetteer, now }),
  );
