...
test('parseSearch: "Honolulu" → place (city first), "HNL" → airport, coordinates → place', () => {
  ...
  assert.equal(parseSearch('hnl', ctx).intents[0]!.place!.id, 'airport:iata:HNL', 'lower-case code still resolves through the gazetteer');
  ...
  assert.deepEqual(parseSearch('N 21.3 W 157.9', ctx).intents[0]!.place!.position, { latitude: 21.3, longitude: -157.9 });
  ...
  assert.equal(parseSearch('12junk, 34oops', ctx).intents.some((i) => i.place?.kind === 'coordinate'), false, 'never guesses coordinates');
});
...
test('parseSearch: identifiers — ISS, icao24, callsign, MMSI, NORAD', () => {
  ...
  assert.deepEqual(parseSearch('A1B2C3', ctx).intents[0]!.objectHint!.idCandidates, ['aircraft:icao24:a1b2c3'], 'hex is lower-cased');

  const cs = first('UA123', 'query')!;
  assert.deepEqual(cs.query, { objectTypes: ['aircraft'], filters: [{ field: 'labels.callsign', op: 'eq', value: 'UA123' }] });
  assert.equal(parseSearch('UA123', ctx).intents.some((i) => i.kind === 'object'), false, 'callsign is a filter, never an id (ADR-011)');
  assert.deepEqual(first('ual123', 'query')!.query!.filters![0]!.value, 'UAL123');

  const both = parseSearch('AB1234', ctx);
  assert.ok(both.intents.some((i) => i.kind === 'object') && both.intents.some((i) => i.kind === 'query'), 'ambiguous hex/callsign yields both candidates');

  assert.deepEqual(first('366123456', 'object')!.objectHint, { type: 'vessel', idCandidates: ['vessel:mmsi:366123456'] });
  assert.deepEqual(first('norad 25544', 'object')!.objectHint, { type: 'satellite', idCandidates: ['satellite:norad:25544'] });
  assert.deepEqual(first('sat 25544', 'object')!.objectHint, { type: 'satellite', idCandidates: ['satellite:norad:25544'] });
  assert.deepEqual(first('NORAD:48274', 'object')!.objectHint, { type: 'satellite', idCandidates: ['satellite:norad:48274'] });
});
...
test('parseSearch: object types with spatial phrases', () => {
  ...
  assert.deepEqual(japan.query!.region, { kind: 'bounds', bounds: { west: 122.9, south: 24, east: 146.1, north: 45.6 } });

  const la = first('fires near Los Angeles', 'query')!;
  assert.deepEqual(la.query!.objectTypes, ['fire-detection']);
  assert.deepEqual(la.query!.region, { kind: 'circle', center: { latitude: 34.0522, longitude: -118.2437 }, radiusM: 100_000 });
  ...
  assert.deepEqual(hawaii.query!.region, { kind: 'bounds', bounds: { west: -160.3, south: 18.9, east: -154.8, north: 22.3 } });
  ...
  assert.deepEqual(within.query!.region, { kind: 'circle', center: { latitude: 21.3187, longitude: -157.9224 }, radiusM: 50_000 });
  assert.deepEqual(first('vessels within 20 nm of Pearl Harbor', 'query')!.query!.region, { kind: 'circle', center: { latitude: 21.3649, longitude: -157.9527 }, radiusM: 37_040 });

  const inCal = first('quakes in California', 'query')!;
  assert.equal(inCal.query!.region!.kind, 'bounds');
  assert.deepEqual(first('earthquakes Japan', 'query')!.query!.region, japan.query!.region, 'place without preposition still becomes a region');
  assert.deepEqual(first('planes around Seattle', 'query')!.query!.region, { kind: 'circle', center: { latitude: 47.6062, longitude: -122.3321 }, radiusM: 100_000 });
...
test('parseSearch: thresholds and time phrases', () => {
  ...
  assert.deepEqual(first('M6.5 quakes', 'query')!.query!.filters, [{ field: 'properties.magnitude', op: 'gte', value: 6.5 }]);
  assert.deepEqual(first('earthquakes above 5', 'query')!.query!.filters, [{ field: 'properties.magnitude', op: 'gte', value: 5 }]);
  assert.deepEqual(first('magnitude > 4 earthquakes', 'query')!.query!.filters, [{ field: 'properties.magnitude', op: 'gt', value: 4 }]);
  assert.deepEqual(first('earthquakes below 3', 'query')!.query!.filters, [{ field: 'properties.magnitude', op: 'lte', value: 3 }]);
  assert.deepEqual(first('M5+', 'query')!.query!.objectTypes, ['earthquake'], 'M-form implies earthquakes');

  assert.deepEqual(first('aircraft > 10000 ft', 'query')!.query!.filters, [{ field: 'position.altitudeM', op: 'gt', value: 3048 }]);
  assert.deepEqual(first('aircraft above 30000 feet', 'query')!.query!.filters, [{ field: 'position.altitudeM', op: 'gte', value: 9144 }]);
  assert.deepEqual(first('planes >10000ft', 'query')!.query!.filters, [{ field: 'position.altitudeM', op: 'gt', value: 3048 }]);
  assert.deepEqual(first('ships faster than 20 kt', 'query')!.query!.filters, [{ field: 'motion.speedMps', op: 'gt', value: 10.289 }]);
  assert.deepEqual(first('aircraft at least 500 knots', 'query')!.query!.filters, [{ field: 'motion.speedMps', op: 'gte', value: 257.222 }]);
  ...
  assert.deepEqual(first('fires yesterday', 'query')!.query!.time, { start: '2026-09-20T00:00:00.000Z', end: '2026-09-21T00:00:00.000Z' });
  assert.deepEqual(first('alerts since yesterday', 'query')!.query!.time, { start: '2026-09-20T00:00:00.000Z', end: iso(0) });
  assert.deepEqual(first('quakes past week', 'query')!.query!.time, { start: iso(-7 * 86_400_000), end: iso(0) });
  assert.deepEqual(first('quakes in the last 3 days', 'query')!.query!.time, { start: iso(-3 * 86_400_000), end: iso(0) });
  ...
  assert.deepEqual(first('quakes this week', 'query')!.query!.time, { start: '2026-09-21T00:00:00.000Z', end: iso(0) }, '2026-09-21 is a Monday');
  const combo = first('M4+ earthquakes near Tokyo in the past 2 days', 'query')!;
  assert.deepEqual(combo.query!.region, { kind: 'circle', center: { latitude: 35.6762, longitude: 139.6503 }, radiusM: 100_000 });
...
test('parseSearch: free text, stop words and commands', () => {
  ...
  assert.equal(parseSearch('offline pack', ctx).intents.find((i) => i.kind === 'command')!.command, 'download-offline-pack');
  assert.deepEqual(matchCommands('avi', DEFAULT_COMMANDS).map((m) => m.command.id), ['lens-aviation']);
  assert.equal(parseSearch('fires', { ...ctx, commands: [] }).intents.some((i) => i.kind === 'command'), false);
