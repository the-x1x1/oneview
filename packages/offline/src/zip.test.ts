...
async function expectRejected(
  file: string,
  pattern: RegExp,
  limits?: Parameters<typeof ZipReader.open>[1],
): Promise<void> {
  await assert.rejects(
    ZipReader.open(file, limits),
    (err: unknown) => err instanceof ZipFormatError && pattern.test(err.message),
    `expected ${pattern} for ${path.basename(file)}`,
  );
}
...
test('zip: round-trip of deflate and store entries, streamed from memory and from a file', async () => {
  ...
  assert.deepEqual(
    ...
    ['manifest.json', 'data/big.ndjson', 'maps/x.pmtiles'],
  );
  assert.equal((await r.readEntry('manifest.json')).toString(), '{"a":1}');
  const chunks: Buffer[] = [];
  const res = await r.streamEntry(r.entry('data/big.ndjson')!, (chunk) => {
...
test('zip: entry name policy', () => {
  ...
  const good = [
    ...
    'search/index.json',
    'licenses/NOTICES.md',
    'data/earthquakes-usgs-earthquakes.ndjson',
  ];
...
test('adversarial: zip-slip and absolute/backslash/drive-letter names are rejected at open', async () => {
  ...
  for (const [name, re] of cases) {
    const file = await writeTemp(
      dir,
      `slip-${cases.findIndex((c) => c[0] === name)}.zip`,
      rawZip([ok, { name, data: text('x') }]),
    );
...
test('adversarial: symlink attribute is rejected', async () => {
  ...
  const file = await writeTemp(
    dir,
    'symlink.zip',
    rawZip([ok, { name: 'data/link.geojson', data: text('/etc/passwd'), externalAttributes: (0o120777 << 16) >>> 0 }]),
  );
...
test('adversarial: duplicate names (case-insensitive) are rejected', async () => {
  ...
  const file = await writeTemp(
    dir,
    'dup.zip',
    rawZip([ok, { name: 'data/a.geojson', data: text('1') }, { name: 'DATA/A.GEOJSON', data: text('2') }]),
  );
...
test('adversarial: oversize declared sizes and totals are rejected before inflating', async () => {
  const dir = await tempDir();
  const huge = await writeTemp(
    dir,
    'huge.zip',
    rawZip([ok, { name: 'data/a.geojson', data: text('x'), declaredUncompressed: 3 * 1024 * 1024 * 1024 }]),
  );
  await expectRejected(huge, /limit/);
  const total = await writeTemp(
    dir,
    'total.zip',
    rawZip([
      ok,
      { name: 'data/a.ndjson', data: Buffer.alloc(600, 0x41), method: 0 },
      { name: 'data/b.ndjson', data: Buffer.alloc(600, 0x42), method: 0 },
    ]),
  );
...
test('adversarial: lying uncompressed size is caught while inflating (both directions)', async () => {
  ...
  const under = await writeTemp(
    dir,
    'under.zip',
    rawZip([ok, { name: 'data/a.ndjson', data: payload, declaredUncompressed: 100 }]),
  );
  let r = await ZipReader.open(under);
  await assert.rejects(
    r.streamEntry(r.entry('data/a.ndjson')!, () => undefined),
    /exceeds the declared/,
  );
  await r.close();
  const over = await writeTemp(
    dir,
    'over.zip',
    rawZip([ok, { name: 'data/a.ndjson', data: payload, declaredUncompressed: payload.length + 1 }]),
  );
...
test('adversarial: compression-bomb ratio is rejected at open', async () => {
  ...
  const stored = await writeTemp(
    dir,
    'stored.zip',
    rawZip([ok, { name: 'data/zeros.ndjson', data: zeros, method: 0 }]),
  );
...
test('adversarial: CRC tampering and corrupt deflate data are caught', async () => {
  ...
  const wrongCrc = await writeTemp(
    dir,
    'crc.zip',
    rawZip([ok, { name: 'data/a.ndjson', data: payload, declaredCrc: 0x12345678 }]),
  );
  ...
  await assert.rejects(
    r.streamEntry(r.entry('data/a.ndjson')!, () => undefined),
    /CRC-32 mismatch|inflate failed|declared/,
  );
  await r.close();
  await fs.rm(dir, { recursive: true, force: true });
});
...
test('adversarial: truncated archives and directory/data overruns are rejected', async () => {
  ...
  const overrun = await writeTemp(
    dir,
    'overrun.zip',
    rawZip([
      ok,
      { name: 'data/a.ndjson', data: text('abc'), method: 0, declaredCompressed: 5000, declaredUncompressed: 5000 },
    ]),
  );
...
test('adversarial: encryption, unknown methods, zip64 markers, store-size mismatch and local/central disagreement', async () => {
  ...
  const z64 = await writeTemp(
    dir,
    'z64.zip',
    rawZip([ok, { name: 'data/a.ndjson', data: text('x'), declaredUncompressed: 0xffffffff }]),
  );
  await expectRejected(z64, /zip64|limit/);
  const z64extra = await writeTemp(
    dir,
    'z64x.zip',
    rawZip([ok, { name: 'data/a.ndjson', data: text('x'), centralExtra: Buffer.from([0x01, 0x00, 0x00, 0x00]) }]),
  );
  ...
  const storeMismatch = await writeTemp(
    dir,
    'store.zip',
    rawZip([ok, { name: 'data/a.ndjson', data: text('abcd'), method: 0, declaredUncompressed: 3 }]),
  );
  await expectRejected(storeMismatch, /mismatched sizes/);
  const renamed = await writeTemp(
    dir,
    'renamed.zip',
    rawZip([ok, { name: 'data/a.ndjson', data: text('abcd'), localName: 'data/b.ndjson' }]),
  );
  const r = await ZipReader.open(renamed);
  await assert.rejects(
    r.streamEntry(r.entry('data/a.ndjson')!, () => undefined),
    /local header name differs/,
  );
  await r.close();
  await fs.rm(dir, { recursive: true, force: true });
});
