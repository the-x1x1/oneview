...
export function renderNotices(input: NoticesInput): string {
  ...
  lines.push('');
  lines.push(
    `Built ${input.createdAt} for bounds W ${fmt(input.bounds.west)} · S ${fmt(input.bounds.south)} · E ${fmt(input.bounds.east)} · N ${fmt(input.bounds.north)}.`,
  );
  lines.push('');
  lines.push(
    'This world pack contains data only (no code). Each source below permitted offline packaging and redistribution at build time; the attribution lines must be shown wherever the data is displayed.',
  );
  lines.push('');
  lines.push('## Sources');
  lines.push('');
  for (const src of [...input.sources].sort((a, b) => a.providerId.localeCompare(b.providerId))) {
    ...
    if (files.length)
      lines.push(
        `- Files: ${files.map((f) => `\`${f.path}\`${f.rowCount !== undefined ? ` (${f.rowCount} rows)` : ''}`).join(', ')}`,
      );
    lines.push('');
  }
  ...
  lines.push('');
  lines.push(
    '`search/index.json` is derived from the place and airport layers above and carries the same attribution.',
  );
  lines.push('');
  return lines.join('\n');
}
