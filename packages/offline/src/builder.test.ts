...
import { SEED_AIRPORTS_PROVIDER_ID, SEED_PLACES_PROVIDER_ID, WorldPackBuildError, WorldPackBuilder, resolveRegionBounds, seedPolicies, type WorldPackBuildRequest } from './builder.js';
...
const PROTOMAPS_POLICY: ProviderDataPolicy = {
  ...
  attributionRequired: true, attributionText: 'Protomaps · © OpenStreetMap contributors (ODbL)', termsUrl: 'https://docs.protomaps.com/',
};
const NO_REDIST_POLICY: ProviderDataPolicy = { ...PROTOMAPS_POLICY, redistributionAllowed: false, offlinePackAllowed: true };
...
function policies(extra: Record<string, ProviderDataPolicy> = {}): (id: string) => ProviderDataPolicy | undefined {
  const map: Record<string, ProviderDataPolicy> = { ...seedPolicies(), 'protomaps-builds': PROTOMAPS_POLICY, [USGS]: USGS_POLICY, ...extra };
  return (id) => map[id];
}
...
function baseRequest(dir: string, over: Partial<WorldPackBuildRequest> = {}): WorldPackBuildRequest {
  return {
    id: 'hawaii-test', name: 'Hawaii test', region: { preset: 'hawaii' }, include: ['places', 'airports'],
    sources: { placesGeoJsonPath: PLACES, airportsGeoJsonPath: AIRPORTS }, policies: policies(),
    licenses: (id) => (id.startsWith('worldview-seed') ? 'MIT' : id === USGS ? 'U.S. Government work — public domain' : 'ODbL 1.0'),
    outputPath: path.join(dir, 'hawaii.worldpack'), clock: new VirtualClock(Date.parse('2026-09-21T12:00:00Z')), ...over,
  };
}
...
test('builder: region inputs resolve to bounds (preset, bounds, circle) and bad ones are refused', () => {
  ...
  assert.deepEqual(resolveRegionBounds({ bounds: { west: 1, south: 2, east: 3, north: 4 } }), { west: 1, south: 2, east: 3, north: 4 });
...
test('builder: a provider whose policy forbids packing is refused with a clear error and nothing is written', async () => {
  ...
  const attempt = async (policy: ProviderDataPolicy | undefined, pattern: RegExp) => {
    const req = baseRequest(dir, { include: ['map', 'places'], sources: { pmtilesPath: pm, placesGeoJsonPath: PLACES }, policies: policies(policy ? { 'protomaps-builds': policy } : {}) });
    if (!policy) req.policies = (id) => (id === 'protomaps-builds' ? undefined : policies()(id));
    await assert.rejects(builder.build(req), (err: unknown) => err instanceof WorldPackBuildError && err.code === 'POLICY_REFUSED' && pattern.test(err.message), `expected ${pattern}`);
...
test('builder: missing or invalid sources are refused before writing', async () => {
  const dir = await tempDir();
  const builder = new WorldPackBuilder();
  await assert.rejects(builder.build(baseRequest(dir, { include: ['map'], sources: {} })), (e: WorldPackBuildError) => e.code === 'SOURCE_MISSING');
  await assert.rejects(builder.build(baseRequest(dir, { include: ['earthquakes'], sources: {} })), (e: WorldPackBuildError) => e.code === 'SOURCE_MISSING');
  const notPm = await writeTemp(dir, 'not.pmtiles', Buffer.from('definitely not pmtiles'));
  await assert.rejects(builder.build(baseRequest(dir, { include: ['map'], sources: { pmtilesPath: notPm } })), (e: WorldPackBuildError) => e.code === 'INVALID_SOURCE' && /PMTiles v3/.test(e.message));
  const badGeo = await writeTemp(dir, 'bad.geojson', Buffer.from('{"type":"FeatureCollection","features":[{"type":"Feature","geometry":{"type":"Point","coordinates":[999,0]},"properties":{}}]}'));
  await assert.rejects(builder.build(baseRequest(dir, { include: ['places'], sources: { placesGeoJsonPath: badGeo } })), (e: WorldPackBuildError) => e.code === 'INVALID_SOURCE');
  await assert.rejects(builder.build(baseRequest(dir, { id: 'Bad Id' })), (e: WorldPackBuildError) => e.code === 'INVALID_REQUEST');
  await assert.rejects(builder.build(baseRequest(dir, { include: [] })), (e: WorldPackBuildError) => e.code === 'INVALID_REQUEST');
  await assert.rejects(builder.build(baseRequest(dir, { outputPath: path.join(dir, 'x.zip') })), (e: WorldPackBuildError) => e.code === 'INVALID_REQUEST');
  await fs.rm(dir, { recursive: true, force: true });
});
...
test('builder: Hawaii pack from the seed fixtures — clipped layers, search index, NOTICES, checksums, report', async () => {
  ...
  const req = baseRequest(dir, {
    include: ['map', 'places', 'airports', 'earthquakes'], version: '1.0.0', clock,
    sources: { pmtilesPath: pm, placesGeoJsonPath: PLACES, airportsGeoJsonPath: AIRPORTS, history: history.store, earthquakeWindowDays: 30 },
  });
  ...
  assert.ok(report.layers.places!.kept >= 20 && report.layers.places!.kept < 40, `places kept ${report.layers.places!.kept}`);
  ...
  assert.deepEqual(report.entries.map((e) => e.path), ['maps/hawaii-basemap.pmtiles', 'data/places.geojson', 'data/airports.geojson', 'data/earthquakes.ndjson', 'search/index.json', 'licenses/NOTICES.md']);
  assert.deepEqual(report.sources.map((s) => s.providerId).sort(), ['protomaps-builds', USGS, SEED_AIRPORTS_PROVIDER_ID, SEED_PLACES_PROVIDER_ID].sort());
  ...
  assert.equal(manifest.sourcePolicies.find((p) => p.providerId === 'protomaps-builds')?.attribution, PROTOMAPS_POLICY.attributionText);
  assert.equal(manifest.sourcePolicies.find((p) => p.providerId === USGS)?.license, 'U.S. Government work — public domain');
  ...
  const again = baseRequest(dir, { ...req, outputPath: path.join(dir, 'again.worldpack'), clock: new VirtualClock(Date.parse('2026-09-21T12:00:00Z')), include: ['map', 'places', 'airports'], sources: { pmtilesPath: pm, placesGeoJsonPath: PLACES, airportsGeoJsonPath: AIRPORTS } });
  const first = baseRequest(dir, { ...again, outputPath: path.join(dir, 'first.worldpack'), clock: new VirtualClock(Date.parse('2026-09-21T12:00:00Z')) });
...
test('builder: an empty region still builds but warns, and a circle region clips', async () => {
  const dir = await tempDir();
  const builder = new WorldPackBuilder();
  const report = await builder.build(baseRequest(dir, { region: { bounds: { west: 0, south: -1, east: 1, north: 0 } } }));
  ...
  const circle = await builder.build(baseRequest(dir, { outputPath: path.join(dir, 'circle.worldpack'), region: { center: { latitude: 21.31, longitude: -157.86 }, radiusM: 60_000 } }));
