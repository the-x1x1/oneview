...
test('offline: Hawaii world pack builds, installs and serves local search with the network off', async () => {
  ...
  try {
    ...
    const report = await new WorldPackBuilder().build({
      ...
      policies,
      licenses: (id) => (id === USGS ? 'U.S. Government work — public domain' : 'MIT'),
      outputPath,
      clock,
    });
    ...
    const registry = new WorldPackRegistry({
      dataDir,
      appVersion: '0.1.0',
      clock,
      flags: () => ({ history: true, collections: true, localAircraft: false }),
    });
    ...
    assert.ok(
      attribution.some((a) => /U\.S\. Geological Survey/.test(a)),
      'USGS attribution travels with the pack',
    );
    assert.ok(
      attribution.some((a) => /WORLDVIEW project \(MIT\)/.test(a)),
      'seed attribution travels with the pack',
    );
    ...
    assert.equal(
      capabilities.localMap,
      false,
      'no basemap extract is bundled in the repository (see docs/OFFLINE-PACKS.md)',
    );
      ...
      JSON.stringify(
        {
          ...
          build: {
            ...report,
            outputPath: path.basename(report.outputPath),
            reportPath: path.basename(report.reportPath),
            layers: { ...report.layers },
          },
          verification: {
            ok: verification.ok,
            entries: verification.entries.map((e) => ({
              path: e.path,
              kind: e.kind,
