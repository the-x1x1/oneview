...
export async function seededHistory(
  dataDir: string,
  clock: { now(): number },
  ...
  const hawaii: Array<[number, number]> = [
    [19.42, -155.29],
    [19.48, -155.61],
    [20.7, -156.2],
    [21.3, -157.9],
  ...
  hawaii.forEach(([lat, lon], i) =>
    obs.push(quake(`hv${i}`, new Date(now - (i + 1) * 86_400_000).toISOString(), lat, lon, 2.5 + i * 0.4)),
  );
  japan.forEach(([lat, lon], i) =>
    obs.push(quake(`jp${i}`, new Date(now - (i + 2) * 86_400_000).toISOString(), lat, lon, 4.0 + i)),
  );
