...
import type { WorldPackContent, WorldPackContentKind, WorldPackManifest, WorldPackSourcePolicy } from '../../src/manifest.js';
...
export const TEST_POLICY: WorldPackSourcePolicy = {
  providerId: 'worldview-seed-places', license: 'MIT', attribution: 'Seed places: WORLDVIEW project (MIT)', offlinePackAllowed: true, redistributionAllowed: true,
};
...
export const HAWAII_ENTRIES: PlaceEntry[] = [
  { id: 'place:city:honolulu-hi', name: 'Honolulu', altNames: [], kind: 'city', countryCode: 'US', position: { latitude: 21.31, longitude: -157.86 }, importance: 0.85 },
  { id: 'airport:PHNL', name: 'Daniel K. Inouye International Airport', altNames: ['Honolulu Airport'], kind: 'airport', iata: 'HNL', icao: 'PHNL', countryCode: 'US', position: { latitude: 21.32, longitude: -157.92 }, importance: 0.5 },
  { id: 'place:region:oahu-hi', name: 'Oʻahu', altNames: ['Oahu'], kind: 'region', countryCode: 'US', position: { latitude: 21.48, longitude: -157.98 }, importance: 0.8 },
];

export function defaultFiles(): TestPackFile[] {
  const places = Buffer.from(JSON.stringify({ type: 'FeatureCollection', features: HAWAII_ENTRIES.map((e) => ({ type: 'Feature', geometry: { type: 'Point', coordinates: [e.position.longitude, e.position.latitude] }, properties: { id: e.id, name: e.name, altNames: e.altNames, kind: e.kind, importance: e.importance } })) }));
  const index = Buffer.from(JSON.stringify(new PlaceIndex(HAWAII_ENTRIES).toJSON()));
  return [
    { path: 'data/places.geojson', kind: 'geojson', data: places, providerId: 'worldview-seed-places', objectType: 'place', rowCount: HAWAII_ENTRIES.length },
...
export function buildTestPack(opts: TestPackOptions = {}): { bytes: Buffer; manifest: WorldPackManifest } {
  ...
  if (!files.some((f) => f.kind === 'notices')) files.push({ path: 'licenses/NOTICES.md', kind: 'notices', data: Buffer.from('# Data notices\n\nSeed places: WORLDVIEW project (MIT)\n') });
  const contents: WorldPackContent[] = files.map((f) => ({
    path: f.path, kind: f.kind, sizeBytes: f.data.length, sha256: sha256(f.data),
  ...
  const manifest: WorldPackManifest = {
    ...
    contents,
    sourcePolicies: opts.sourcePolicies ?? (contents.some((c) => c.providerId === TEST_POLICY.providerId) ? [{ ...TEST_POLICY }] : []),
  ...
  const entries: RawEntry[] = files.filter((f) => !(opts.omitEntries ?? []).includes(f.path)).map((f) => ({ name: f.path, data: f.data, ...f.raw }));
