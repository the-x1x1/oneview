...
export interface LabelBox { id: string; left: number; top: number; right: number; bottom: number }

export function labelBox(c: LabelCandidate, padding = 0): LabelBox {
  const anchor = c.anchor ?? 'below';
  let left: number, top: number;
  if (anchor === 'center') { left = c.x - c.width / 2; top = c.y - c.height / 2; }
  else if (anchor === 'right') { left = c.x; top = c.y - c.height / 2; }
  else { left = c.x - c.width / 2; top = c.y; }
  return { id: c.id, left: left - padding, top: top - padding, right: left + c.width + padding, bottom: top + c.height + padding };
}
...
export function declutterLabels(candidates: LabelCandidate[], viewport: { width: number; height: number }, options: DeclutterOptions = {}): Set<string> {
  ...
  for (const c of ordered) {
    ...
    const c0 = Math.floor(box.left / cell), c1 = Math.floor(box.right / cell), r0 = Math.floor(box.top / cell), r1 = Math.floor(box.bottom / cell);
    let blocked = false;
    outer: for (let r = r0; r <= r1; r++) for (let col = c0; col <= c1; col++) {
      const bucket = grid.get(`${r}:${col}`);
      if (!bucket) continue;
      for (const other of bucket) if (overlaps(box, other)) { blocked = true; break outer; }
    }
    if (blocked) continue;
    visible.add(c.id);
