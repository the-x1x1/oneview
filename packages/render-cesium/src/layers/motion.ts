import type { RenderMotion } from '@worldview/render-core';
 ...
 * position at this poll and at the next, both SGP4), placed by wall-clock time.
 *
...
export class Movers {
  ...
  set(key: string, marker: Omit<MovingMarker, 'fromMs' | 'toMs'>, motion: Pick<RenderMotion, 'fromMs' | 'toMs'>): void {
    const m: MovingMarker = { ...marker, fromMs: motion.fromMs, toMs: motion.toMs };
...
/**
 * How often moving markers are stepped, from how many metres a pixel covers at the camera:
 * often enough that a low orbit (~7.5 km/s) moves at most half a pixel a step, but never more
 * than 30 times a second nor less than once every two seconds. The whole globe in view,
 * where a satellite crosses a pixel in several seconds, costs two steps a second.
 */
export function motionStepMs(metresPerPixel: number): number {
  if (!Number.isFinite(metresPerPixel) || metresPerPixel <= 0) return 1000 / 30;
  return Math.max(1000 / 30, Math.min(2000, (0.5 * metresPerPixel) / 7.5));
}
