...
export class DensityLayer {
  ...
  constructor(
    private readonly cesium: Pick<
      CesiumLike,
      'Rectangle' | 'groundPrimitivesSupported' | 'createGroundRectangles' | 'ClassificationType'
    >,
  ...
  flush(): boolean {
    ...
    const cells = [...this.cells.entries()].map(([id, c]) => ({
      id,
      rectangle: boundsToRectangle(this.cesium, c.bounds),
      color: this.theme.color(densityColor(c.resolved, c.intensity)),
    }));
    ...
    if (previous) {
      this.primitives.remove(previous);
      ...
      for (const [id, c] of this.cells) {
        ...
        this.entities.set(
          ...
          entities.add({
            id,
            rectangle: {
              coordinates: boundsToRectangle(this.cesium, c.bounds),
              material: this.theme.color(densityColor(c.resolved, c.intensity)),
              outline: false,
