...
export function lineMaterial(cesium: Pick<CesiumLike, 'Material'>, theme: CesiumTheme, resolved: ResolvedStyle, lineStyle: RenderFeature['style']['lineStyle']): MaterialLike {
  const color = theme.color(resolved.color);
  switch (lineStyle ?? 'solid') {
    case 'solid': return cesium.Material.fromType('Color', { color });
    case 'dashed': return cesium.Material.fromType('PolylineDash', { color, gapColor: theme.color({ ...resolved.color, a: 0 }), dashLength: 16 });
    case 'trail': return cesium.Material.fromType('PolylineDash', { color, gapColor: theme.color({ ...resolved.color, a: 0.25 }), dashLength: 12 });
  }
}
...
export class PolylineLayer {
  ...
  constructor(private readonly cesium: Pick<CesiumLike, 'Cartesian3' | 'Material'>, private readonly theme: CesiumTheme, readonly collection: PolylineCollectionLike) {}
  ...
  upsert(feature: RenderFeature, resolved: ResolvedStyle): void {
    ...
    if (!positionsValid(positions)) { this.remove(feature.id); return; }
    ...
    if (existing) { existing.positions = cartesians; existing.material = material; existing.width = width; existing.show = true; return; }
    this.items.set(feature.id, this.collection.add({ id: feature.id, positions: cartesians, width, material, show: true }));
  }
