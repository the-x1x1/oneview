...
interface LabelEntry { label: LabelLike; position: Cartesian3Like; priority: number; width: number; height: number; centered: boolean }
...
export class LabelLayer {
  ...
  constructor(
    private readonly cesium: Pick<CesiumLike, 'Cartesian3' | 'Cartesian2' | 'HeightReference' | 'VerticalOrigin' | 'HorizontalOrigin' | 'LabelStyle'>,
  ...
  upsert(feature: RenderFeature, resolved: ResolvedStyle): void {
    ...
    if (!text) { this.remove(feature.id); return; }
    ...
    const fontPx = centered ? Math.max(10, Math.min(16, Math.round(iconSizePx(resolved, true) * 0.4))) : resolved.labelFontPx;
    ...
    if (existing) {
      ...
      l.position = position; l.text = text; l.font = font; l.fillColor = fill; l.outlineColor = outline;
      l.pixelOffset = new this.cesium.Cartesian2(0, offsetY);
      l.heightReference = heightReferenceFor(this.cesium, mode);
      existing.position = position; existing.priority = priority; existing.width = width; existing.height = height; existing.centered = centered;
      return;
    }
  ...
  declutter(project: (position: Cartesian3Like) => { x: number; y: number } | undefined, viewport: { width: number; height: number }): number {
    ...
    for (const [id, e] of this.items) {
      ...
      if (!p) { behind.push(id); continue; }
      candidates.push({ id, x: p.x, y: p.y, width: e.width, height: e.height, priority: e.priority, anchor: e.centered ? 'center' : 'below' });
    }
