...
export type ImageryFactoryModule = Pick<CesiumLike, 'TileMapServiceImageryProvider' | 'UrlTemplateImageryProvider' | 'ArcGisMapServerImageryProvider' | 'OpenStreetMapImageryProvider' | 'IonImageryProvider' | 'IonWorldImageryStyle' | 'buildModuleUrl'>;
...
export function createNaturalEarthImagery(cesium: ImageryFactoryModule): Promise<ImageryProviderLike> {
  return cesium.TileMapServiceImageryProvider.fromUrl(cesium.buildModuleUrl('Assets/Textures/NaturalEarthII'), { credit: NATURAL_EARTH_ATTRIBUTION });
}

export function createEsriWorldImagery(cesium: ImageryFactoryModule): Promise<ImageryProviderLike> {
  return cesium.ArcGisMapServerImageryProvider.fromUrl(ESRI_WORLD_IMAGERY_URL, { credit: ESRI_ATTRIBUTION, enablePickFeatures: false });
}
...
export function createXyzImagery(cesium: ImageryFactoryModule, opts: { url: string; attribution: string; maxZoom: number; tileSize?: number }): ImageryProviderLike {
  const init: { url: string; credit: string; maximumLevel: number; tileWidth?: number; tileHeight?: number } = { url: opts.url, credit: opts.attribution, maximumLevel: opts.maxZoom };
  if (opts.tileSize !== undefined) { init.tileWidth = opts.tileSize; init.tileHeight = opts.tileSize; }
...
export function createIonImagery(cesium: ImageryFactoryModule, assetId: number, accessToken: string): Promise<ImageryProviderLike> {
