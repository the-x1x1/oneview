...
import { altitudeForBounds, cameraToViewState, normalizeHeadingDegrees, resolveFlyTarget, viewStateToCamera } from './view.js';
...
test('label declutter: priority wins collisions, deterministic ties, off-screen hidden', () => {
  ...
  const t1 = { ...a, priority: 5 }, t2 = { ...b, priority: 5 };
  ...
  const many = Array.from({ length: 200 }, (_, i) => ({ id: `m${i}`, x: (i % 20) * 40, y: Math.floor(i / 20) * 40, width: 30, height: 12, priority: i }));
...
test('view: camera ↔ ViewState round trip, zoom/altitude coupling, heading normalisation', () => {
  const view = cameraToViewState({ longitude: 10 * DEG, latitude: 50 * DEG, height: 12_000, heading: 90 * DEG, pitch: -45 * DEG, rectangle: { west: 9 * DEG, south: 49 * DEG, east: 11 * DEG, north: 51 * DEG } });
  ...
  const again = cameraToViewState({ longitude: back.longitude * DEG, latitude: back.latitude * DEG, height: back.height, heading: back.heading, pitch: back.pitch });
  ...
  assert.equal(fromZoom.height, zoomToAltitudeM(10, 50), 'altitude derived with the shared contract formula at the view latitude');
...
test('view: fly targets prefer bounds, then altitude, then zoom, then a sensible default', () => {
  const current = { center: { latitude: 0, longitude: 0 }, altitudeM: 8_000_000, zoom: 1, headingDegrees: 0, pitchDegrees: -90 };
  const bounds = { west: -1, south: -1, east: 1, north: 1 };
  assert.deepEqual(resolveFlyTarget({ position: { latitude: 0, longitude: 0 }, bounds }, current), { kind: 'bounds', bounds });
  assert.equal((resolveFlyTarget({ position: { latitude: 1, longitude: 2 }, altitudeM: 300 }, current) as { height: number }).height, 300);
  ...
  assert.equal((resolveFlyTarget({ position: { latitude: 1, longitude: 2 } }, current) as { height: number }).height, 50_000);
...
test('picking: primitive ids and entity ids resolve to feature ids; anchors prefer the feature position', () => {
  ...
  const point: RenderFeature = { id: 'obj:a', objectId: 'a', geometry: { kind: 'point', position: { latitude: 1, longitude: 2, altitudeM: 3 } }, style: { styleClass: 's' }, interactive: true, priority: 1, layer: 'l' };
  ...
  assert.deepEqual(r, { featureId: 'obj:a', objectId: 'a', position: { latitude: 1, longitude: 2 }, screen: { x: 5, y: 6 } });
...
test('feature routing and store: routes per geometry/style, replaceLayers, layer moves and removals', () => {
  const base: RenderFeature = { id: 'p', geometry: { kind: 'point', position: { latitude: 0, longitude: 0 } }, style: { styleClass: 'aircraft' }, interactive: true, priority: 1, layer: 'aircraft' };
  assert.deepEqual(routeFeature(base), ['point']);
  assert.deepEqual(routeFeature({ ...base, style: { styleClass: 'aircraft', icon: 'aircraft', label: 'X' } }), ['billboard', 'label']);
  assert.deepEqual(routeFeature({ ...base, geometry: { kind: 'cluster', position: { latitude: 0, longitude: 0 }, count: 3, bounds: { west: 0, south: 0, east: 1, north: 1 } } }), ['cluster', 'label']);
  assert.deepEqual(routeFeature({ ...base, geometry: { kind: 'line', positions: [] } }), ['polyline']);
  assert.deepEqual(routeFeature({ ...base, geometry: { kind: 'polygon', rings: [] }, style: { styleClass: 'e', label: 'L' } }), ['polygon', 'label']);
  assert.deepEqual(routeFeature({ ...base, geometry: { kind: 'circle', center: { latitude: 0, longitude: 0 }, radiusM: 1 } }), ['circle']);
  assert.deepEqual(routeFeature({ ...base, geometry: { kind: 'density', bounds: { west: 0, south: 0, east: 1, north: 1 }, count: 1, intensity: 1 } }), ['density']);
  ...
  const r2 = store.apply({ upsert: [{ ...base, layer: 'moved', style: { styleClass: 'a', icon: 'aircraft' } }], remove: ['q'] });
  ...
  store.apply({ upsert: [{ ...base, id: 'm1', layer: 'moved' }, { ...base, id: 'm2', layer: 'moved' }], remove: [] });
...
test('attribution: escaped credit markup, diffed static credits, map credit follows the stack', () => {
  ...
  assert.equal(attributionHtml({ text: 'Esri', url: 'https://www.esri.com' }), '<a href="https://www.esri.com" target="_blank" rel="noopener">Esri</a>');
  assert.equal(attributionHtml({ text: 'js', url: 'javascript:alert(1)' }), 'js', 'non-http urls are not linked');
  const display = { credits: [] as Array<{ html: string; showOnScreen: boolean }>, addStaticCredit(c: { html: string; showOnScreen: boolean }) { this.credits.push(c); }, removeStaticCredit(c: { html: string; showOnScreen: boolean }) { this.credits.splice(this.credits.indexOf(c), 1); } };
  const sync = new CreditSync(display, (html, showOnScreen) => ({ html, showOnScreen }));
  const first = sync.apply([{ id: 'usgs', text: 'USGS', onScreen: false }, { id: 'osm', text: '© OpenStreetMap contributors', onScreen: true }]);
  assert.deepEqual(first, { added: ['usgs', 'osm'], removed: [] });
  assert.deepEqual(sync.apply([{ id: 'usgs', text: 'USGS', onScreen: false }, { id: 'osm', text: '© OpenStreetMap contributors', onScreen: true }]), { added: [], removed: [] }, 'same list is a no-op');
  ...
  map.show('A'); map.show('A'); map.show('B');
  assert.equal(display.credits.length, 1);
...
test('viewer: GEV options (no widgets, msaa 4, preserved buffer) and trackpad pinch relay', () => {
  ...
  const controller = { zoomEventTypes: [0, { eventType: 3, modifier: 0 }] as Array<number | { eventType: number; modifier: number }> | undefined };
  const target = {
    scene: { screenSpaceCameraController: controller },
    container: { addEventListener: (t: string, l: (e: unknown) => void) => listeners.set(t, l), removeEventListener: (t: string) => listeners.delete(t) },
    canvas: { dispatchEvent: (e: object) => { dispatched.push(e); return true; } },
  };
  const created: Array<Record<string, unknown>> = [];
  const dispose = installTrackpadPinchZoom({ CameraEventType: { WHEEL: 3 }, KeyboardEventModifier: { CTRL: 1 } }, target as never, (_type, init) => { const e = { ...init, type: 'wheel' }; created.push(e); return e; });
  ...
  let prevented = 0;
  const wheel = (over: Partial<{ ctrlKey: boolean; deltaMode: number; deltaY: number }>) => ({ ctrlKey: true, deltaMode: 0, deltaX: 0, deltaY: -3, deltaZ: 0, screenX: 1, screenY: 2, clientX: 3, clientY: 4, preventDefault: () => { prevented++; }, stopPropagation: () => undefined, ...over });
  relay(wheel({}));
...
test('geometry and billboard helpers: height modes, validation, icon sizes, rotation', () => {
  ...
  assert.ok(Math.abs(headingToBillboardRotation(90) + Math.PI / 2) < 1e-12, 'clockwise heading → counter-clockwise rotation');
});
