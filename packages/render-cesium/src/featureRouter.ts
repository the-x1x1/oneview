...
export function routeFeature(feature: RenderFeature): FeatureRoute[] {
  ...
  switch (g.kind) {
    ...
    case 'cluster':
      ...
      break;
    case 'line': routes.push('polyline'); break;
    case 'polygon': routes.push('polygon'); if (feature.style.label) routes.push('label'); break;
    case 'circle': routes.push('circle'); if (feature.style.label) routes.push('label'); break;
    case 'density': routes.push('density'); break;
  }
  return routes;
}

export interface StoredFeature { feature: RenderFeature; routes: FeatureRoute[] }
...
export class FeatureStore {
  ...
  get(id: string): RenderFeature | undefined { return this.byId.get(id)?.feature; }
  routesOf(id: string): FeatureRoute[] { return this.byId.get(id)?.routes ?? []; }
  layers(): string[] { return [...this.byLayer.keys()]; }
  idsInLayer(layer: string): string[] { return [...(this.byLayer.get(layer) ?? [])]; }
  values(): IterableIterator<StoredFeature> { return this.byId.values(); }

  /** Returns the removals (with their previous routes) and the upserts (with previous routes, if any) in application order. */
  apply(update: FeatureUpdate): { removed: Array<{ id: string; previous: StoredFeature }>; upserted: Array<{ previous: StoredFeature | undefined; next: StoredFeature }> } {
    ...
    for (const feature of update.upsert) {
      ...
      if (previous && previous.feature.layer !== feature.layer) this.byLayer.get(previous.feature.layer)?.delete(feature.id);
      this.byId.set(feature.id, next);
      let set = this.byLayer.get(feature.layer);
      if (!set) { set = new Set(); this.byLayer.set(feature.layer, set); }
      set.add(feature.id);
  ...
  clear(layer?: string): string[] {
    if (!layer) { const ids = [...this.byId.keys()]; this.byId.clear(); this.byLayer.clear(); return ids; }
  ...
  private delete(id: string): StoredFeature | undefined {
    ...
    if (set) { set.delete(id); if (set.size === 0) this.byLayer.delete(prev.feature.layer); }
    return prev;
  }
}
