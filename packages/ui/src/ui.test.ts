...
import {
  Button, IconButton, Toggle, Tabs, Panel, FieldList, Section, Drawer, Popover, Tooltip, Search, CommandPalette, VirtualList,
  StatusBadge, SourceBadge, Timeline, initialTimelineState, EmptyState, ErrorState, LoadingState, Dialog, Icon, ICON_NAMES, GLYPHS, badgeLabel, type VirtualListProps,
} from './index.js';
...
function assertNoPlaceholders(html: string) {
  assert.ok(!/TODO|placeholder text|lorem ipsum|coming soon/i.test(html.replace(/placeholder="[^"]*"/g, '')), 'no placeholder text in markup');
}
...
test('Icon: every glyph renders as a single svg with paths; decorative by default, labelled when asked', () => {
  ...
  assert.ok(labelled.includes('role="img"') && labelled.includes('aria-label="Search"') && labelled.includes('<title>Search</title>'));
...
test('Button / IconButton: variants, pressed state, accessible names', () => {
  ...
  assert.ok(html.includes('type="button"') && html.includes('wv-btn--primary') && html.includes('aria-pressed="true"') && html.includes('Play'));
...
test('Toggle: role=switch with aria-checked and label association', () => {
  const html = render(h(Toggle, { checked: true, onChange: noop, label: 'Automatic updates', description: 'Downloads are verified' }));
  assert.ok(html.includes('role="switch"') && html.includes('aria-checked="true"') && html.includes('Automatic updates') && html.includes('Downloads are verified'));
  assert.match(html, /aria-labelledby="[^"]+"/);
});

test('Tabs: tablist with roving tabindex and controlled panel', () => {
  const items = [{ id: 'selection', label: 'Selection' }, { id: 'sources', label: 'Sources', badge: 3 }, { id: 'timeline', label: 'Timeline', disabled: true }];
...
test('Panel, FieldList, Section: undefined values are omitted, region landmark', () => {
  const html = render(h(Panel, { title: 'Sources', subtitle: '8 sources', region: true }, h(Section, { title: 'Identity' }, h(FieldList, { rows: [{ label: 'Callsign', value: 'UAL123' }, { label: 'Registration', value: undefined }] }))));
...
test('Drawer / Dialog: closed renders nothing; open renders dialog semantics', () => {
  ...
  assert.ok(drawer.includes('<aside') && drawer.includes('wv-drawer--left') && drawer.includes('Settings') && drawer.includes('content'));
  const dialog = render(h(Dialog, { open: true, title: 'Diagnostics', description: 'Runtime state', onClose: noop, footer: h(Button, null, 'Close') }, 'body'));
  assert.ok(dialog.includes('role="dialog"') && dialog.includes('aria-modal="true"') && dialog.includes('Diagnostics') && dialog.includes('Runtime state') && dialog.includes('body'));
...
test('Popover / Tooltip', () => {
  const pop = render(h(Popover, { label: 'Speed', open: true, renderTrigger: (p) => h('button', { ...p, type: 'button' }, 'Open'), children: 'inside' }));
  assert.ok(pop.includes('aria-expanded="true"') && pop.includes('role="dialog"') && pop.includes('inside'));
  const closed = render(h(Popover, { label: 'Speed', renderTrigger: (p) => h('button', { ...p, type: 'button' }, 'Open'), children: 'inside' }));
...
test('Search: combobox semantics, results list, empty text', () => {
  ...
  const html = render(h(Search, { label: 'Search', value: 'hon', onChange: noop, results, onPick: noop, inline: true }));
  assert.ok(html.includes('role="combobox"') && html.includes('aria-autocomplete="list"') && html.includes('role="listbox"') && html.includes('Honolulu') && html.includes('Place'));
  const empty = render(h(Search, { label: 'Search', value: 'zzz', onChange: noop, results: [], onPick: noop, inline: true, emptyText: 'Nothing found' }));
...
test('CommandPalette: closed renders nothing; open lists available commands with groups and shortcuts', () => {
  ...
  assert.ok(html.includes('aria-label="Command palette"') && html.includes('Jump to live') && html.includes('Timeline') && !html.includes('Hidden command'));
  assertNoPlaceholders(html);
});
...
test('VirtualList: renders only the windowed rows with listbox semantics; empty state', () => {
  ...
  const html = render(h(RowList, { items, itemHeight: 32, height: 320, label: 'Feed', getKey: (it) => it.id, renderItem: (it) => h('span', null, it.label), selectedKey: 'row-2' }));
  ...
  const empty = render(h(RowList, { items: [], itemHeight: 32, height: 100, label: 'Feed', getKey: () => '', renderItem: () => null, emptyState: h(EmptyState, { title: 'No events yet' }) }));
...
test('StatusBadge / SourceBadge: labels for every kind; text carries meaning', () => {
  ...
  const src = render(h(SourceBadge, { name: 'USGS Earthquakes', status: 'LIVE', providerId: 'usgs-earthquakes', onClick: noop }));
  assert.ok(src.includes('<button') && src.includes('USGS Earthquakes') && src.includes('Live') && src.includes('data-provider="usgs-earthquakes"'));
});
...
test('Timeline: transport, speeds, slider semantics, availability honesty', () => {
  ...
  assert.ok(html.includes('role="slider"') && html.includes('aria-disabled="true"') && html.includes('No history available yet'));
  assert.ok(html.includes('>LIVE<'));
  const hist = { ...state, mode: 'HISTORICAL' as const, cursorMs: NOW - 3600_000, availability: [{ objectType: 'earthquake', ranges: [{ startMs: NOW - 86_400_000, endMs: NOW }] }] };
  const h2 = render(h(Timeline, { state: hist, dispatch: noop, typeLabels: { earthquake: 'Earthquakes' } }));
  assert.ok(h2.includes('aria-label="Play"') && h2.includes('Earthquakes') && h2.includes('aria-disabled="false"') && h2.includes('07:00:00 UTC') && h2.includes('HISTORICAL'));
  assert.ok(h2.includes('wv-timeline__mark'));
});

test('Empty / Error / Loading states', () => {
  const e = render(h(EmptyState, { title: 'No selection', description: 'Pick something on the map', action: { label: 'Search', onClick: noop } }));
  assert.ok(e.includes('role="status"') && e.includes('No selection') && e.includes('Pick something on the map') && e.includes('Search'));
