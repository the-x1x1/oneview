...
> Release 0.1.0-rc.2 — release candidate. Builds are unsigned; see
...
Download `WorldView-Setup-0.1.0-rc.2.exe` (per-user install, no admin) or
`WorldView-Portable-0.1.0-rc.2.zip` from the releases page, verify it against
...
```powershell
Get-FileHash .\WorldView-Setup-0.1.0-rc.2.exe -Algorithm SHA256
```
